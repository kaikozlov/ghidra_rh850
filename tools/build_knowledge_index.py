...
def artifact_registry(data_generated: list[str], owners: dict[str, list[str]],
                      suites: dict[str, dict[str, object]]) -> str:
    lines = [
        ...
        "",
        "| Artifact | Owning suite(s) | Gate test(s) |",
        "|---|---|---|",
    ]
...
def main() -> int:
    ...
    for sname, suite in suites.items():
        ...
        for path in suite.get("paths", []):  # type: ignore[union-attr]
            pp = REPO / path
            if pp.is_dir():
                for f in sorted(pp.rglob("*")):
                    if f.is_file() and f.suffix in (
                        ...
                        owners[repo_path(str(f), REPO)].append(sname)
            else:
                owners[repo_path(path, REPO)].append(sname)
    ...
    registry_body = artifact_registry(data_generated, owners, suites)
    ...
    unmapped_data = sorted(p for p in data_files if p not in owners)
    ...
    for path in unmapped_data:
        warnings.append(f"data artifact not owned by verification.toml: {path}")
    ...
    for path in sorted(data_files):
        lines.append(f"| `{path}` | {', '.join(unique(owners.get(path, []))) or '—'} |")
    ...
    labels = {
        ...
        "unmapped_data": "data/ files without verification ownership",
    }
    for key in labels:
