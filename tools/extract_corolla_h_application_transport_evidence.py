#!/usr/bin/env python3
"""Compact H-native decompiler evidence for residual application transport roles."""
...
OUT=ROOT/'data/generated/corolla_8965H1202000_application_transport_decompiler_evidence.json'
TARGETS=[0x7A382,0x7A402,0x7ADC2,0x7B040,0x47ADA]
...
def main():
 ...
 p={'schema':'corolla-h-application-transport-evidence-v1','software_id':'8965H1202000','image':{'path':str(RAW.relative_to(ROOT)),'codeflash_size':len(raw),'codeflash_sha256':sha(raw)},'source_corpus':{'path':str(SRC.relative_to(ROOT)),'sha256':sha(SRC.read_bytes()),'boundary':'only the five already-identified application transport bodies are compacted'},'functions':out,'function_count':len(out)}
