...
# project-path/environment/headless safety policy lives in tools/project/run_headless;
...
usage() {
  cat <<'EOF'
Usage: tools/project/export_ghidra_project.sh list
       tools/project/export_ghidra_project.sh PROFILE [OUTPUT [SUMMARY_OUTPUT]]
...
TARGET_FIELD() { python3 "$ROOT/tools/project/analysis_target.py" "$ANALYSIS_TARGET" --field "$1"; }
...
"$ROOT/tools/project/run_headless" \
...
case "$PROFILE" in
  application-tx-producers)
    ...
    ;;
  outside-functions)
    python3 "$ROOT/tools/project/apply_function_discovery_adjudications.py" \
...
PY
    ;;
  semantic-coverage)
    python3 "$ROOT/tools/project/apply_semantic_review_status.py" \
      --ledger "$PRIMARY_OUT" \
...
PY
    ;;
  project-inventory)
    python3 "$ROOT/tools/project/project_inventory.py" validate "$PRIMARY_OUT"
    ;;
esac
