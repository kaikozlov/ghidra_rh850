...
usage(){ echo "Usage: tools/project/snapshot_target_project.sh --target TARGET --project-dir DIR [--parity-project-dir DIR]"; }
while (($#)); do case "$1" in
...
for d in "$PROJECT_DIR" ${PARITY_DIR:+"$PARITY_DIR"}; do
  ...
  python3 "$ROOT/tools/project/fingerprint_processor.py" --expect "$d/processor_manifest.json" --ghidra-version "$GHIDRA_VERSION" --cli-version "$GHIDRA_CLI_VERSION" --sla "$V850_EXT_DIR/data/languages/v850e3.sla" >/dev/null
done
TMP=$(mktemp -d "$BUILD_TMP/target-finalize.XXXXXX"); trap 'rm -rf "$TMP"' EXIT
export_inventory(){ GHIDRA_ANALYSIS_TARGET="$TARGET" PROJECT_DIR="$1" "$ROOT/tools/project/export_ghidra_project.sh" project-inventory "$2" >/dev/null; }
export_inventory "$PROJECT_DIR" "$TMP/a.jsonl"
if [[ -f "$BASE" ]]; then
  python3 "$ROOT/tools/project/project_inventory.py" compare "$BASE" "$TMP/a.jsonl"
else
  [[ -n "$PARITY_DIR" ]] || { echo "first target promotion requires --parity-project-dir" >&2; exit 1; }
  export_inventory "$PARITY_DIR" "$TMP/b.jsonl"
  cmp -s "$TMP/a.jsonl" "$TMP/b.jsonl" || { echo "independent target rebuild inventories differ" >&2; python3 "$ROOT/tools/project/project_inventory.py" compare "$TMP/a.jsonl" "$TMP/b.jsonl" || true; exit 1; }
...
python3 "$ROOT/tools/project/project_layout.py" validate-snapshot --snapshot-dir "$SNAP" --project-name "$PN"
