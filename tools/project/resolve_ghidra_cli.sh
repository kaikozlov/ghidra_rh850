...
#
# Usage:
#   tools/project/resolve_ghidra_cli.sh                    # existing resolver behavior
#   tools/project/resolve_ghidra_cli.sh --ensure-vendored # rebuild missing/stale vendored CLI; no system fallback
#   tools/project/resolve_ghidra_cli.sh --ensure-vendored --allow-system
#
...
if ((ENSURE_VENDORED)); then
  ...
  if ((NEED_BUILD)); then
    if [[ -x "$ROOT/tools/project/build_ghidra_cli.sh" ]] && command -v cargo >/dev/null 2>&1; then
      echo "Vendored ghidra CLI is missing or stale — rebuilding..." >&2
      "$ROOT/tools/project/build_ghidra_cli.sh" >&2
