...
3. **resolver readiness** — whether the image is in the exact state
   ``tools/security/resolve_secoc_patch_image.sh`` accepts (geometry + optional
...
def check_resolver_readiness(
    ...
    result: dict[str, Any] = {
        "resolver": "tools/security/resolve_secoc_patch_image.sh",
    ...
    result["next_step"] = (
        "run: tools/security/resolve_secoc_patch_image.sh <image> - (disposable Ghidra import; "
