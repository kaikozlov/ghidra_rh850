...
SCHEMA = "corolla-pre-tss3-opendbc-message-comparison-v3"
...
def build(args: argparse.Namespace) -> dict:
    ...
    if bridge["schema"] != "corolla-8965H1202000-openpilot-state-bridge-v4":
    ...
    roles = [
        ...
        role_row(
            ...
            consequence="Do not extend/sign classic 0x2E4. H/F instead consume protected FD 0x0B6 signal255 as a target steering-angle command, with signal254 selecting cooperative-control profiles. The controller-equivalent signal255 scale is 1024/17870 deg/count (~1.000121519 mrad/count); recover the exact OEM unit/profile labels, remaining request/validity/cadence semantics, SecOC freshness/key behavior, and stock-source suppression before injection.",
        ),
        role_row(
    ...
    return {
        ...
        "evidence_boundary": (
            ...
            "Whole-vehicle openpilot messages may legitimately be absent from an EPS image; only EPS-local command/feedback migrations are treated as architectural evidence. B6 controller-equivalent physical scaling is closed; the exact OEM B6 unit/profile labels and upstream producer/authentication remain open."
        ),
        "upstream": {
