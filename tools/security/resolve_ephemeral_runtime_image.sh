...
usage() {
  cat <<'EOF'
Usage: tools/security/resolve_ephemeral_runtime_image.sh CODEFLASH.bin [manifest.json] [variant-id]
...
from pathlib import Path
import sys
from tools.security.build_ephemeral_runtime_manifest import load_codeflash, sha256
try:
