...
3. ./tss3-unified-signer bringup /tmp/tss3-bringup
   ...
   On exact Camry, bringup then becomes deliberately operator-paced: it releases Panda and waits for you before restarting Brake/EPB only; after that command exits and the Brake application F181 has returned, it releases Panda again and waits for you before restarting FRC only. After the FRC application returns it releases Panda again and waits for you to decide when the Toyota network has settled before final DRCC/EPS-resident verification. There is no chained automatic peer reset and no DTC clear.
