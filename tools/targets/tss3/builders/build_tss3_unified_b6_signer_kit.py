...
def build(target: str, out: Path) -> dict:
    ...
    testing = f"""Unified TSS3 functional-0x777 signer test kit\nTarget: {target}\nSource commit: {commit}\n\nPreferred tester flow:\n1. ./tss3-unified-signer doctor\n2. Put the vehicle in NRTD/READY=0 and Park.\n3. ./tss3-unified-signer bringup /tmp/tss3-bringup\n   The command proves the stock 0x777 mailbox, installs the volatile resident, then pauses.\n   Transition directly to READY/Park WITHOUT powering the EPS off, remain stationary, and press Enter.\n   Continue only if qualification passes.\n4. Camry F33 only: if the programming bootstrap leaves DRCC unavailable, run\n   ./tss3-unified-signer recover-drcc /tmp/tss3-recover-drcc.json\n   while READY/Park/stationary. This preserves the RAM resident, snapshots/clears the\n   peer communication DTCs through the already-qualified maintenance transport, and\n   requires the FRC DRCC permission DIDs to recover. It is a same-cycle experiment,\n   not proof that the programming outage can be made invisible.\n5. READY/Park/stationary: ./tss3-unified-signer replace-current /tmp/tss3-replace-current.json\n   This derives the live 0x025 steering angle, verifies fresh healthy stationary/Park state, sends one no-offset C7 generation, then sequence zero to release.\n6. STOP after stationary qualification. Do not infer road readiness from a passing signer pulse; Corolla's native-shape stock-ACC cancel carrier is implemented but receiver acceptance, physical steering, and stock-system coexistence remain vehicle qualification items.\n7. Once those vehicle-level contracts are closed, normal openpilot integration streams changed nonzero C7 generations on functional 0x777 and sends sequence zero when lateral is inactive.\n\nManual equivalent: preflight -> install in NRTD, direct NRTD->READY without OFF, then qualify.\nC7 is the only recurring host control tag. The same authenticated 4-KiB payload contains Camry, Crown, and Corolla runtime profiles and selects the exact profile on the ECU; no C6 helper-loading phase remains. This runtime does not patch CodeFlash and retains the legacy target-specific signer for recovery.\nA full EPS power cycle removes the RAM resident and requires bringup again.\n"""
