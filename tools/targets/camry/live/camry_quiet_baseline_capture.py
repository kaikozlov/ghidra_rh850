...
reset CAN communications or rewrite speeds/modes. It accepts only SILENT or
NOOUTPUT safety; if cooperative handoff has already fallen to SILENT, it selects
NOOUTPUT so the observer remains ACK-capable while keeping the intercept relay
closed. Panda power-save is then temporarily disabled so all receive
transceivers/IRQs are available.

No CAN data frame is ever submitted. USB heartbeats keep the receive-only Panda
...
def capture(duration: float, output: Path) -> dict[str, object]:
    ...
    try:
        ...
        result: dict[str, object] = {
            ...
            "duration_s": (capture_end_ns - start_ns) / 1e9,
