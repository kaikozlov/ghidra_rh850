...
def load_august(path: Path) -> dict:
    ...
    with gzip.open(path, 'rt') as f:
        for line in f:
            ...
            elif bus == 2 and address == 0x08A and len(data) == 32:
    ...
    for family in (direct,):
        for streams in family.values():
            for rows in streams.values():
                rows.sort()
    for family in (a8, ca, c9):
        for rows in family.values():
            rows.sort()
    return {'direct': direct, 'a8': a8, 'ca': ca, 'c9': c9}
...
def resume_evidence(role_report: dict, groups: dict) -> list[dict]:
    ...
    for group in ('3b', '3c'):
        z = groups[group]
        a8, ca, c9 = z['can_08a_2_32'], z['can_0ca_0_32'], z['can_0c9_2_32']
        for event in role_report['stock_resumes'][group]:
            ...
            for delta in (-700, -500, -300, -200, -100, 0, 100, 200, 300, 500, 700):
                ...
                ar, cr, nr = latest(a8, t), latest(ca, t), latest(c9, t)
                ad, cd, nd = ar[2:].astype(np.uint8), cr[2:].astype(np.uint8), nr[2:].astype(np.uint8)
...
def build() -> dict:
    ...
    return {
        'schema': 'camry-longitudinal-request-candidates-v1',
        ...
        'topology_normalization': {
            ...
            'rule': 'Compare Toyota network role and physical direction first; raw Panda bus number is era-dependent.',
        },
