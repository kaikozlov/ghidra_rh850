...
- the 0x081 steering-reference word: nearest-time-paired equality strata against
...
- the full 0x08A byte census: constants, latch-mirror bits, active flags,
  the B26 mod-64 freshness counter, and the bounded absence of a separate
  damping-gain alphabet on that PDU;
...
def byte_census(rows_08a):
    ...
    return {
        ...
        "damping_gain_absence": {
            ...
            "conclusion": ("B24 is the only byte with the recorder assist {0/50/100} alphabet; "
                           "no other 0x08A byte carries that same alphabet. This scan does not "
                           "identify a separate damping-gain field on 0x08A and does not exclude "
                           "a differently encoded field or a carrier outside 0x08A."),
        },
    }
...
def delayed_ack_section(streams, onsets):
    ...
    return {
        ...
        "onsets": results,
        "classification": ("zero delayed persistent flips under the declared criterion at either "
                           "clean onset; no delayed persistent grant/ack-class announcement was "
                           "identified on the captured Bus-4 segment"),
    }
...
def main() -> int:
    ...
    art = {
        ...
        "interpretation": {
            "conclusion": (
                "No separate stock-LTA actuation/grant CAN carrier was identified within the "
                "declared retained-capture search: 0x0B6, legacy 0x131/0x2E4, and the four "
                "configured F33 telemetry Tx IDs are absent, and the delayed-persistent-flip scan "
                "found no grant/ack-class announcement. These observations do not imply an "
                "incomplete EPS interface or a private EPS stub: the independent VAR-066 "
                "GTS+/exact-F33/repin join places B6 on the reached Bus-4 Brake/EPS segment. "
                "Stock LTA also need not publish B6 because exact F33 has a B6-inactive internal "
                "path into motor-current control."
            ),
            "new_carrier": (
                "0x081 B16:B17 (s16BE) strongly supports the same mode-switching steering-reference "
                "quantity as 0x08A B18:B19: static pairs frequently agree, drive-B ID11 batch "
                "medians agree within one count in 92.0555% of shared batches, and in manual state "
                "the word tracks measured 0x025 angle at the F33 B6 scale. Moving-pair disagreement "
                "and drive-A fast-slew strata preclude a blanket byte-equality claim."
            ),
            "proof_boundary": (
                ...
                "transform, a grant, or a causal command path. No 0x08A-to-B6 transform is "
                "established. Production output remains unauthorized."
            ),
        },
    }
