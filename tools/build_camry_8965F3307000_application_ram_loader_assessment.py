...
def build() -> dict:
    ...
    need(family_routes==[0,0xFF,2,0xFF,0xFF,4], f"generated-COM family routing drift: {family_routes}")
    need(u32(image,0x218E4)==0x21ACC, "route-0 record base drift")
    xcp_tx_route=[u16(image,0x21AF4+i*2) for i in range(5)]
    need(xcp_tx_route==[0x66,0x102,0x8,0x37,0x4], f"XCP TX route record drift: {xcp_tx_route}")
    tx_handle_map=u32(image,0x22E38)
    need(tx_handle_map==0x22DB8, f"RSCFD Tx-handle map base drift: {tx_handle_map:#x}")
    tx_handle_entry=tx_handle_map+2*0x37
    need(tx_handle_entry==0x22E26 and image[tx_handle_entry:tx_handle_entry+2]==bytes((1,8)), "XCP hardware Tx-handle mapping drift")
    ...
    # predicate reports active. Owner 0 is configured on bit 4 (0x10); the
    # foreground owner machine has a three-tick delay, but its online event is
    # independently dynamic, so 15 ms is a configured delay rather than a
    # promise that every vehicle state admits XCP after 15 ms.
    need(image[0x21866]==0x10 and image[0x21BE1]==1 and image[0x21BE2]==0x10, "XCP communication-owner mask drift")
    need(u16(image,0x21B8C)==3, "XCP owner delay drift")
    need(image[0x7F23C:0x7F250].hex()=="8600c332c431a60f1b9101061fffe30704537f00", "XCP owner-active predicate drift")
    need(image[0x7F030:0x7F042].hex()=="8600c332200ee1ffc4310052460f19917f00", "XCP owner-online setter drift")
    ...
    return {
      ...
      "control_transfer_audit":{
        "raw_indirect_control_transfer_census":{
          "total":496,"application":487,
          "mnemonics_total":{"jarl":403,"jmp":93},
          "mnemonics_application":{"jarl":395,"jmp":92},
          "reset_thunk_outside_function_classifier":"0x00000032",
          "source":"ghidra/scripts/investigate/ExportIndirectControlTransfers.java",
        },
        "computed_call_sites_reviewed_total":495,
        "computed_call_sites_reviewed_application":487,
        "computed_call_classifier_provenance":{
          "direct_referenced_definition_sites":161,
          "direct_referenced_non_ram_sites":152,
          "direct_referenced_lower_ram_sites":9,
          "direct_referenced_xcp_window_sites":0,
          "locally_resolved_without_operand_reference_sites":330,
          "no_definition_within_24_instruction_backtracker_sites":4,
          "direct_lower_ram_cells":[
            {"cell":"0xFEBF0FD0","consumers":["0x0000435E","0x0000437C","0x0000440E"],"closure":"boot-only authenticated payload callback; below application XCP window"},
            {"cell":"0xFEBF6B04","consumers":["0x00073EE6"],"writer":"0x00073EEE","fixed_targets":["0x000766F4","0x000767EA"],"closure":"writer selects one of two fixed CodeFlash targets"},
            {"cell":"0xFEBF117C","consumers":["0x00088614"],"closure":"guarded callback family; fixed CodeFlash installers plus complement checks"},
            {"cell":"0xFEBF1194","consumers":["0x00088716","0x0008873C"],"closure":"guarded callback family; fixed CodeFlash installers"},
            {"cell":"0xFEBE5628","consumers":["0x00091678","0x000916BE"],"closure":"temporary service callback derived from fixed CodeFlash service configuration; request bytes do not become a pointer"},
          ],
          "all_direct_lower_ram_cells_below_xcp_write_window":True,
          "bounded_note":"The classifier reports source-cell provenance, not complete target semantics. Fixed descriptor/table families and the four backtracker misses are closed separately below.",
        },
        ...
        "exception_saved_pc_audit":{
          "exception_return_sites":["0x00020102","0x00065C60","0x00071372","0x00071456","0x00071502","0x000715AE","0x00071A90","0x00071C40"],
        ...
        "audit_scripts":{
          ...
          "indirect_transfers":{"path":"ghidra/scripts/investigate/ExportIndirectControlTransfers.java","sha256":sha_file(ROOT / "ghidra/scripts/investigate/ExportIndirectControlTransfers.java")},
