...
- **SecOC** — runtime CAN authentication on six RX PDUs through ICU-S slot 4.
  ...
  selector 4 in software. Its sole configured stock caller is a CAN-fed test
  harness that stock RoutineControl RID `0x100F` startRoutine activates through wrapper
  `0x8A782 -> 0x69018`; generated bytes are still compared locally and the stock
  graph has no production SecOC transmit path. Classic sender construction and
  freshness are resolved, and Stage 7 specifies a minimum foreground
  application-context signing proxy through the serialized ICU driver. Remaining
  questions are dynamic: live slot-4 command-5 permission,
