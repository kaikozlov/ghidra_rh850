...
## Current GTS+ corpus-wide DDB census (2026-09-07)

The current release is now censused as a reproducible artifact rather than by
one-off shell counts. Across **Gen + Spe** there are **1,805 DDB files**
(NA 546 / EU 612 / JP 647), of which **1,703 are parseable ECU databases**
(NA 512 / EU 578 / JP 613). The remaining 102 are 90 regional `M_*/V_*/U_*`
string databases, six format-1 `Toyota.ddb` masters, and six format-3
`Viewer.ddb` databases. `data/generated/gtsplus_2026/ddb_corpus_census.json` is
the pinned inventory.

More importantly, the current `KgpDataCtrl.dll` table factory is no longer an
unknown boundary. `CDbTableRead::MakeTable` at `0x10081C10` dispatches format 1
to the `0x10081C80` factory and format 2 to the `0x100845D0` factory. Their
type-indexed jump tables call exported `CDb*Table` constructors directly. The
independent factory artifact therefore assigns an exact OEM class name to every
type in both namespaces (`0..93` format 1, `0..171` format 2); all **162 table
types actually present in the 1,703 ECU DBs are named**. This replaces the old
"unknown table id" bucket with concrete work queues. Examples that matter to
the current control audit include:

- 68/71 = `CDbActTestP5Table` / `CDbRoutineActTestP5Table`;
- 108/109/110 = `CDbDDRHeaderInfoDataIdTable` /
  `CDbDDRCaseInfoDataIdTable` / `CDbDDRMonitorTable`, followed by the matching
  trigger/address/invalid-condition tables; the same DDR family repeats at
  127–132, 136–141 and 145–150;
- 118 = `CDbFactorDataTable`, not an anonymous event table;
- master 23 = `CDbSubSystemTable`, 44 = `CDbInstallingEcuListTable`, 55 =
  `CDbCanBusListTable`, and 60 = `CDbCustomListMTable`.

The exact map is generated by `extract_gtsplus_factory_table_map.py`, not by the
parser it audits. `parse_ddb.py` now carries the complete current class map and
`verify_gtsplus_ddb_factory_table_map.py` requires every parser name to agree
with the independently recovered host factory.

### Type 118 is Record-of-Behavior factor metadata

Current host ownership makes the meaning of type 118 substantially narrower
than the earlier bounded guess. `GetRoBP5_DT.dll` imports
`CDbFactorDataResRecords::GetFactorData` and `GetFactorHelp`; the host class is
`CDbFactorDataTable`. In `PSC_P5.ddb` the table has 396 20-byte records. Its
string-bearing fields resolve factor/help text such as `History of no IG on in
49 Days` and `Stop Start-up (Steering Lock)`, while the key/group fields vary
across groups 301/401/501/601/701/801. It is therefore RoB factor/help metadata
used by the P5 Record-of-Behavior path, **not** evidence for a generic PSC
power-state-history protocol or a constant `0x12D` record class.

### Shared string indices are not typed references

The census also catches a specific trap that can manufacture false cross-ECU
semantics. `M_English.ddb` entries `0x30CE/0x30F5/0x30F6/0x30F8/0x30FB` spell
`Req Limit EPS ...`, but `CDbDataIdForDmTable::FindDbItem1` consumes type-61
`+0x02` as a **u16 data-id/DID** and `SetRecString` is a no-op. Numeric equality
with a string index is not a string reference. Concrete witness: in
`EMPS_P5.ddb`, type-61 value `0x30F5` participates in DID `0x10F5/0x30F5`, whose
actual type-62 monitor is `Motor 2 V Phase Duty`; in `HV_P5.ddb`, `0x30CE` is
likewise an alternate DID for transmission-status monitors. Corpus greps must
therefore resolve a field through its host consumer before attaching a global
string-table name to it. The genuine P4 `EPC_P4` active test `Request to Limit
EPS` remains separate evidence.

## Steering/control semantics exposed by the full corpus

A Gen+Spe sweep across NA/EU/JP contains **24,332 Active Test rows** (14,152
type-11, 7,358 type-68, 2,822 type-71) and 3,717 unique resolved names. There is
no row whose *literal name* says "set steering angle" or "set steering torque",
but that is only a vocabulary observation and does **not** close a diagnostic
control path. The complete sweep exposes a stronger unresolved lead on the
current Camry path:

- Current Camry category 498 (`FRC_P5`, `Front Recognition Camera 2`) has a
  routine Active Test named **`PCS Collision Avoidance Assist`**, Active Test
  ID `0xA411`, RID **`0x1604`**. The recovered executor geometry is fixed and
  parameterless: start `31 01 16 04`, stop `31 02 16 04`, result
  `31 03 16 04`. This proves Toyota ships an executable RoutineControl surface
  under that control name; it does **not** yet prove what physical action the
  routine causes or that the exact Camry accepts it under the current session /
  authorization state.
- The same `FRC_P5` database exposes `PCS Collision Avoidance Assist Function`
  at DID `0x2122` (alternate `0x4122`), bit 8, `OFF/ON`. This is a stock
  observation point for whether that named function is active.
- The Camry install set also contains `A_B_CAN_P5` (SRS Airbag). Its four exact
  `CDbDDRMonitorTable` groups contain repeated OEM monitor names
  **`Emergency Steering Assist Request Flag`** and
  **`Collision Avoidance Assist 1/2/3`**. These are recorder/snapshot semantics,
  not yet live-DID or wire-field assignments, but they establish that current
  P5 diagnostic schemas explicitly model emergency-steering/collision-avoidance
  request state.
- `PCS2_P5` is not one of this Camry's mounted candidates, so it is used only as
  a same-generation semantic oracle. It names **`PCS Steering Request`** at DID
  `0x1017/0x3017`, bit 8, plus `Emergency Steering Assist Invalid 1/2` at bits
  25/26 of `0x100B/0x300B`. `DRS_P5` similarly names `Active Steering Control
  Permission`; the P6 successor `ADCU_P6` names `Active Steering Status`. These
  cross-generation/cross-ECU rows establish Toyota's vocabulary, not ownership
  by the exact F33 FRC.

This changes the next RE question. The diagnostic corpus is **not closed** by a
negative Active Test name search. The high-value work is to determine what
`FRC_P5` RID `0x1604` changes, correlate its `PCS Collision Avoidance Assist
Function` state with the already recovered lateral-request traffic, and use the
P5 emergency-steering/request names as oracles while tracing the corresponding
state in the exact Camry participants. Static planning remains safe/read-only;
no routine request is sent by the extractor or CLI unless an explicit execution
path is invoked.

The complete 34-ECU Camry-HV install set itself remains owned by the vehicle
resolver artifact (TMS-089). `IPA_P5` is one useful negative-at-name-level
check: it has only `Front Buzzer`/`Rear Buzzer` direct Active Tests, while its
368 monitors expose Advanced Park, Remote Park, Steer-By-Wire and power-steering
equipment/status vocabulary. That absence does not prove IPA lacks an indirect
steering-control role.
...
- Findings with this document as canonical home: [SECOC-034](../reference/index.md#finding-secoc-034), [TMS-015](../reference/index.md#finding-tms-015), [TMS-090](../reference/index.md#finding-tms-090)
- Corrections with this document as canonical home: [CORR-031](../reference/index.md#correction-corr-031), [CORR-036](../reference/index.md#correction-corr-036)
