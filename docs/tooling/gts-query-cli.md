...
Role `0x63` handles **multi-control Active Test initialization**. `--item` is the type-33 group ID, not a direct member ID. The current generic P5 plugin reads 12-byte `CDbMultiDidIdTable` rows keyed by u16 `+0x00`, takes member Active Test ID from `+0x02`, and sorts on u32 `+0x06`. The current DataMonitor runtime additionally consumes the low u16 at `+0x06` as input slot 1/2, materializes each member through the ordinary type68 direct executor, and OR-composes complete start/stop frames. All member DID bytes must match; mixed-DID groups fail closed. The generated catalog therefore exports input slots plus `materializable`/`blocked` composer status. Categories may bind role `0x63` without having type-33 groups; the planner reports an empty census instead of manufacturing groups.
...
For role `0x08`, `--item` is the direct Active Test ID from the selected category's type-68 `CDbActTestP5Table`. The planner joins that exact row to the current plugin's initialization state machine: when `initial_read_mode == 0`, selector `0xCA` supplies a base `22FFFF` request and the plugin substitutes the row's `+0x34` DID into request bytes 1/2 before send. It also reproduces the plugin's linked-Data-Monitor lookup by DID and bit range. The planner continues into the shared P5 runtime executor: type67 `CDbDataIdForActTable` supplies the DID encoding mode; selector `0x9D` supplies `2FFFFF03 -> 6F` for start/short-term adjustment and selector `0x64` supplies `2FFFFF00 -> 6F` for return-control. The displayed `+N` remains intentionally symbolic in the offline plan because bit geometry proves only a minimum, but the **live source of N is now closed**: `CCommCachePlusP5::CheckSupportDid` resolves selector `0xCA`, sends `22 <DID>`, requires a response longer than the three-byte `62 <DID>` echo, and appends `received_length - 3` to the per-ECU `CCmdDataIdLengthList`. The exporter therefore emits a separate `runtime_length_probe` even when role-`0x08` initial-read mode is 1; this probe is support/runtime state, not the selected-test initial-value display transaction. `active-test-wire-minimum` stays a static illustration, while `runtime-length-probe` describes the exact read-only transaction that can materialize N on a live ECU. The planner itself still sends nothing and does not assert that role `0x06` would expose the test on a particular vehicle. Unknown direct-test IDs fail closed.
...
Registry schema v2 also carries the recovered ordinary-P5 Data Monitor decoder as
`p5-linear-msb0-v1`. Current `DatMonCommon.dll::CPacketConvertor` proves that signal
bit numbering is MSB-first within each byte (`bit 0 = byte 0 bit 7`) and multi-byte
fields are assembled big-endian. `CommandDataLib.dll::CComDataConvert` then applies
unsigned or two's-complement interpretation at the declared bit width followed by
`trunc_toward_zero(raw * Mul / Div) + Offset`; the DDB decimal count renders that
integer as `value / 10^decimal_point_count`, and pattern-display lookup compares the
converted integer before decimal rendering. The role-`0x41` signal-info plugin joins
exactly those type-13 physical fields into `CCmdConversionTbl`; that table calls the
basic conversion overload, not the separate special-format overload used by unrelated
time/chassis/illuminance-style display paths. Each exported DID signal explicitly names
its decoder kind so future non-P5 formats can fail closed instead of inheriting this
contract implicitly.

`tools/gts registry toyota-current --out FILE.zip` is the default Comma-side export rather than a vehicle-specific registry. Bundle schema **`toyota-diagnostics-bundle-v2`** keeps Toyota's resolver layers independent: all regional vehicle/type-41/type-59 tables, install sets, all 2,136 category identities, class-`0x10D` category+phase routes, per-category D1/D2/DD frames, literal support-plugin dispatch, family-local `support_mode`, Toyota transport-controller selection, and lazy decoded catalogs. Catalog availability no longer decides whether a category exists or is routable. Current support-family selection comes from Toyota's DLL table (`p3/p4/p5/p6`), not generation guesses. The ordinary Toyota P5 mode reproduces `22 01 01` plus MSB-first group/member expansion while retaining advertised `xx00` IDs and skipping Toyota's `F300`/`FD00` group queries; Subaru/Suzuki/Mazda/Hino modes remain distinct rather than inheriting that implementation. Standard P6 is fully exported: DID `22 A1 00 -> A1nn -> nn00..nnFF` with `A1FD/A1FE` retained but unexpanded, and RID `31 01 D1 00 -> D1nn -> nn00..nnFF` with `D1F0/D1FE` retained but unexpanded. Phase `0x18`/`0x38` routes are materialized through Toyota's `CCommCtrlISO15765_29BitCan` normal-fixed address construction (`0x18DA<target>F1` request / `0x18DAF1<target>` response), rather than treating the raw class-`0x10D +0x08` target byte as an 11-bit CAN ID. Other Toyota controllers remain explicitly typed so local transport-executor coverage cannot become a category-support decision. P3/P4 also carry their `Spe/Toyota.ddb` live probe programs and semantic type-41 criteria; a VIN hit requiring that second stage is explicitly non-final. VIN10 low5 `5..19` rejection is recorded together with Toyota's separate legacy `SelectCarType.dll` / `SelectCarTypeVin.dll` role bindings, whose binaries are absent from the current corpus and are therefore an unresolved implementation path rather than an unsupported-generation declaration. The bundle deliberately contains no Panda logical-bus default: that is caller/harness state, not Toyota DB semantics. `tests/verify_toyota_diag_bundle.py` regenerates the ZIP byte-for-byte and verifies these boundaries plus Camry and 4Runner install/route resolution. The older Camry JSON remains only a compatibility/evidence fixture.
...
behind **Central Gateway**. These are Toyota vehicle-network domain names, not Panda bus
numbers: the active post-repin profile remains Panda bus0. The registry separately
retains the tracked 2026-08-26 **pre-repin** identity observations without rewriting that
history: EPS F181 `8965F3307000` + `8A3113303100` / F18C
`8965033K9011J2740743`; FRC F181 `8646F3315000`, part `8646C06091`, F18C
`TN69400026030404235J`; Brake F181 `F152633K0000`, part `8954147040`, F18C
...
The legacy Camry registry schema **v6** predates the universal bundle and remains an evidence/compatibility fixture. It added Toyota vehicle/mounted-ECU/capability metadata and category routes to the older execution-model surface. Its historical generation/session fields are retained as evidence but are not permission inputs in the universal runtime. Earlier catalog sections remain preserved in shape:
