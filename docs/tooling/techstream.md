...
capture synchronized stock-LTA traffic—to recover the remaining
...
`tests/verify_tss3_b6_sender_attribution.py`. The next code-level sender target is
therefore a decoded, vehicle/era-matched category-435 `07B0` application paired
with the matched `0792` FRC application (or synchronized stock-LTA traffic), not
another scan of the same opaque FRC CUWs.
