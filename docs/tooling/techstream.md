...
V18 also narrows the software-identity lookup. `TCUWCanUnifiedCIDGetter.dll`
imports `CUnifiedUtils::ReadSoftwareID` and calls it on the normal node path
**before** its mode-specific dispatch; the imported implementation is byte-pinned
as UDS `22 F1 81` with expected `62 F1 81`. The only recovered extra SWIN path
is the unique mode-2 camera special case: a single `0792` literal is constructed
into the comparison object, and only that branch calls `GetSWINForFCM`, which
uses direct `0x792→0x79A` transport and DID `0x1FFF`. The CID getter contains no
`07B0` literal. Thus **F181 is the generic Unified software-ID acquisition path**
to use when identifying a prospective category-435 target; `1FFF` must not be
transferred from the FRC. This is a host-route conclusion, not proof that a
future `07B0` package uses `P5-Unified04` or that F181 is a Brake-specific OEM
DID name.

Therefore the next directed target is a true-TSS3 CUW whose **`Node01/DiagID`
is `07B0`**, ideally vehicle/era-matched to the `0792` FRC package, plus the
decoded `FRC_P5` image or a synchronized stock-LTA capture spanning FRC/Brake/
EPS. The remaining task is the actual payload transformation and SecOC sender/
key/freshness ownership. The FRC and Brake/EPB Techstream Active-Test catalogs
are exhausted as setpoint-writer candidates; use them only as fixed/actuator
probes. Do not promote the diagnostic graph into a daisy-chain payload claim
without producer code or synchronized traffic evidence.
