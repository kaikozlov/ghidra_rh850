...
Toyota then resolves the actual mounted set rather than treating every statically possible ECU as present. `CDAExecGetMountEcuList` / `GetMountEcuListNoCnfm` starts from the selected installation data and runs the common `CEcuConnectCheck` machinery across the candidates, producing a live `CCmdEcuInfo` list. This is the OEM equivalent of vehicle-specific ECU enumeration: the master database supplies the expected topology and the runtime connection checks remove absent alternatives.
