...
The generic **current P5 per-DTC freeze-frame path** is also closed at the request and raw-record framing layer. Master role `0xB5` uses category-0 `GetEachFrzFrmDatP5_DT.dll` as the current ordinary-P5 fallback; exact P6 categories override that role with `GetEachFrzFrmDatP6_DT.dll`. The ordinary P5 plugin resolves selector `0xCF` to **`19 04 00 00 00 FF`** with positive check **`59 04`**. `CGetFrzFrmDatP5Base::SetDTCToSndFrame` writes the selected 24-bit DTC into request bytes 2/3/4, leaving byte5 as snapshot-record selector `0xFF` (all records). The receive parser treats bytes after `59 04` as `DTC[3] | status | repeated(snapshot_record:u8 | identifier_count:u8 | repeated(DID:be16 | length:u8 | data[length]))`; unlike ordinary UDS generic DID parsing, each snapshot DID carries an explicit one-byte data length. `GetFFDType` maps snapshot-record numbers into Toyota FFD families, but signal-level interpretation still depends on freeze-frame-specific metadata and is not inferred from Data Monitor tables. The universal diagnostic bundle therefore exports this as `p5_dtc_snapshot` with raw DID blocks exact and semantic decoding explicitly unresolved. This path is read-only and directly matches ISO 14229 subfunction `0x04 reportDTCSnapshotRecordByDTCNumber`.
