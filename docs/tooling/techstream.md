...
For P5, `GetSupportP5_DT.dll` exposes `(ecuId, optionId, id) -> support`: option 1/2/3 dispatches PID/DID/RID checks. DID support is exact and live: selector `0xC8` sends `22 01 01`; the `62 01 01` payload is decoded MSB-first into supported `xx00` groups, then each selected `xx00` group is queried and expanded to `xx01..xxFF`, skipping the terminal alias to the next group's `xx00`. Routine support is rooted at selector `0xCC`, `31 01 10 01 -> 71...`. P6 is independently identified by `GetSupportMultiP6_DT.dll`; its current recovered support roots include DID `22 A1 00` and routine `31 01 D1 00`. Those roots are sufficient for a live response witness, but the complete P6 hierarchy is not yet reproduced, so the runtime reports enumeration as an implementation boundary rather than calling P6 unsupported.
