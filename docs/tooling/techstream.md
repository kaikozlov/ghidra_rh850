...
The resulting Toyota resolution pipeline is therefore **master vehicle decision -> selected install set -> live mounted ECU list -> per-ECU PID/DID/RID support -> diagnostic operation**. Independent tooling should reproduce that division rather than substitute a hand-curated vehicle profile or require bespoke per-DID "wire proof" before using already-recovered common P5 behavior. This resolver establishes identity, installation, and advertised diagnostic capability; it does not by itself establish that an arbitrary mutating operation is authorized or safe to execute.

Machine-readable evidence is `data/generated/gtsplus_2026/vehicle_resolver_semantics.json`, generated by `tools/techstream/extract_gtsplus_vehicle_resolver.py` and verified by `tests/verify_gtsplus_vehicle_resolver.py`.
