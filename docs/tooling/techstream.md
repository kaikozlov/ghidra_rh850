...
Therefore the next directed target is decoded `FRC_P5` **plus category-435
`ABS_P5`/Brake firmware**, or a synchronized stock-LTA capture spanning those
modules, to recover the actual payload transformation and SecOC sender/key/
freshness ownership. The FRC and Brake/EPB Techstream Active-Test catalogs are
now exhausted as setpoint-writer candidates; use them only as fixed/actuator
