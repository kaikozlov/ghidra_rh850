...
RoB values then use the existing P5 physical/unit/pattern chain (type 13/15/14) and `CCmdCommSignalLSBP5::ChangeSignalLSB`. The current protected `CommandCommon.dll` does not expose that body, so the dynamic-LSB boundary is retained explicitly. The recovered V18 implementation establishes that a missing `CDbCommLsbDataIdData` (type-64) row leaves the base physical coefficients unchanged. On the selected Camry-HV install set this distinction collapses cleanly: all **26** RoB-capable logical categories have no type-64 table, and across their **4,445** current type-88 signal rows the cross-DID support-condition key is zero. Thus the generated Camry RoB signal metadata has final GTS-equivalent base scaling plus only the exact local type-90 mode gate. Concrete witness: `EMPS_P5` **Steering Angle** is DID `0x5037`, bits `0..15`, signed, `Mul=15`, `Div=1`, `Offset=0`, one decimal, unit `deg`; raw `0x0001` renders `1.5 deg` and raw `0xFFFF` renders `-1.5 deg`. The generated diagnostic catalogs now export `rob.behavior_codes` and DID-scoped `rob.signals` from this current schema so runtime consumers need no DDB offsets.
