...
The generic **current P5 RoB behavior-code inventory** is closed at its first-stage wire layer. Role `0xA0` binds `GetRoBP5_DT.dll` directly on ordinary P5 categories (unlike the category-0 fallback used by role `0xB5`). Its `CCmdCommRoBData` setup resolves selector `0xF3`; for the proven current Toyota P5 categories that selector contains exactly **`AB 01 -> EB 01`** and **`AB 11 -> EB 11`**. `GetRoBP5_DT.dll` `FUN_100010C0` sends each selected frame, then computes `(received_length - 2) / 2` and consumes response bytes from offset2 as big-endian 16-bit behavior codes. This is only the RoB **code inventory**. The same plugin subsequently performs per-behavior/subfunction record acquisition through a separate second-stage path; that record-body protocol is intentionally not collapsed into the inventory export. The generated diagnostic bundle therefore emits `p5_rob_code_inventory` only when the target category has an exact `GetRoBP5_DT.dll` role-`0xA0` binding and the complete `AB01/AB11` selector-`0xF3` pair. Categories that fall back to P4 or do not have that exact pair remain unexported.
