...
#### 6.2.5 Vehicle/profile, mounted-ECU, and P5 capability resolution

Current GTS+ contains a first-class resolver layer above individual diagnostic commands. `CDAExecSelectVehicle` / `CSelectCarTypeVin10` does not merely present a static vehicle picker: it dispatches vehicle resolution according to the live communication generation, then returns Toyota vehicle, phase, option, and decision information. The distinction matters for the runtime implemented here. Phase3/Phase4 use master `CDbVehicleDecisionTable` (type 41 / class `0x129`), but current **Phase5 does not**: `CGetCarInfoPhase5::virtual_4 @ 0x10004D80` is exactly `B8 01 01 04 C0 C2 14 00`, returning `0xC0040101` without querying class `0x129`. Current P5 generation-low5 values are 20/21; generation-low5 22 dispatches Phase6 instead. The current-P5 selector therefore relies on `CDbVinVehicleDecisionTable` (type 59 / class `0x13B`) at this stage. Its matcher is recovered exactly: the key is ECU category ID (`u16`), phase/protocol byte, and the first eleven VIN bytes, with per-position wildcard bits in the record flags. In the current NA master the two Camry-HV VIN-decision rows resolve vehicle type `12704`, whose type-43 vehicle name is `Camry HV`; type-5 then maps that vehicle to candidate install-set IDs and type-44 maps each install set to expected ECU categories.

Toyota then resolves the actual mounted set rather than treating every statically possible ECU as present. `CDAExecGetMountEcuList` / `GetMountEcuListNoCnfm` starts from the selected installation data and runs the common `CEcuConnectCheck` machinery across the candidates, producing a live `CCmdEcuInfo` list. The call contract is now exact: each 24-byte type-44 install row supplies category ID at `+0x06`, connection-frame ID at `+0x08`, CommSet ID at `+0x0A`, and a communication/phase-type byte at `+0x13`; these are passed to `CEcuConnectCheck::CommConnectionNoBuffer`. That routine resolves DB class `0x111` `CDbCommFrameTable`, applies the selected CommSet, transmits the database send frame, masks the returned bytes with the database receive mask, compares them against the database receive-check bytes, and marks the candidate mounted only on a successful match. The lower route is also now closed: current `KGP_CommFrameCtrl.dll` `CCommFrameCtrl::GetEcuAddr(category, phase) @ 0x10035420` queries DB class **`0x10D` = `CDbProtInfoTable`**; `FindDbItem1 @ 0x100C2590` matches category at record `+0x00`, `FindDbItem2 @ 0x100C2690` matches phase byte at `+0x18`, and `GetEcuAddr` returns the request address from u16 `+0x08`. The current P5 frame builder also consumes byte `+0x0A` from that selected protocol row as the logical address extension. Record `+0x14` is a separate **legislated physical request address**: current masters contain `0x7E0..0x7E7` there, not the `0x7E8..0x7EF` responses. For the standard 11-bit OBD route the corresponding response is request + 8. Consequently all **34/34** current NA Camry-HV install candidates resolve independently through Toyota's own route table: ordinary examples include Engine `0x700`, Steering Angle Sensor `0x7B3`, FRC `0x792`, plus routes not present in the older local 17-address sweep such as Combination Meter `0x7C0` and SRS `0x780`; phase-`0x22` categories share request ID `0x750` while remaining distinct through Toyota address extensions (for example TPM `0x2A`, Brake Booster `0x29`, CMCCM `0x7B`, DMC `0x96`). Every Camry install row uses connection frame `0` and CommSet `9`; frame 0 has empty send/mask/check variables. Thus this current Camry path is category-scoped Toyota transport resolution/connectivity, **not an F181/F190 identity-DID probe and not a locally maintained address allowlist**.

For current P5 controllers, support is resolved again at the individual capability level. `GetSupportP5_DT.dll` accepts `(ecuId, optionId, id)` and dispatches option `1` to `CheckSupportPid`, `2` to `CheckSupportDid`, and `3` to `CheckSupportRid`, returning one support boolean. DDB presence alone is therefore not Toyota's live-support criterion. The DID implementation is recoverable byte-for-byte: selector `0xC8` starts with `22 01 01`; the `62 01 01` payload is decoded MSB-first into supported `xx00` groups, each selected `xx00` group is queried in turn, and its second-level MSB-first bitmap expands to `xx01..xxFF` (skipping the terminal alias that would become the following group's `xx00`). Routine support uses the corresponding current-P5 `0xCC` path rooted at `31 01 10 01 -> 71...`.

The current master also gives an exact diagnostic-family discriminator that is stronger than interpreting generation bits: **P5 categories bind role `GetSupportP5_DT.dll`**, while current P6 categories such as `6000 Engine_CM_P6` bind `GetSupportMultiP6_DT.dll` and do not bind the P5 helper. Each NA/EU/JP master contains 179 category identities with a `GetSupportP5_DT.dll` binding; after requiring the regional ECU DDB to exist, the current release yields **135 NA / 161 EU / 143 JP** clean P5 catalogs. The class-`0x110` generation-low5 values remain part of Toyota's session dispatch, but they are not used as a substitute family classifier.

Session continuity is category-local too. Across every shipped P5 catalog, D1/D2 resolve to exact `10 01` / `10 03`, but selector `0xDD` has multiple current families rather than one Camry-global keepalive: NA has 124 `22 F1 86` session polls and 11 `22 D1 00` DID polls; EU has 149 `22 F1 86`, 11 `22 D1 00`, and one `10 03` extended-session refresh; JP has 129 `22 F1 86` and 14 `10 03` refreshes. Category 851 `PCM_P5MMC` is a concrete `22 D1 00` witness; category 471 is the EU/JP `10 03` refresh witness. Independent tooling should therefore resolve D1/D2/DD from the selected Toyota category rather than project the F33 `22 F1 86` pattern across the whole P5 family.

For the current-P5 family implemented here, the resulting Toyota resolution pipeline is therefore **generation/communication selection -> VIN vehicle decision -> selected install set -> category+phase transport route -> live mounted ECU list -> per-ECU PID/DID/RID support -> diagnostic operation**. Type-41 vehicle decision remains part of Toyota's P3/P4 path and must not be inserted into Phase5 simply because the table exists in the master. Independent tooling should reproduce that division rather than substitute a hand-curated vehicle profile or require bespoke per-DID "wire proof" before using already-recovered common P5 behavior. This resolver establishes identity, installation, routing, and advertised diagnostic capability; it does not by itself establish that an arbitrary mutating operation is authorized or safe to execute.

Machine-readable resolver evidence is `data/generated/gtsplus_2026/vehicle_resolver_semantics.json`, generated by `tools/techstream/extract_gtsplus_vehicle_resolver.py` and verified by `tests/verify_gtsplus_vehicle_resolver.py`. The clean three-region runtime export is `data/generated/gtsplus_2026/toyota_diag_bundle_current.zip`; `tests/verify_toyota_diag_bundle.py` regenerates it byte-for-byte and verifies the P5 plugin boundary, regional catalog counts, category-local session families, and both Camry-HV and non-Camry vehicle resolution.
