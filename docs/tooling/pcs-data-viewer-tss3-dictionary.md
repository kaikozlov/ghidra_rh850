...
> `tools/techstream` and `tests` directories.
...
initializer and event logs remain the next evidence needed to recover the
state transition and to distinguish an automatic-start observation from an
authorization rule.
