...
> bit/scaling/RoB tables, the full ADU recorder table, and the current FCM TSS3
> Image-FFD payload transform.
>
> **Artifact:**
...
> by `tests/verify_gtsplus_pcs_data_viewer_tss3_dictionary.py`; recovered ADU
> rows are in `pcs_data_viewer_adu_semantics.json`, generated by
> `extract_pcs_data_viewer_adu_semantics.py` and independently verified by
> `verify_gtsplus_pcs_data_viewer_adu_semantics.py` under the corresponding
> `tools/techstream` and `tests` directories. Legacy and Phase-5 DDR rows are
> similarly preserved in `pcs_data_viewer_ddr_semantics.json`.
...
The same recovered executable now exposes the complete
`ADUDetailInfo::.cctor`: **7,851 `P6DetailBitAssignInfo` rows across 1,369
recorder DIDs**. The generated ADU artifact preserves every row and selects the
control surfaces relevant to `0x08A` analysis. The aggregate request/result
schema is exact:

| ADU DID | Recovered layout |
|---|---|
| `2A02` | lower ID byte 1; signed acceleration bytes 2--3 at 0.001; distribution byte 4; shift, EPB, accelerator-override prohibition, and low-priority bytes 5--8 |
| `2A03` | upper ID byte 1; signed acceleration bytes 2--3 at 0.001; distribution byte 4 |
| `1592` | overall, brake, and powertrain arbitration-result longitudinal IDs in bytes 1, 2, and 3; `0xFF` invalid |
| `1617` | arbitration-result acceleration-valid field |
| `161D` | signed arbitration-result acceleration at 0.001; `0x7FFF` invalid |

This table also resolves why an upper/lower pair need not identify one feature.
`1F03..1F07` explicitly describe **PDA(OAA) upper-limit** requests, beginning
with `OAA request vertical ID (upper limit)`. The TSS3 resource block
`5B07..5B11` independently describes **PDA(DA) lower-limit** ID, acceleration,
speed, distribution, stop/hold permission, low-priority, and EPB requests. A
runtime pair such as upper ID 17 plus lower ID 23 is therefore structurally
consistent with cooperating PDA subfunctions; it is not evidence that ID 17
globally means PDA.

That distinction matters because the recovered viewer contains **24 named
longitudinal/vertical-ID fields but no integer-to-client legend** for values 13,
17, or 23. It stores raw requester/result integers. Meaning still depends on
...
(`15D0`, `15D1`, `2C82`, `2C83`), and arbitration/actuation (`1592`, `1617`,
`161D`, `1629`). `15EE` is a signed 0.25-m/s² deceleration output and `1629_9`
is the automatic-brake pressurization status bit. A PCS alert proves a warning
stage occurred; it does not prove that VMC received or executed the FRC brake
request. In particular, a relay that blocks the FRC's native `0x08A` also blocks
the native request carried there unless an upstream-shaped replacement
preserves that request.
...
`DetailBitAssignInfo` rows**. The viewer's exact decoder reads `ByteCount`
bytes, computes `(raw & BitCount) >> RightShiftCount`, then applies
`raw * Lsb + Offset`; `BitCount` is therefore a mask despite its name. Phase-5
`5493` uses a two-byte read with mask `0x20` and shift 5, while `5494` uses a
two-byte read with mask `0x01` and shift 0. They are distinct one-bit
observables with the same English label. Their runtime memory addresses come
from the DDR acquisition definition rather than this static field table, so
the recovered masks do not prove that the two bits share a source word. The
viewer still supplies no resume authorization state machine and does not say
which bit is the automatic-start edge.
