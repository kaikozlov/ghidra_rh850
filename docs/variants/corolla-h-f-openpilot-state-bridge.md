...
returning B6 cannot revive the stale command. Openpilot's native 100-Hz changed C7
...
Ghidra emulator against this 100-Hz/200-Hz schedule, expiry, zero-release, wrap,
...
## 9.1 Cross-generation road evidence: `0x08A` / `0x081` predates the Camry (VAR-133)

A direct re-read of the two retained Corolla road captures corrects an omission in the
earlier selected-ID reducers: both vehicles already carry the same 32-byte
`0x08A` / `0x081` control request/result family later recovered in detail on the
2026 Camry. The old extractors simply did not include those two IDs in their role
inventories; the raw captures did.

The pinned 2023 albinoelephant public-route segment
`a74eba85c97eaf67|00000004--555953f500/0`
(`d246a559...477b`) contains, as incoming `src<128` logical-bus-1 traffic:

- **2,355** `0x08A/32` frames; B21[5:0] is `0` in 2,355/2,355 and B24 is `0` in
  2,355/2,355;
- **1,961** `0x081/32` frames; B13[5:0] is `0` in 1,961/1,961;
- **zero** incoming `0x0B6`, `0x131`, and `0x2E4`. The apparent 5-byte `0x2E4`
  population in the legacy route summary is openpilot output: 5,037 `sendcan`, 5,025
  Panda returned echoes, and 11 rejected echoes, not stock vehicle traffic.

The independently supplied Span 2025 moving rlog
`span_67fd5b833889fedf_00000010--17084916da--3--rlog.zst`
(`f1ae7c40...d439f`) reproduces the same family during a ~60-s moving capture:

- **2,400** incoming logical-bus-1 `0x08A/32`; B21=0 and B24=0 throughout;
- **2,000** incoming logical-bus-1 `0x081/32`; B13=0 throughout;
- **zero** incoming `0x0B6`, `0x131`, and `0x2E4`.

The shared field geometry is not an ID-only coincidence. Joining `0x08A B18:B19`
(signed big-endian) to the nearest retained `0x025` measured-steering publication gives:

| capture | pairs | Pearson r | fitted deg/count | error vs `1024/17870` |
|---|---:|---:|---:|---:|
| albinoelephant 2023 | 2,355 | 0.999611 | 0.0574099 | +0.187% |
| Span 2025 | 2,400 | 0.999504 | 0.0574832 | +0.315% |

The exact H/F/Camry controller-equivalent comparison value is
`1024/17870 = 0.0573027420 deg/count`. The small fit error is consistent with
publication timing and the coarse/fractional `0x025` measurement geometry; it is not
used to transfer an OEM wire-unit name. `0x081` independently tracks the same reference
word: using the latest `0x08A` no older than 100 ms, Target Lateral ID matches in
**1,961/1,961** public-route pairs and **2,000/2,000** Span pairs; the reference words
are byte-identical in 1,271 and 1,513 pairs respectively, with median absolute raw
...
handoff. They nevertheless materially constrain the architecture: factory road traffic
shows the same CAN reference family while the captured segments contain neither native
B6 nor the classic `0x2E4/0x131` steering commands. This supports treating the unresolved
Camry step as a general TSS3 reference-to-actuator handoff problem. It does not prove a
serial/local handoff, but it gives no positive evidence for a Honda-style dedicated
serial steering command either.

Deterministic evidence is generated by
`tools/targets/corolla/extract/extract_corolla_2023_public_route_opendbc_evidence.py` and
`tools/targets/corolla/extract/extract_span_2025_discord_rlog_opendbc_evidence.py`, with the common reduction in
`tools/toyota_support/toyota_route_opendbc_common.py`.

### 9.2 September-16 integration consequence: longitudinal stays stock-owned (VAR-163)

The later Camry request-plane recovery changes the integration meaning of the same
Corolla traffic. `0x08A` is not a lateral-only carrier: its request-side layout includes
two packed longitudinal request-ID/allocation bytes plus two signed16 ×0.001 m/s²
acceleration request values. Re-decoding Span's retained active-ACC `0x08A` witness with
that layout gives request A ID **17**, allocation **3**, request B ID **23**, allocation
**1** on all 37 ACC-engaged frames. The two signed acceleration slots are equal
frame-for-frame and sweep 15 values from **-0.387 to +0.082 m/s²**. The same PDU
simultaneously retains the lateral request fields documented above. This is the shared TSS3
application-request plane, with `0x081` carrying the corresponding chassis-side
result/reference family.

That supersedes the old integration inference drawn from the successful contributor
`0x160` modify-and-forward experiment. The experiment remains useful historical evidence
that `0x160` is FRC-origin Profile-5 traffic and that the reported drive preserved
Toyota longitudinal behavior while frames were modified. It no longer establishes
`0x160` as the authoritative TSS3 longitudinal command ingress: the current cross-system
request/result architecture places the application acceleration requests in `0x08A`,
while the strongest retained Camry evidence classifies `0x160` as motion/state feedback.
There is no Corolla-specific evidence strong enough to override that shared architecture.

The openpilot consequence is therefore fail-closed and shared with Camry: retain
`0x08A` in the TSS3 DBC as decoded request-plane evidence, retain `0x160` only as a
neutral read-only FRC state/evidence PDU, and keep Toyota `STOCK_LONGITUDINAL`. The host
must neither synthesize nor suppress `0x160` for longitudinal control. Native TSS3
longitudinal can be reconsidered only after a clean `0x08A` source-ownership path is
...
successfully and originally exposed three software gaps. Same-day follow-up now maps the proven
Corolla fault/inhibit aggregate to ordinary temporary steering unavailability and implements
software-requested stock ACC cancellation in native openpilot shape using cloned bus-1 `0x101`
with only `BRAKE_PRESSED` asserted. Receiver acceptance of that cancel is still vehicle-unqualified.
The hybrid `0x3BF` fallback is not implemented, and GTS subtype is still not propagated directly
into CarParams. Earlier broad completion claims and references to a general cross-powertrain
fallback should be read with these explicit limits.
