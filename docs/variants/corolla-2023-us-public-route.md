...
`tools/toyota_support/analyze_toyota_dataflash.py` was run against the contributor's public
...
`tools/variants/compare_variant_function_bodies.py` and regenerated byte-for-byte by
...
raw CodeFlash by `tools/variants/compare_variant_application_rx.py`. It locates the normal
...
body SHA-256. `tests/verify_corolla_h.py`
...
`tests/verify_corolla_h.py` regenerates and pins the report.
...
`tests/verify_corolla_h.py` regenerates and pins
the result.
...
generated by `tools/targets/corolla/extract/extract_corolla_h_supervisor_external_ingress_census.py` from
...
`tests/verify_corolla_h.py` explicitly
...
`tests/verify_corolla_h.py` pins the promotion rules.
...
`tests/verify_corolla_h.py` pins the eight role
...
decompiler evidence. `tests/verify_corolla_h.py` pins all nine
...
evidence own this conclusion; `tests/verify_corolla_h.py`
...
`tests/verify_corolla_h.py` pins the four roles, selector table,
...
decompiler evidence. `tests/verify_corolla_h.py` pins
...
42-role target-native evidence file. `tests/verify_corolla_h.py`
...
seven-function evidence file. `tests/verify_corolla_h.py`
...
evidence. `tests/verify_corolla_h.py` pins the six
...
plus compact H-native evidence. `tests/verify_corolla_h.py`
...
evidence. `tests/verify_corolla_h.py` pins
...
both CodeFlash images by `tests/verify_corolla_h.py`.
...
`tests/verify_corolla_h.py`, and
`tests/verify_corolla_h.py` pin the exact raw
...
`data/generated/corolla_8965H1202000_b6_secoc_verification_decompiler_evidence.json`;
`tests/verify_corolla_h.py` independently regenerates
...
`tests/verify_corolla_hf.py` pins the static H/F mapping and
