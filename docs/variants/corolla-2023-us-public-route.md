...
### 6.1 Default/global-key falsification and whole retained-memory scan

The service observation that a replacement camera did not require a visible key
configuration step is **not** evidence that this Corolla lacks SecOC. The retained
wire data already contains Toyota's classic authenticated family: `0x00F` carries
trip/reset freshness plus a 28-bit authenticator, while genuine bus-1 `0x116` and
`0x24D` use the corresponding protected-message trailer. Because the packing and
AES-CMAC verification algorithm are independently implemented in pinned opendbc and
in `tools/toyota_secoc_oracle.py`, the simplest "maybe everybody just uses a dumb
default key" hypotheses can be falsified directly against the captured tags.

`data/generated/corolla_2023_albino_secoc_default_key_probe.json` records three
offline tests. First, the **publicly printed** `KEY_4` example from pinned
I-CAN-hack/secoc (the older RAV4-Prime/Sienna extraction family) authenticates
**0/588** public-route synchronization frames, **0/2,496** usable `0x116` frames,
**0/59** `0x24D` frames, and **0/1,232** synchronization frames from Albino's later
TSKM collection. This disproves reuse of that particular public legacy example key
on the retained Corolla evidence; it does not establish whether other Corollas share
a different platform key.

Second, all **65,536** 16-byte keys formed by repeating one two-byte word eight
times were tested against the first synchronization frame of both captures. There
were zero matches. This family includes every repeated-single-byte key, including
`00*16` and `FF*16`, so the obvious blank/default-key theories are closed without
assuming entropy.

Third, every unique overlapping 16-byte value in the retained CPU-visible H corpus
was used as a candidate key: normalized CodeFlash, five physical-DataFlash views
(restricted to the real first 32 KiB of each 64-KiB host read), three extended
CodeFlash reads, three GlobalRAM reads, and three PE1 LocalRAM reads. There are
**711,279 unique candidate values** across those retained bytes; none authenticates
the first `0x00F` from either the public-route or local TSKM oracle. This extends the
earlier one-DataFlash negative across every retained CPU-visible memory class.

The acquisition boundary matters. The TSKM CAN collection and memory dumps are
separate jobs/runtime epochs, and the older public route has no `carFw`/F181 join to
the exact H image. Therefore this is strong evidence against a **stable raw 16-byte
key represented verbatim in the retained CPU-visible corpus**, not proof that the
secret cannot change between epochs. It also does not exclude a Corolla/platform
pre-provisioned key, a vehicle-specific pre-provisioned key, automatic provisioning,
or a derived/hardware-resident key. Exact H static analysis independently points in
that last direction: all three configured SecOC profiles select ICU-S type-1
**slot 4**, while no raw 16-byte key load is recovered in the CPU-side verification
path.

Consequently, the most defensible reading of "camera replacement needs no visible
re-key" is a **key-lifecycle observation**, not a no-SecOC observation. A fixed or
pre-provisioned Corolla key remains possible, but it is not the public old-family
key, not an all-zero/all-FF style key, and is not present verbatim in the retained H
CPU-visible memory under the two captured oracle epochs.
...
`0x23` requests another reset candidate through `0x8891E(...,0x201)`. A later
competing-sender audit recovered an important distinction that this original summary
missed: `0x22` still calls generic failure handler `0x888A6`, and B6 can be forwarded
to COM despite that verification failure during the bounded generated failure-forwarding
modes described below. "No command7" therefore does **not** universally mean "no
...
- on CMAC mismatch, `0x88BE2` sets high16 in the callback argument, so `0x8A07A`
  does **not** commit the pending freshness and `0x8891E(...,0x200)` uses B6's one
  same-PDU retry. A first mismatch therefore does not take the verified-delivery arm.
  If the retry budget is exhausted, however, `0x8891E` enters generic failure state
  `0x96` and calls `0x888A6`, so the generated failure-forwarding policy can still
  release the queued PDU under its bounded modes.

That failure-forwarding policy is exact for B6. `0x8857C` zeros global counter
`FEBE5408`; normal receive-main path `0x88308→0x88288→0x886DA` increments it by one
until the configured raw threshold at `0x25726`, **204**. `0x886FC` can reset the
counter. B6 profile byte `+0x09` is 0 (ordinary profile). `0x888A6` calls PDU upper
route `0x88856` when either global state `FEBE53EE` is D2 (`0x88512`) **or** profile
`+0x09 != 1` and `FEBE5408 < 204`. Thus a hard freshness failure or retry-exhausted
CMAC mismatch can reach PDU42/COM during that bounded grace/global-override mode even
though authentication failed. Freshness is still **not committed**. Once the counter
is >=204 and the global D2 override state is inactive, the recovered failure handler
does not route failed B6 through `0x88856`. The wall-clock duration of the 204-count
window and the OEM name of the D2 override mode remain unrecovered.

Freshness-valid but MAC-invalid B6 therefore cannot advance committed freshness.
Outside the bounded failure-forwarding modes it cannot reach the steering application;
inside those modes the generated stack can deliver the failed queued payload without
turning it into an authenticated success. This corrects the earlier unconditional
"mismatch cannot deliver" statement (CORR-111).
...
the CMAC path, result 0 as match, and freshness **commit** only on successful
authentication before the normal verified-delivery arm. H/F additionally has the
raw-bound generated failure-forwarding policy above, so upper delivery itself must not
be equated unconditionally with authentication success. The generic upper bodies are not claimed byte-identical;
