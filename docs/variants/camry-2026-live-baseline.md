...
A 2026-08-30 first live validate-only attempt exposed and corrected one host-side
bootstrap regression before any flash write. Exact application F181 matched and boot
SecurityAccess plus RID `0x10F0` accepted, but the generic RAM-exec helper had sent
DID `0x0203 = 01 00 00 00 00` merely because `cpu_index=0`. Retained exact-F33
acquisition transcripts prove this ECU is the **old** boot stack and accepts
`0x0203 = 00 00 00 00 00`; `01 00 00 00 00` is the newer-stack CPU0 offset
selector. The uploaded validate payload therefore produced zero telemetry and the
deployer correctly left `apply_ready=false`; no persistent write was attempted. The
shared bootstrap now derives DID `0x0203` from **UDS stack variant + CPU index**, while
RequestDownload memory ID remains independently CPU-indexed (`1` for CPU0, `0` for
CPU1). This split is regression-tested across old/new × CPU0/CPU1 and mirrored into
the openpilot-side planner. The failed preflight record is retained under
`targets/camry-2026/raw-20260830/secoc-patch-preflight-oldstack-0203-regression/`.
