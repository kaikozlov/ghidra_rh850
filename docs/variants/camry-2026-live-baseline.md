...
### 2026-08-30 — F33 dev-lateral relay topology correction
...
Panda forwarding counters showed every received frame was being forwarded in both directions, so the failure was not a simple dropped-frame whitelist. The working topology is instead to leave the Toyota-B physical intercept relay closed for exact-F33 dev lateral, disable Panda software forwarding, and inject protected B6 directly onto bus0. This preserves the native Toyota bus timing/order/framing while still permitting B6 TX. The corresponding development Panda is commit 927d3e78 and opendbc safety commit 886b32a9 (parent kai-openpilot e275e6fd8).

Validation: clear FRC DTCs, boot normal READY, wait for live `TOYOTA_CAMRY_TSS3` / Toyota safety param 4096, run 15 s with the relay-closed dev mode, then rescan FRC. Result: zero fault-status records; none of the seven communication-loss DTCs returned. Only non-fault `TEST_NOT_COMPLETED_SINCE_LAST_CLEAR` records remained. Therefore the prior network malfunction was caused by intercept/store-and-forward topology, not by the persistent EPS Gate-2 patch or by B6 itself.
