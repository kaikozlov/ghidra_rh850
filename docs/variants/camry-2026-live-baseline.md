...
Operation” / 1=“Cruise Control in Operation”**. The prepared capture therefore requires
stable overlap of `0x1601` LTA-enabled state **and `0x1914=1`**, plus independently
observed motion and healthy Bus-4 protected traffic, before treating a continued zero-B6
result as the strong operational discriminator. If that synchronized operating context is
captured while B6 remains absent, do **not** resume an arbitrary EPS-ID hunt: move the RE
boundary outward to the FRC/Brake transformation or inward to a non-COM/internal EPS path. Deterministic evidence is
