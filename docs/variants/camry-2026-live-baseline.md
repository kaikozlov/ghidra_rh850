...
`0x08A`, `0x371`, and `0x412`**. These messages are therefore state/display-plane
evidence, not EPS ingress or proof of a steering-command wire. The current `0x08A`
producer mapping is unavailable, so the conclusion is deliberately **a strong
cross-domain numeric+dynamic join: bus-0 `0x08A B21=11` is strongly identified as
LTA/LCA active, not target-native byte-exact OEM producer-wire proof**.
...
This supersedes VAR-067's “generic lateral/HUD candidate” wording (CORR-129) while
preserving its wire/ownership boundary. It does not identify the current producer, prove
any of the three layouts byte-exactly, show that F33 receives them, establish B6 sender
behavior, or authorize steering output. Deterministic evidence is
