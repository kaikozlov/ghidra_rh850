...
FRC DID `0x1601` remains the exact OEM-name oracle. Current GTS+ resolves its value
...
poller remains useful to attach Toyota's exact `0x1601` LTA name to the repeated lateral/HUD
candidate state and to corroborate `0x1914`; it is mechanically read-only (`22 16 01` /
`22 19 14` only) and stops if either exact positive response stream is absent/stale for
two seconds. Do **not** wait for that naming capture before investigating the actual
steering path: §20 already establishes zero B6 during machine-recovered cruise operation
and long repeated lateral/HUD candidate intervals. Move the RE boundary outward to the
...
## 20. Existing drives recover cruise operation and a distinct lateral/HUD state without B6
...
A second state class is structurally distinct from the cruise-main latch and from the
momentary button echoes. During first-drive segment5, `0x08A` byte21 becomes **`0x0B`**
with byte24 `0x64` for **16.119256 s**. In the confirmation drive the same state begins in
segment20 and persists across the segment boundary until the segment21 CANCEL for
**57.184128 s**. In those intervals:

- `0x081/32` byte13 mirrors `0x0B` for >99.8% / >99.9% of samples respectively;
- `0x412/8` is dominated by the same payload **`14 00 00 44 01 EE 93 07`** at 94.4% /
  98.3%; and
- the combined intervals retain **237,097 incoming frames / 73.303384 s / zero B6 on
  every bus**.

This state cannot be reduced to a button echo or to the cruise-main latch. Its transitions
can occur seconds after engagement and also at unanchored times; the confirmation-drive
long interval begins about seven seconds after the final RES+ state transition and ends
with CANCEL. Historical Toyota DBCs name classic `0x412/8` `LKAS_HUD`, and current GTS+
contains an FRC_P5 Active-Test entry named `LTA Indicator 1`, which make the display/lane
interpretation worth pursuing. **Neither is promoted here as the exact current-Camry wire
meaning.** Exact F33 does not accept `0x08A` as an EPS Rx PDU, and the current FRC
`0x1601` state was not sampled in these old routes. The defensible label is therefore a
**lateral/HUD candidate state**, not an OEM-proved LTA-active bit and not a steering
command.

The important architectural consequence is nevertheless stronger than §16's original
button-only evidence. Zero B6 is now observed during machine-recovered cruise operation
and during two long, independently repeated lateral/HUD candidate intervals, including
the first drive in which the operator reported green LTA/steering assistance. It is no
longer productive to explain the entire zero-B6 result by saying that cruise may not have
been operating. The exact `0x1601` poller remains valuable to attach Toyota's name to this
state class, but the reverse-engineering work should proceed in parallel: correlate the
Bus-1 `0x180..0x18C` upstream family and EPS outputs/internal state specifically inside
these intervals, and audit the non-COM/internal EPS boundary if no external B6 appears.
Do not infer a B6 transmitter/template from absence and do not authorize steering output
from this state reconstruction.

Deterministic evidence is
`data/generated/camry_2026_cruise_lta_edge_census.json`, regenerated by
`tools/analyze_camry_2026_cruise_lta_edges.py` from the two already-hash-pinned §16 route
artifacts and verified by `tests/verify_camry_2026.py`.
...
`tools/techstream/build_camry_f152633k0000_brake_acquisition.py` and verified by
...
`0x59448`'s `FEBE7Fxx` references are WRITEs only. RH850/P1M-E has **exactly one
...
Therefore the directly recovered ordinary-COM parameter-bank inputs cannot explain the retained Class-L-specific `0x030 B22:B23` motor-feedback shift. Combined with VAR-074/077/078, the remaining discriminator has moved inward again: **internal/fault/diagnostic selector state or another internal baseline-assist term**, plus the still-needed target-native FRC `0x1601` LTA name join. The 0x51E observations are only about one sample per second, so they do not establish high-rate timing; their stronger fact is that the relevant fields are zero for the entire retained routes. Nothing here proves Class-L is OEM LTA or authorizes output.
...
For a future passive validation capture, the most discriminating EPS reads are therefore: **`0x1C38` first** as a direct proxy for still-unresolved `FEBECB38`; **`0x1C02` second** as Toyota-named final command torque; and **`0x1C3E` as a control** expected to stay quiet under the retained-drive moving-mode conditions. These should be synchronized with FRC `0x1601`/`0x1914`; none requires or authorizes steering output.
