...
`1=LTA Disabled`. This proves the FRC diagnostic control state, not continuous steering
torque by itself. If a synchronized stable enabled interval is captured while B6 remains
absent, do **not** resume an arbitrary EPS-ID hunt: move the RE boundary outward to the
FRC/Brake transformation or inward to a non-COM/internal EPS path. Deterministic evidence is
