...
Deterministic evidence: `tools/analyze_camry_8965F3307000_dataflash_nvm_owners.py`
...
`tools/build_camry_f33_gate2_semantic_patch.py` reproduces the stage-1 source
...
This new resident is statically/audited-built but **not yet live-qualified**. The next
vehicle action after a full OFF is therefore `./f33-pre install` in NRTD, direct NRTD->READY,
then one `./f33-pre phase P`. An exact queue match moves the unexplained transformation
inside F33 downstream of enqueue. No queue hit keeps physical/RSCFD/CanIf acceptance open.
A queue hit with a different signature proves profile2 activity but not that this phase's
frame was queued. Nothing in this result justifies another MAC/counter/result patch or a
move to downstream D-G gates before phase P settles that identity boundary.

Machine-readable reduction is
`data/generated/camry_f33_runtime_monitor_20260908.json`; raw field files are retained under
`targets/camry-2026/raw-20260908/runtime-monitor-session/`; deterministic verification is
`tests/verify_camry_f33_runtime_monitor_20260908.py` plus the pre-aggregate build checks in
