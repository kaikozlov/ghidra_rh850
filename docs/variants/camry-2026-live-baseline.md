...
2. recover the internal FRC packer/SecOC/freshness path that emits the feature-selected, pre-VMM-arbitration
   request as protected `0x08A`; ECU-level publisher ownership is already closed to FRC;
3. recover the downstream Brake verification/request-arbitration/result/request-generation and the
...
| `FEBEC156&3` map selector | `C54A2/C5554` ← `FEBEAC2F ← FEBEB121`; ordinary path begins at `0x51E B0[3:0]` / `FEBEB124`. Current installed Camry `HV_P5` DID `0x1004` names the matching value domain **Drive Mode Select Status** (`0=Normal`, `2=Sport`, `4=Sport S+`, `6=Eco`). | driver-selected drive-mode calibration; CAN-field name is recovered from the enum/code join pending a synchronized DID/CAN toggle | recovered |

**For this C5EE moving-mode subfamily, drive-mode selection is a calibration no-op in the exact image.** All four entries of every
`FEBEC156&3` pointer table used here alias one table: `0xD39DC→0xB018A ×4`, `0xD3A1C→0xB01D2 ×4`,
`0xD3A5C→0xB01E6 ×4`, `0xD3A9C→0xB01FA ×4`, `0xD3ADC→0xB019A ×4`, `0xD3B1C→0xB01B2 ×4`.
That does **not** make the selector globally inert: §31 closes a separate `C28FC/C2B64` bank in which Sport is materially distinct from Normal/Eco.
...
VAR-079 originally closed this as an unnamed parameter-bank selector immediately upstream of several `D0218` baseline-assist terms. The exact codeflash plus current installed Camry diagnostic dictionary now recover its useful vehicle meaning: this is the **driver-selected drive-mode steering calibration**. The OEM wire-field name is still one dynamic join short of verified because the retained drives never toggle it, but the static enum/code alignment is exact enough to identify the mode behavior.

**OEM enum join.** Current Camry `HV_P5` (installed category 397) exposes DID `0x1004` **Drive Mode Select Status** with `0=Normal`, `1=Power`, `2=Sport`, `3=Sport S`, `4=Sport S+`, `5=Comfort`, `6=Eco`, `9=Range`, and `11=Chauffeur`. Exact F33 receives `0x51E/8` sig160 `B0[3:0] -> FEBE8030 -> FEBEF050`; `B3430/B3686` debounce it into `FEBEB124`, and `B35DC/B372A` branch on the same enum-shaped values. Value `2` or `4` produces `FEBEB121=0x11`; `C54A2` maps that to `FEBEC158=0x77`, and `C5554` selects **`FEBEC156=1`**. Value `6` falls through to selector 0. Value 0 can resolve to selector 0 or 2 depending companion state. For the **main `C28FC/C2B64` driver-torque assist surface**, selector 0 and selector 2 are byte-identical; Sport selects selector 1, the only distinct bank for that surface. The full C156-reader census below preserves one nuance: selector 2 differs from selector 0 in three much smaller shaping regions, so `Normal==Eco` must not be generalized to every possible value-0 companion/customization substate. The retained ordinary Normal captures have the companion value fields zero throughout and therefore exercise the selector-0-compatible path. A synchronized `HV_P5 22 1004` plus `0x51E` mode-toggle capture remains the independent wire-name closure.

**Complete selector chain.** The scalar-value subset recovered into this family is seven signals: `0x51E/8` sig160 `B0[3:0] -> FEBE8030 -> FEBEF050`, sig163 `B1[3:0] -> FEBE8033 -> FEBEF14A`, and sig166 `B5[7:6] -> FEBE8032 -> FEBEF141`; `0x13B/8` sig224 `B2[3:0] -> FEBE8082 -> FEBEF14B`; `0x490/1` sig280 `B0[6:4] -> FEBE80D2 -> FEBEF168` and sig281 `B0[3:0] -> FEBE80D3 -> FEBEF0A1`; and `0x1DA/8` sig282 `B0[3:0] -> FEBE80D6 -> FEBEF156`. `0x58074` stages these cells. Resolved COM-receive validity companions can suppress qualification but do not choose a bank directly. `BCBD8` snapshots the decoded state to `FEBEAC2F`; `C54A2/C5554` produce `FEBEC158/FEBEC156`; and `C28FC` chooses the calibration block consumed by `C2B64`. This is an assist-calibration selector, not a steering-target magnitude.
...
**Exact calibration surfaces.** For the primary driver-torque term, healthy base `0x10100` contains four selector banks of eight `0x44` torque curves each. Selectors 0/2/3 are byte-identical; selector 1 differs by **215 bytes**. `C58B8` interpolates the rows against conditioned SP1 vehicle speed at exact breakpoints **0, 7.68, 19.2, 38.4, 76.8, 128, 192, 256 km/h**. `C2B64` then evaluates the two adjacent driver-torque curves against `FEBEC128` and blends them by the speed fraction. The torque axis is the same Q8.8 domain as Toyota DID `0x1035 Steering Wheel Torque` (`0x100` internal counts = 1.000 N·m). `C29B2` also exports curve slope; `C2C32` blends that into `FEBEBF40`, which feeds the separate `C8678 -> FEBEC4C0` torque+speed term. So Sport changes more than one downstream contribution.

The complete direct calibration census makes that scope precise. Exact F33 has **31 direct readers of `FEBEC156`**. Across **26 direct selector-strided healthy calibration regions**, selector 1/Sport differs from selector 0 in exactly five: the `C28FC/C2B64` driver-torque surface (**215 bytes**), the `C6E7E` 8-map bank (**40 bytes**), `C7AB0`'s second map (**6 bytes**), `C91F2`'s first map (**2 bytes**), and `C9258`'s second map (**4 bytes**). Every other direct region aliases selector 0, and all separately pointer-indexed selector families enumerated in the artifact alias across 0..3. The four smaller differences are live shaping paths rather than dead calibration: `C6E7E` reaches `C39C`, `C7AB0` reaches `C41E`, and the `C91F2/C9258` pair reaches `C5A8`; all three terms are summed by `D0162` into the ordinary `D0218` assist aggregate. Their exact Toyota calibration names remain bounded.

Selector 2 is also now bounded: relative to selector 0 it differs only in the three tiny `C7AB0/C91F2/C9258` shaping regions, **not** in the 215-byte `C28FC/C2B64` driver-torque surface or the 40-byte `C6E7E` Sport-specific bank. This is why the ordinary zero-companion Normal path and Eco can share the same primary steering-weight surface while a less-common value-0 companion/customization substate is not byte-for-byte globally identical.

The direct `C2B64 -> FEBEBF3C` difference is large at normal hand torques. Representative exact-map evaluations of the selector-0 primary surface versus Sport are **40 km/h / 1.0 N·m: 703 vs 280 (-60.2%)**; **60 km/h / 2.0 N·m: 1869 vs 1002 (-46.4%)**; **80 km/h / 1.0 N·m: 366 vs 215 (-41.3%)**; and **120 km/h / 1.0 N·m: 343 vs 109 (-68.2%)**. These percentages are for this one base-assist term, **not total rack/motor assist**: `D0218` sums several torque/speed/return terms before the shared current-control funnel. They nevertheless explain the substantially heavier Sport steering directly from the exact codeflash rather than as a generic drive-mode assumption.

**Retained-drive join.** The two relay-correct CAN-only captures are Normal-mode-compatible throughout: in drive A all **519** observed `0x51E` frames have sig160=0; in drive B all **600** have sig160=0. The other selector inputs remain zero/absent as previously reported. This still rejects the drive-mode selector as the retained Class-L/LTA transition discriminator, but it no longer means the selector is functionally unimportant: the captures simply never exercise the distinct Sport bank. The `0x51E` observations are only about one sample per second, so they do not establish high-rate transition timing.

Deterministic evidence is `data/generated/camry_8965F3307000_internal_assist_oracles.json`, generated by `tools/targets/camry/builders/build_camry_8965F3307000_internal_assist_oracles.py` and verified by `tests/verify_camry_8965F3307000_internal_assist_oracles.py`; the earlier retained-drive reduction remains `data/generated/camry_2026_baseline_selector_live.json`.
...
The original VAR-091/CORR-149 topology-only interpretation in this section is superseded by the later relay-open/FRC-CommunicationControl source result. The FRC is not merely the request-side semantic participant: protected `0x08A` publication is inside the FRC assembly. The current split is therefore **FRC feature-owner selection + secured `0x08A` request egress -> downstream Brake/VMM request arbitration -> `0x081` arbitration-result/status + post-arbitration request generation -> B6/EPS**. VAR-090/092 still close default-bank `D0218` as not an F33 COM copy of the published milliradian. Synchronized FRC Operation FFD remains useful for observing the feature-local-to-generic request transition and the downstream arbitration result. Protected B6 remains an independent candidate openpilot ingress.
...
packer/key/freshness implementation, and separately the downstream Brake/VMM
verification/request-arbitration/result/request-generation path. The B6 DTC attribution
to the Brake System Control Module domain remains the immediate downstream source-domain
anchor.
...
carries the **feature-selected request** and is FRC-owned egress, but remains **upstream of
the VMM request-arbitration result**. Exact F33 is neither its transmitter nor consumer;
native Bus 1 does not carry it. Open questions are the exact FRC
selector/packer/security-core path and the downstream Brake/VMM verification/request-
arbitration/result/request-generation path to B6. Skid/Brake/CGW are no longer candidate
publishers of `0x08A` itself. Do not send `0x08A` to EPS.
...
downstream Brake/VMM verification/request-arbitration/result/request-generation -> B6
transform. Synchronized feature-local FFD + `5282` can expose the FRC owner transition;
`5285/57DE/5265` + `0x081` expose the separate downstream arbitration/result/plant path.
Do not hunt another EPS CAN field or send `0x08A` to EPS.
...
- **Consequence (placement corrected by later source evidence).** No separate
  ...
  attributed immediate B6 source domain and downstream request-arbitration/result/request-generation peer.
...
**generic request -> VMM arbitration -> result/target/feedback** step is downstream:
protected `0x08A` leaves the FRC as a request to the Brake/VMM arbiter, Brake returns the
arbitration result/status on `0x081`, and post-arbitration Brake-domain request-generation
logic generates the actuator-side target path. The recorder also contains external ABS/VSC
and EPS observations, so recorder presence alone still does not prove the exact code/core
that computes every downstream result.
...
flowchart LR
  ...
  O8 --> BRK["Brake / VMM<br/>request arbitration"]
  BRK --> O81["0x081<br/>arbitration result / status back to FRC"]
  BRK --> RG["post-arbitration<br/>request generation"]
  RG --> B6["protected B6<br/>steering-controller target"]
  B6 --> EPS["EPS / F33"]
  O81 --> APPS
```
...
egress. **The downstream VMM arbiter still remains after `0x08A`.** What remains open is
downstream implementation: the exact FRC packer/security controller, Brake
verification/request-arbitration/result/request-generation transform, B6 Tx/freshness/CMAC
owner, and the final physical routing into EPS. `0x08A` must not be drawn as EPS ingress
or as a bus carrying simultaneous raw LTA/LDA/PDA candidates.
...
**Historical transport-boundary note:** the later relay-open/FRC-CommunicationControl
experiment supersedes this section's former downstream-proxy interpretation and places
protected `0x08A` publication inside the FRC assembly. The negative Bus-1 search below
remains valid evidence about the FRC's separate native Bus-1 E2E interface, but it no
longer describes an unresolved FRC->external-proxy hop.

At this historical stage the source-side question was narrower than several earlier
summaries made it sound. We knew the FRC hosted Toyota's TSS3 request/arbitration-result
recorder vocabulary, the observed native Bus-1 periodic family used exact AUTOSAR E2E
Profile 5 rather than Toyota SecOC, and authenticated `0x08A` was visible on the
Bus-4/chassis pair. The retained CAN alone did **not** justify the leap from "we have not
found a simple Bus-1 request field" to "there is no unsigned FRC request egress" or "the
handoff must be private." That methodological negative remains useful even though later
source isolation closes ECU-level `0x08A` egress to the FRC assembly.
...
This design does not interpose on the logical `0x08A -> Brake/VMM request arbitration -> request generation -> B6`
path; its exact physical/security realization remains open and does not require that hidden link to carry camera-originated B6. Stock
