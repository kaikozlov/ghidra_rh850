...
The next live discriminator stays FRC DID `0x1601`. Current GTS+ resolves its
value dictionary, so the exact OEM-named enabled-state oracle is **`LTA Switch Condition
Flag=1 (ON)` plus `LTA Control Condition=0 (LTA Enabled)`**; the other control value is
`1=LTA Disabled`. This proves the FRC diagnostic control state, not continuous steering torque by itself.
Current GTS+ also resolves FRC DID `0x1914` bit8 as **0=“Cruise Control Not in
Operation” / 1=“Cruise Control in Operation”**. The prepared normal-loggerd capture path
above therefore requires stable overlap of `0x1601` LTA-enabled state **and `0x1914=1`**,
plus independently observed motion and healthy Bus-4 protected traffic, before treating a
continued zero-B6 result as the strong operational discriminator. The identity-bound
poller itself is mechanically read-only (`22 16 01` / `22 19 14` only) and stops if either
exact positive response stream is absent/stale for two seconds; this prevents capture
plumbing from becoming a second Panda owner or a vehicle-control path. If that synchronized
operating context is captured while B6 remains absent, do **not** resume an arbitrary
EPS-ID hunt: move the RE boundary outward to the FRC/Brake transformation or inward to a
non-COM/internal EPS path. Deterministic evidence is
