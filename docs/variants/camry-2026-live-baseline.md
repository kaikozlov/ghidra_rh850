...
**Consequence.**  The stock-competition explanation is now closed for the B6
non-response seen here.  We no longer need to hypothesize that an admitted comma
B6 was merely being averaged against a simultaneous FRC LTA request: Toyota's
autonomous request/reference plane is demonstrably ID0 for ~201 s while B6 stays
ID11, and multi-second large B6 errors still produce almost no rack/motor response.
This does **not** identify the failing rung.  It instead pushes the problem back
to B6 admission/authentication/freshness or a downstream cooperative-controller
gate inside the EPS.  VAR-114..123 remain the relevant acceptance-ladder work;
there is no justification for changing steering sign, scale, or planner tuning
