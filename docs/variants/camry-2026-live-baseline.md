...
role-0 **`DelDiagCodeP4.dll`**. Its current master exposes clear selector `0x01`
...
The next live discriminator stays FRC DID `0x1601`. If a synchronized capture
machine-proves `LTA Control Condition` active while B6 remains absent, do **not** resume an
arbitrary EPS-ID hunt: move the RE boundary outward to the FRC/Brake transformation or
inward to a non-COM/internal EPS path. Deterministic evidence is
