...
**0.026993%** error). In the retained ID11 intervals the same field changes from
feedback-shaped to a leading quantity: best measured-angle lag is **+50 ms** in drive A
and **+225 ms** in drive B. Current `EMPS2_P5` DID `0x1CEE` independently places
...
Current GTS+ topology places Front Camera on Toyota Bus 1 and Brake/EPS together on Bus
4, while exact F33 accepts protected B6 on the latter. The retained `0x08A` therefore
...
Bus-1/upstream-to-Brake/EPS-B6 transformation and signer, or authorize steering output.
...
protected B6 remains its ingress. The missing work is therefore the upstream-frame
integrity/authentication contract and the Bus-1/upstream-to-B6 transform, signer,
