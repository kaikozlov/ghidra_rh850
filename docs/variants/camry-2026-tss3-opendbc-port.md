...
example a touch-wheel Camry, XW60 Prius, Grand Highlander, or RX): independently
toggle left/right hand contact while logging the meter-side network and FRC-side
network, then join those transitions to SRS grip VCH and FRC hands-on state. A
normal comma harness may not expose the **meter's Bus 3**, so a direct meter-side
CAN tap or gateway-side capture may be required. The reproducible static evidence
for this recovery is generated by
`tools/techstream/extract_gtsplus_tss3_steering_touch_path.py` into
