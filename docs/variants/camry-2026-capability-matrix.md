...
| Steering faults | Current `0x030 B6[2]`, cooperative command inhibit B16[0], and angle inhibit B19[0] feed normal temporary-unavailability reporting. The Sep-17 replacement-rack road route adds 11 moving temporary drops, all B16[0] with B6[2]/B19[0] clear; route + firmware branch elimination identifies the recoverable `CEE7C -> CAFB` **target-step plausibility** gate (ID11: 78 raw/effective sequence step), not CAN/Panda loss | B16[0] still merges recoverable CAFB with latched CAFD at the wire. The route proves the >78 discontinuity occurs after smooth/fresh C7 inside B6 replacement; a skipped replacement from transient/non-clean command-5 completion is the leading mechanism, but resident-private miss counters were not logged on this drive |
...
| Host-command loss | Unified C7 control uses a seven-nominal-5-ms lease: changed nonzero generations renew it, unchanged generations expire, and zero releases immediately. The restored split field path is now live-proven through install on the replacement rack: the **522-byte** high resident survived byte-exact, initialized at count 224, all **150 C6 words / 600 helper bytes** transferred and read back, and arm succeeded. C6 is installation-only; recurring control remains C7. The helper is 594 bytes before padding. | First native-oracle qualification then false-failed before command 5 (`native_frame_count=5`, `command5_attempts=0`, `native_verified=2`). The captured native B28 was `0x78` while steady authenticated/committed state later agreed at trip/reset `600/888`, consistent with sampling a reset-transition frame one epoch behind. The helper now leaves freshness skew and command-5 rc2 retryable and latches failure only on non-retryable completion/driver errors or a clean MAC mismatch. This retry fix is regression-built and needs one live rerun. |
