...
| Host-command loss | The universal 4-KiB signer payload self-selects the F33 profile, parks its 572-byte helper at `FEF07C00`, and the 462-byte high resident installs that helper into `FEBF0000` at the qualified count-224 boundary; C6 is no longer required. Changed C7 generation renews seven nominal 5-ms foreground ticks; unchanged generation expires; zero releases immediately. After the first healthy-rack attempt on 2026-09-17 returned to stock application with the expected high resident absent, the bootstrap was corrected to discriminate the boot family at low CodeFlash `0x0C80`, run the exact family boot-init/validity sequence, and only then read application identity `0x20860`; install now also waits through count 224 and byte-attests both resident and helper | Corrected one-shot universal installation is regression-built but **awaits healthy-rack live requalification**; the failed first attempt is not steering/actuation evidence |
