...
| Host-command loss | Unified C7 control uses a seven-nominal-5-ms lease: changed nonzero generations renew it, unchanged generations expire, and zero releases immediately. The field path has reverted to the proven split architecture: an exact-F181-bound F33 stage installs a **522-byte** high resident at `FEBFF9F0`; after count 224, the host loads the current **572-byte helper padded to 600 bytes / 150 words** over functional `0x777` C6 and arms it. C6 is installation-only; recurring control remains C7. No field payload writes `FEF07C00` in boot context | Same-rack A/B on 2026-09-17 proved the old audited replay payload `48f269ae…` still retains its 406-byte resident byte-exact on the replacement EPS, so rack/boot-call/high-tail regression is excluded. Both newer self-install attempts cleared the resident. The split field resident/current helper combination is regression-built and now needs live qualification; the boot-context GlobalRAM path remains experimental |
