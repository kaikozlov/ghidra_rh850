...
| Host-command loss | The universal 4-KiB signer payload self-selects the F33 profile, parks its 572-byte helper at `FEF07C00`, and the 462-byte high resident installs that helper into `FEBF0000` at the qualified count-224 boundary; C6 is no longer required. Changed C7 generation renews seven nominal 5-ms foreground ticks; unchanged generation expires; zero releases immediately | The one-shot universal installation is statically/reproducibly built but **not yet live-qualified** on the healthy rack |
