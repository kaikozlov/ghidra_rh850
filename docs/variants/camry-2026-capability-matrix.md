...
| Host-command loss | The unified C7 resident uses a seven-nominal-5-ms lease: changed nonzero generations renew it, unchanged generations expire, and zero releases immediately. The field kit now uses an **exact-F181-bound one-shot bootstrap**: known F33 boot init and validity calls, no boot-time CodeFlash data read, 462-byte resident to `FEBFF9F0`, and 572-byte helper transit through `FEF07C00` for count-224 installation into `FEBF0000`; C6 is not required. Install waits through count 224 and byte-attests resident plus helper before READY handoff | Two 2026-09-17 healthy-rack attempts with the experimental self-dispatching all-target bootstrap both returned to stock application with the expected high resident absent; the second settled read showed `FEBFF9F0` effectively zero. The exact-target field bootstrap is regression-built but **awaits healthy-rack live qualification**; neither failed attempt is steering/actuation evidence |
