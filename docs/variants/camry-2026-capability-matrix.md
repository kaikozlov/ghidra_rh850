...
| Native longitudinal | **Not advertised on Camry after the September-16 role audit.** The former `0x160` encoder is historical/RE code. `0x08A` is the established FRC-side request plane; B8:B9/B11:B12 form the indistinguishable signed16 ×0.001 upper/lower acceleration-request pair. Brake-owned `0x081` adds selected longitudinal ID B6[5:0] and result acceleration B20:B21 candidates, giving the first coherent request→arbitration-result layout | Exact upper/lower A/B ordering remains unresolved. B6/B7 now strongly fit the two packed request-ID/allocation bytes (ID bits7:2, allocation bits1:0); shift/EPB/override/priority metadata and `57D3` validity remain unresolved. Stock Toyota-B leaves `0x08A` unsplit, so a clean source-suppression/sole-emitter boundary is still required before native long. Do not revive `0x160` or compete with stock `0x08A` |
...
Long toggle is requested. Protected `0x08A` is the already-established FRC-side
