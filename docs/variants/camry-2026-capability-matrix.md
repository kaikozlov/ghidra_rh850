...
**Current checkpoint: September 17, 2026, after replacement-rack road qualification and the post-authenticated B6 ownership redesign.**
...
| Steering faults | `0x030 B6[2]`, cooperative command inhibit B16[0], and angle inhibit B19[0] feed ordinary temporary-unavailability reporting. The Sep-17 road route has 11 moving B16-only drops; exact branch elimination closes them as recoverable `CEE7C -> CAFB` target-step failures (ID11 limit 78 raw/effective sequence step) while host C7 changes by at most 3 raw. Historical working route `8d` has the same raw inhibit four times; route `93` has none | The old pre-SecOC signer could let an untouched native B6 through whenever per-frame replacement failed. The new Camry field backend removes that race by authenticating native B6 completely stock and overriding only the post-SecOC route44 application copy before `4BD46`. It is regression-built and still needs live verification that the B16 events disappear |
...
| Host-command loss | Unified C7 retains the seven-nominal-5-ms lease: changed nonzero generations renew it, unchanged generations expire, and zero releases immediately. Split installation is live-proven on the replacement rack (522-byte high resident, 150-word/600-byte C6 transfer, byte-exact arm). The new Camry helper is **218 bytes** before padding and ignores C6/other DCM traffic for runtime generations | The prior command-5 oracle and signer were live-qualified, including exact native/computed trailer equality, but command 5 is no longer part of the Camry driving path. New qualification proves native authenticated route44 publication while released; live lease/override/release still needs one bounded rerun |
...
| Stock-ACC coexistence / recovery | The diagnostic clear path preserves pre-clear DTC evidence and can remove the EPS communication-warning state without resetting the resident | **Observed negative on the exact Camry:** DRCC did not re-enable in the same ignition cycle after the EPS programming transition and DTC clear. A full vehicle restart restored DRCC but removed the RAM signer. Production coexistence therefore requires avoiding the disruptive programming handoff or recovering a deeper peer state than DTC memory |
...
| Deployment | Unified TSS3 kit builds exact Camry/Crown split-loader and Corolla embedded-helper variants around one functional-`0x777` C7 contract. Camry field runtime is now post-auth route44 override: no persistent patch, SecOC-result bypass, recovered key, or runtime command-5 signing is required | Camry post-auth helper ordering and mutation boundary are firmware-closed and regression-built but need on-car qualification. `0x08A` remains the cleaner long-term Toyota request-plane architecture once sole-source ownership is recovered |
