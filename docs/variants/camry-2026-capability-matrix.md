...
| Host-command loss | Unified functional-`0x777` split helper is 596 bytes in the existing 600-byte transfer image; changed C7 generation renews seven nominal 5-ms foreground ticks; unchanged generation expires; zero releases immediately | Lease semantics transfer from the road-proven supervised helper, but the new functional-`0x777` binary/transport combination is **not yet live-qualified** |
