...
| Findings with exact `verified` grade | 64 | Exact-grade rows in `FINDINGS.md`; qualified/mixed/partial-grade rows are not included in this scalar |
...
| Exact cross-variant/target-generation transfers verified | 1 tracked foreign CodeFlash regression (`8965H1202000`) | The foreign Corolla image independently transfers the Gate-2 semantic/CRC resolver, all three crypto roots, boot-SA stage-1 shape, callback-free runtime control discovery, application/boot CAN1 continuity, and asynchronous PROGRAMMING reset architecture. Its actual Gate-2 queue is `00F/D7/B6`, so steering-bridge applicability correctly fails closed. Official comma hardware plus the field pin-swap report now also bounds Toyota-B CAN1-vs-relay topology; `4514000`, Span's `8965F1208000`, and newer-TSK applicable targets remain artifact/evidence bounded |
