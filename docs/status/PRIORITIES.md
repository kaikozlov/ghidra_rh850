...
1. **Recover Toyota's stock request/signing and source-suppression architecture.** VAR-153/CORR-186 close the EPS-side software question: F33 has one configured B6 receive path, copies each successfully delivered 32-byte profile2 PDU unchanged into route44, and has no recovered B6 constructor, sanitizer, replacement path, or autonomous route44 ticker. The retained zero-host-TX control therefore contains independent successful profile2 deliveries; it does not show Panda ID11 being rewritten to ID0. The Sep-10 two-stage ingress observer additionally saw D7 and native B6 at the deterministic post-CanIf/pre-SecOC boundary while seeing none of 121 Panda-returned ID63 markers. For that exact treatment, neither EPS SecOC rejection nor EPS post-ingress rewriting explains the miss.
   ...
   OQ-054 now owns the native production blocker. **FRC application selection is upstream of the wire:** LTA/LDA/LCA/PDA/PCS are FRC-resident feature applications that can be enabled simultaneously; the FRC state machine selects the current owner and emits the resulting generic request as `5282` / protected `0x08A`. `0x081` is Brake-owned downstream result/status feedback. The repin direction experiment closes their physical flow: protected `0x08A` is native on the FRC/camera-side endpoint of the intercepted Toyota Bus-4 pair and moves toward Brake; `0x081` is native on the Brake/chassis side and returns toward FRC. FRC normal-Tx suppression removes `0x08A`, placing its secured publisher inside the FRC assembly. Brake/VMM then validates/realizes the already-selected FRC request and generates the downstream steering-controller instruction B6 toward EPS. Recover (1) the FRC-internal feature-owner state machine and `0x08A` signing/key path, and (2) the Brake `0x08A` verification/result/request-generation -> B6 path; do not search Brake for LTA-vs-LDA-vs-PDA selection and do not send `0x08A` to EPS itself.
...
the FRC->Brake vehicle-network transform/cadence, downstream Brake result/request-generation
...
recovery and (b) downstream Brake validation/result/request-generation to B6; synchronized
...
classes and residual STORE/DMA holes. CORR-134/VAR-081 recover `0x08A` as a lateral-request representation; CORR-135 rejects a required byte-preserving transform into B6. Later source evidence closes `0x08A` publication to FRC. The remaining discriminators are the **FRC-internal feature selector + SecOC implementation**, the **downstream Brake result/request-generation transform**, and the **exact F33 B6-independent authority selector/modulator**, not more blind bus traffic.
