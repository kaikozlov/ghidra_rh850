...
2. **Preserve the demonstrated steering fallback while moving to the native path.** Primary route `0000008d--a9f348691a` observed 167.289 seconds of steering with C7 sideband control, the EPS-resident command-5 signer, and the installed cumulative stage-5 image; later same-build route `00000093--4066e7ae51` supplies a shorter independent confirmation. The exact route hashes, RAM identities, deployed openpilot file hashes, wire format, OFF/NRTD/READY lifecycle, and reconstructable five-file C7 patch are pinned under `targets/camry-2026/raw-20260910/working-steering/` and in the field runbook. The opendbc base commit alone is not sufficient. Keep this as a reproducible development fallback, not the normal openpilot architecture.
