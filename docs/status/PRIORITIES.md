...
1. **Recover Toyota's stock request/signing and source-suppression architecture.** VAR-153/CORR-186 close the EPS-side software question: F33 has one configured B6 receive path, copies each successfully delivered 32-byte profile2 PDU unchanged into route44, and has no recovered B6 constructor, sanitizer, replacement path, or autonomous route44 ticker. The retained zero-host-TX control therefore contains independent successful profile2 deliveries; it does not show Panda ID11 being rewritten to ID0. The Sep-10 two-stage ingress observer additionally saw D7 and native B6 at the deterministic post-CanIf/pre-SecOC boundary while seeing none of 121 Panda-returned ID63 markers. For that exact treatment, neither EPS SecOC rejection nor EPS post-ingress rewriting explains the miss.
   ...
   OQ-054 now owns the native production blocker. `0x08A` is the upstream request and `0x081` the Brake-owned selected/result feedback. The repin direction experiment closes their physical flow: protected `0x08A` is native on the FRC/camera-side endpoint of the intercepted Toyota Bus-4 pair and moves toward Brake; `0x081` is native on the Brake/chassis side and returns toward FRC. FRC normal-Tx suppression removes `0x08A`, placing its secured publisher inside the FRC assembly. Brake/VMM then generates the downstream steering-controller instruction B6 toward EPS. Recover the FRC-internal `0x08A` signing/key path and the Brake `0x08A` verification/arbitration -> B6 path; do not send `0x08A` to EPS itself.
...
VAR-083 now close the shared command→current-control funnel. CORR-134/VAR-081 recover Bus-4 `0x08A` as the request representation, and CORR-135 shows zero-B6 factory LTA is already compatible with exact F33's internal assist path. The next discriminators are **who produces/security-protects `0x08A`** and **which exact F33 state selects/modulates the B6-independent path**. FRC/EPS synchronization remains useful corroboration, not producer proof.
...
classes and residual STORE/DMA holes. CORR-134/VAR-081 recover `0x08A` as a lateral-request representation; CORR-135 rejects a required transform into B6. The remaining discriminators are its **producer/SecOC/arbitration ownership** and the **exact F33 B6-independent authority selector/modulator**, not more blind bus traffic.
...
Static priority is OQ-054 after CORR-195/196/197 and VAR-161/162: recover the **physical implementation** of Toyota's now-closed logical Vehicle Movement Manager chain. US20200070849A1 plus current GTS separate application request, arbitration-result feedback, post-arbitration request generation, and priority Vehicle Movement Control. The strongest Camry join is `0x08A` = TSS request-side package, `0x081` = Brake/VMM result/status publication, and protected B6 = final steering-controller target/instruction. Exact F33 independently attributes B6 loss to the Brake System Control Module, and later internal route44/profile2 evidence proves native B6 delivery despite older Panda-visible zero-B6 rlogs. CORR-196 localizes direct Panda B6 loss before successful F33 controller admission. VAR-162 then resolves the topology clue: `EBU` is the junction/attachment label on the exact-Camry EPS component row, not a separate installed ECU; Brake Booster/Skid are invariantly attached through No.2 Global CAN Junction, and `ABS_P5` explicitly carries Power-Steering `ch2` communication vocabulary. The leading physical target is therefore exact category-435 Brake/Skid `F152633K0000`: recover its request-generation transform, B6 Tx/secondary-channel routing, source suppression, and SecOC signing/freshness ownership. Central Gateway remains an upstream transport candidate where evidence requires it, but no separate EBU filter ECU should be invented. VAR-091/101/107 and CORR-194 still keep FRC private-preauthentication distinct from physical Bus-4 publication.
...
start producer analysis. Until then any exact Brake-owned B6 Tx/signing/freshness implementation and `0x08A` physical producer/security ownership are acquisition-blocked, not negative. The **logical** `0x08A` request-side -> Vehicle Movement Manager arbitration/request-generation -> B6 final-target relationship is now strongly supported by Toyota's architecture and the independent GTS/F33 names; only its exact F33 physical/security realization remains open. FRC physical Bus-4 ownership is not assumed.
