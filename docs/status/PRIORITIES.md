...
2. **Recover Toyota's stock request/signing architecture.** OQ-054 asks which
   ...
   streams and recovered perception slots but neither consecutive `5282` nor the
   `0x08A` application. Close this with synchronized FRC Operation FFD,
   source-identifying capture, or matched FRC/Brake firmware. Do not block the
   first B6 development probe on this attribution and do not send `0x08A` to EPS.
...
**Current development-path update (VAR-117/CORR-157):** stop adding persistent SecOC-result patches. Stages 3–5 were each applied, reboot-verified, and remained non-delivering at direct PDU44 COM. The configured exact-F33 receive path is closed from RSCFD rule39/CanIf descriptor39 through SecOC profile2 `FEBE547A/FEBE54D4`, freshness/command-7 processing, native-success `8F546`, route44 `7D72C`, COM `FEBE4BFF/FEBE5364`, and `4BD46 -> 80BC/80B8`; no further downstream result/status gate is justified. The next experiment is **non-bypassing observation first**: install the 522/524-byte RAM observer in NRTD, heartbeat-attest, transition directly NRTD→READY without OFF, and run the stationary probe with `--require-observer`. It latches the last B6 seen at the pre-`667E6` queue sample point, pre/post SecOC result/publication state, post queue length, and ICU-S done/status while the host reads freshness slots, COM/application cells, `CB38 -> CC48 -> CC60 -> CC62/CC64`, and raw `0x08A/0x081/0x030`. If queue ingress is observed, only then run the separate RAM route44 bridge with `--require-bridge`; no CodeFlash patch and no nonzero offset before `ADMITTED`. A negative observer means **not observed at the pre-aggregate queue sample point**, not proof that the frame never existed at any instant. OQ-054 stock proxy ownership remains separate. Canonical: [../variants/camry-2026-live-baseline.md](../variants/camry-2026-live-baseline.md) §§40,57.6.
...
**Current exact-F33 software state:** the read-only scaffold is complete and the development B6 path is present but default-off. Current commits are opendbc `8da4bb9b` and kai-openpilot `6dd58cf5e`: passive `0x08A` observables remain, default/release stays `dashcamOnly` + Panda `noOutput`, and the exact-F181 non-release path can select a zero-MAC28 B6 sender plus B6-only `TSS3_DEV_LATERAL` safety after external bridge attestation. Do not rebuild basic state parsing or fabricate a stock B6 template; the current base is explicitly non-stock and must be validated against a patched/bridged receiver.
