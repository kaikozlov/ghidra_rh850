...
1. **Recover Toyota's stock request/signing and source-suppression architecture.** VAR-153/CORR-186 close the EPS-side software question: F33 has one configured B6 receive path, copies each successfully delivered 32-byte profile2 PDU unchanged into route44, and has no recovered B6 constructor, sanitizer, replacement path, or autonomous route44 ticker. The retained zero-host-TX control therefore contains independent successful profile2 deliveries (or an unrecovered mechanism); it does not show Panda ID11 being rewritten to ID0. The old reactive-replacement model is retired.

   OQ-054 now owns the production blocker. Determine which Brake/Skid/CGW participant transmits/signs the inactive ID0/status stream, how FRC request/winner state reaches that participant, and how the stock source is suppressed or replaced. This includes the separate Bus-4 `0x08A` signer/proxy question; do not send `0x08A` to EPS. Close it with matched Brake/FRC firmware or a source-identifying capture. If the B6 source shares the EPS-side chassis segment, Panda's camera-bus relay cannot suppress it and direct same-ID injection is not a native production architecture. Do not add an EPS-side remint workaround, private permission state, or another SecOC result patch.

2. **Keep the deterministic observer only as an optional topology discriminator.** VAR-152's 498-byte mid-aggregate observer remains the correct way to prove whether a particular Panda B6 reaches F33 rule39/CanIf/profile2 on the installed harness. It is no longer needed to rediscover the EPS software route. Run it only if physical receiver admission is required to choose a harness/source-suppression design: require the D7 positive control, then use one ID63 exact-signature block with no SID23 polling during treatment. The assembly route44 bridge remains an ephemeral diagnostic intervention, is timing-blind at its current between-aggregate sample point, and is not a production fix.
...
- Corrections with this document as canonical home: [CORR-131](../reference/index.md#correction-corr-131)
