...
## Current Camry lateral execution priority

Keep two queues separate:

1. **Recover Toyota's stock request/signing and source-suppression architecture.** VAR-153/CORR-186 close the EPS-side software question: F33 has one configured B6 receive path, copies each successfully delivered 32-byte profile2 PDU unchanged into route44, and has no recovered B6 constructor, sanitizer, replacement path, or autonomous route44 ticker. The retained zero-host-TX control therefore contains independent successful profile2 deliveries; it does not show Panda ID11 being rewritten to ID0. The Sep-10 two-stage ingress observer additionally saw D7 and native B6 at the deterministic post-CanIf/pre-SecOC boundary while seeing none of 121 Panda-returned ID63 markers. For that exact treatment, neither EPS SecOC rejection nor EPS post-ingress rewriting explains the miss.

   OQ-054 now owns the native production blocker. **FRC feature-owner selection is upstream of the wire:** LTA/LDA/LCA/PDA/PCS are FRC-resident feature applications that can be enabled simultaneously; the FRC state machine selects which feature populates the generic request and emits it as `5282` / protected `0x08A`. That feature selection is **not** the downstream Vehicle Movement Manager arbitration. The repin direction experiment closes physical flow: protected `0x08A` is native on the FRC/camera-side endpoint of the intercepted Toyota Bus-4 pair and moves toward Brake; `0x081` is native on the Brake/chassis side and returns toward FRC. FRC normal-Tx suppression removes `0x08A`, placing its secured publisher inside the FRC assembly. Brake/VMM then performs request arbitration on the submitted `0x08A` request, publishes arbitration result/status on `0x081`, and feeds post-arbitration request generation toward B6/EPS. Recover (1) the FRC-internal feature-owner state machine and `0x08A` signing/key path, and (2) the Brake `0x08A` verification/request-arbitration/result/request-generation -> B6 path; do not search Brake for simultaneous raw LTA-vs-LDA-vs-PDA feature requests and do not send `0x08A` to EPS itself.

2. **Preserve the demonstrated steering/recovery fallback while moving to the native path.** Historical routes `0000008d--a9f348691a` / `00000093--4066e7ae51` remain the original authority witnesses. September-18 route `0000010c--506d7277c7` is now the stronger coexistence witness: after Brake/EPB `10 02 -> 11 01` followed by FRC `10 02 -> 11 01`, the EPS resident survives and openpilot lateral overlaps stock adaptive cruise for **919.572 s / 19.772 km** with factory lateral IDs remaining 0. Preserve the exact recovery order, route hashes, current openpilot/opendbc identity, and resident lifecycle as the reproducible fallback while the native `0x08A -> Brake/VMM -> B6` architecture is recovered. Raw road logs remain outside git under `/Users/kai/dev/inspect/logs/camry-2026/2026-09-18/0000010c--506d7277c7/`; derived evidence is in `targets/camry-2026/raw-20260918/recovered-road-drive/summary.json`.

3. **Retain the corrected ingress observer as completed topology evidence.** The original 498-byte full-runtime observer failed before initialization and is superseded. The live-qualified version reuses the known-good 524-byte resident and a 120-byte call-free helper. Its completed D7-positive/ID63-negative result is in `targets/camry-2026/raw-20260910/f33-ingress/session-summary.json`; rerun only to test a materially different ingress path or treatment.
...
FRC `0x1601` LTA-active/CAN capture must confirm or refute.
...
the FRC->Brake vehicle-network transform/cadence, downstream Brake request-arbitration/result/request-generation
behavior, SecOC/integrity owner, and suppression/fallback point. It is **not** needed to decide
which lateral feature owns the request; that selection occurs inside the FRC before `0x08A`. Separately, TMS-086 closes **generic real-TSE framing validation** with four public legacy
...
sequence, epoch-consistent reset bits, 50-Hz cadence), all bus-off/core-reset activity
...
VAR-081/CORR-134 identify the observed `0x08A` request representation; CORR-135 rejects the
old assumption that exact F33 itself receives that request. The repin/source experiment is the
stronger physical-source result: the intercepted Toyota Bus-4 pair is split at the FRC connector,
`0x08A` is native on the FRC/camera-side endpoint and forwarded toward chassis, while `0x081`
is native on the Brake/chassis side and returns toward FRC. FRC normal-Tx suppression removes
`0x08A`; Brake normal-Tx suppression removes `0x081`. The secured `0x08A` publisher is therefore
inside the FRC assembly, while the exact internal signer/key/freshness implementation remains
open. Native Toyota Bus 1's 22 camera/radar streams are a separate E2E interface, not evidence
for an external `0x08A` proxy.

**VAR-088 completes the `0x08A` field census and the fork DBC entry** (live-baseline
§39): every application byte is closed across 44,613 deduped bus-0 frames, and EMPS
`0x1CEE` supplies the four-monitor cooperative-control naming. The old Bus-4-DDB producer
negative is superseded by the later source experiment: secured `0x08A` publication is
inside the FRC assembly. Do not redo the byte census or reopen Brake/CGW as `0x08A`
publisher candidates. Remaining work is (a) FRC internal feature-owner/packer/signing
recovery and (b) downstream Brake verification/request-arbitration/result/request-generation to B6; synchronized
FRC Operation FFD is the key selector-state oracle.
...
OEM internal clear/normal classifier state, not independently a Ready authorization bit.

VAR-063 now verifies the maintainer Camry's physical Toyota-B repin itself: the F33
...
B6 signal261 is the sole recovered generated-COM mode selector and signal262 the sole recovered generated-COM external magnitude **inside the shared `FEBECC50/FEBECC62` command funnel**; every non-B6 COM member there is feedback, monitor, plausibility, or gate state. Signal243 explicitly
...
Value Torque** downstream. **CORR-130/VAR-083 now close the physical current-control
convergence:** `CC62` is a pre-slew actuation value, `AC56/EE40A/1C02` is its diagnostic
mirror, and sibling `CC64/AC54/EE40C` drives `6AF4 -> 6E0A -> 6DEC/6DC8/6DD6`. Do not
re-run the broad scalar model-cone derivation or the downstream motor join; move the static
search **upstream of CC50/CC62** instead.
...
B28..B31 strongly match Toyota ordinary-P5 `FV4 || MAC28`; the repin/source experiment now places the protected `0x08A` publisher inside the FRC assembly, while the exact FRC-internal key/profile/CMAC engine, Brake verification path, and actual grant remain open. VAR-113/CORR-153 explicitly leave transformed, multi-field, multiplexed, event-driven, and private-link alternatives open rather than assuming the handoff is private. CORR-135 rejects the old upstream-to-B6 inference; CORR-136 preserves the timing/source-attribution correction, and CORR-194 supersedes CORR-149's FRC key-holder exclusion.

**VAR-081/083/087/091/092/093/094/101/107 steering-command boundary — current highest priority:** keep four questions separate: request publication, physical Bus-4 publication, cryptographic ownership, and grant/actuation (plus any candidate openpilot ingress). `0x08A` B21=Target Lateral ID, B18:B19=signed target angle at the numerically matching `1024/17870` scale, B26 is modulo-64 behavior, and B28..B31 strongly match Toyota ordinary-P5 `FV4 || MAC28`. VAR-091 places it on captured Bus 4 and excludes exact F33 Tx/Rx; VAR-107 closes native Bus-1 traffic as exact AUTOSAR E2E Profile-5 framing—CRC-16/CCITT `0x1021`, B2 alive counter, implicit DataID=CAN ID—with no secret-bearing authenticator on those public PDUs. **CORR-194 corrects the former hardware inference:** current FRC diagnostics/service procedure prove camera-family ECU-Security-Key provisioning, so native Bus-1 E2E framing does not exclude an unseen/private FRC pre-authentication step. The repin/source experiment supersedes the old downstream-publisher candidate set for `0x08A`: the protected request is native on the FRC-side endpoint and its secured publisher is inside the FRC assembly. Exact CMAC/key selection within that module remains open. Rlog timestamps batch a median 14 bus-0 frames, invalidating the former 20/30 ms/`0x0D7`-queue attribution. VAR-093 reads the Bus-1 family as plaintext object slots; VAR-094 closes only consecutive `5282` on native Bus 1 and the inbound SAS-echo direction. VAR-092 closes default-bank `D0218` as not an F33 COM milliradian.

VAR-110/111 sharpen the actuation side. The retained stock-steering witness proves sustained episodes where exact-F33 motor feedback and wheel motion point toward the `0x08A` ID11 target while measured driver torque opposes that direction, with **zero B6**. But the exact F33 recensus now separates physical B6-independent assist from external lane-target authority: the H-like `CEFFC/CE144/.../CF2B2 -> CB38 -> D0218` autonomous-controller family is itself sourced from protected PDU44/B6; `CC5A` is only delayed `CC60` history; `C81A` is local damping/assist; FlexRay/PSI5 are unused; and RSENT1 terminates in steering-torque sensor acquisition. Therefore "D0218 can drive current without B6" must not be read as "D0218 contains the stock lane target." The unexplained stock authority hop is now outside the recovered F33 external-command surface. Required next evidence is (1) FRC Operation FFD `5282/5285/57DE/5265` synchronized with CAN to separate request from winner/grant, (2) FRC firmware/HSM evidence to recover the `0x08A` key/freshness/CMAC generation path, and (3) exact category-435 Brake firmware to recover `0x08A` verification/arbitration plus downstream B6 construction/routing. Do not repeat completed F33 COM/peripheral scans, infer an `0x08A -> B6` transform, or send `0x08A` to EPS. Protected B6 stays a separate external-actuation candidate whose signing/suppression contract can be evaluated independently.

**VAR-096 physical-publisher set bound is superseded for `0x08A` source attribution by the repin/source experiment.** The install-set result still proves there is no separate ADAS arbitration ECU co-installed with FRC_P5 498, but it no longer justifies hunting Brake/Booster/CGW as the protected `0x08A` physical publisher. `0x08A` is native on the FRC-side endpoint before the split; only the signer implementation *inside the FRC assembly* remains open. Brake/Booster/CGW remain relevant to downstream request consumption/arbitration and other protected publications. Canonical source-direction evidence: September-1 communication notebook plus `camry_2026_longitudinal_request_plane.json`.
...
carriers; it does **not** attribute this particular motor/current component to LTA. CORR-130/
VAR-083 now close the shared command→current-control funnel. CORR-134/VAR-081 recover `0x08A` as the request representation, and CORR-135 shows zero-B6 factory LTA is already compatible with exact F33's internal assist path. Later source isolation closes the ECU-level `0x08A` producer to the **FRC assembly**. The next discriminators are **which FRC state machine selects the current feature owner and which internal security path protects its `0x08A` egress**, plus **which exact F33 state selects/modulates the B6-independent path**. FRC/EPS synchronization remains useful for the selector/plant join, not producer attribution.
...
classes and residual STORE/DMA holes. CORR-134/VAR-081 recover `0x08A` as a lateral-request representation; CORR-135 rejects a required byte-preserving transform into B6. Later source evidence closes `0x08A` publication to FRC. The remaining discriminators are the **FRC-internal feature selector + SecOC implementation**, the **downstream Brake request-arbitration/result/request-generation transform**, and the **exact F33 B6-independent authority selector/modulator**, not more blind bus traffic.
...
Static priority is OQ-054 after CORR-195/196/197 and VAR-161/162: recover the **physical implementation** of Toyota's now-closed logical chain. On the exact Camry, FRC-resident LTA/LDA/LCA/PDA/PCS state is selected internally before protected `0x08A` is published; current GTS feature-local objects (`5531/5631/568x/5Axx/5D8D`) precede generic `5282`, and the retained ID changes are FRC egress-owner changes. US20200070849A1 plus current GTS then place **request arbitration downstream of that generic request**, followed by result feedback, post-arbitration request generation, and priority Vehicle Movement Control. The strongest Camry join is therefore **FRC internal feature selector -> `0x08A` request-side package -> Brake/VMM request arbitration -> `0x081` arbitration-result/status -> protected B6 steering-controller target/instruction**. Exact F33 independently attributes B6 loss to the Brake System Control Module, and later internal route44/profile2 evidence proves native B6 delivery despite older Panda-visible zero-B6 rlogs. CORR-196 localizes direct Panda B6 loss before successful F33 controller admission. VAR-162 then resolves the topology clue: `EBU` is the junction/attachment label on the exact-Camry EPS component row, not a separate installed ECU; Brake Booster/Skid are invariantly attached through No.2 Global CAN Junction, and `ABS_P5` explicitly carries Power-Steering `ch2` communication vocabulary. The leading physical target is therefore exact category-435 Brake/Skid `F152633K0000`: recover its request-arbitration/result/request-generation implementation, B6 Tx/secondary-channel routing, source suppression, and SecOC signing/freshness ownership. Central Gateway remains relevant only where independent routing evidence requires it; it is not an `0x08A` publisher candidate, and no separate EBU filter ECU should be invented. VAR-091/101/107 and CORR-194 still require us to distinguish the FRC's native Bus-1 E2E interface from its chassis-facing secured `0x08A` path, even though both now belong to the FRC assembly at ECU level.
...
start producer analysis. Until then any exact Brake-owned B6 Tx/signing/freshness implementation and downstream `0x08A` verification/request-arbitration/result/request-generation implementation are acquisition-blocked, not negative. The **logical** relationship is now: FRC-internal feature-owner selection -> feature-selected but pre-VMM-arbitration `0x08A` request egress -> Brake/VMM request arbitration -> `0x081` result/status + request generation -> B6 final target. Only the downstream physical/security realization remains open; the lateral feature-owner selection itself is not delegated to Brake.
...
This capture can independently cross-check B6 visibility and EPS internal-assist state during named LTA, but stock B6 cadence is no longer a prerequisite. It should instead help attribute exact F33 term/authority changes and, if B6 ever appears in a different mode, bound that separate external-interface behavior. The remaining
...
**Current exact-F33 software state:** `kai-openpilot@7aece7f63` / nested `opendbc@f207c273b645` / `panda@bbc93b17d861` use the ordinary Toyota/openpilot ownership shape: exact-F33 Camry is a normal angle-control platform, `CC.latActive` owns lateral activation, Panda uses ordinary Toyota TSS3 safety, and the sender emits a normal SecOC envelope with a fixed all-zero dummy AES-128 key. The real slot-4 key remains unknown; CORR-178 proves the dummy tag is acceptance-equivalent to the historical zero marker under cumulative stage 5. There are no bridge-attestation Params, `ALLOW_DEBUG`, or `TSS3_DEV_LATERAL` runtime gates. Do not rebuild basic state parsing, reopen an EBU-private repin theory, or fabricate a stock B6 template; localize B6 admission at the internal queue/route44/application ladder first.

For the Camry, two next steps run in parallel but do not block each other. VAR-116/CORR-156 now close stage 2 live: `8F948=003A` + `8F952=E001` is reboot-persistent/CRC-valid, but a READY/Park admission run still sent/echoed 84/84 active B6 frames with `ADB0=0` and `CB00=7`, so stage 2 is disproved as sufficient. Full `FUN_0008F906` recovery moves the next discriminator to the root result materialization at `8F930`: NRTD zero-write exact preflight of live stage 2, APPLY only if exact `E10F14D3->E00714D3`, full OFF->NRTD zero-write persistence verification, then full OFF->READY and repeat the admission-only ladder. The upgraded probe reads direct COM `80BC/80B8` before `ADB0/AE90`, so another negative localizes the failed handoff. Do not request a steering offset until ID11 is `ADMITTED`. The stock-architecture path independently needs synchronized FRC Operation FFD `5282/5285/57DE/5265` plus source-capable evidence for the protected-request owner. Do not send `0x08A` to EPS.
