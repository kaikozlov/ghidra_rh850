...
Static priority is OQ-054 after CORR-195/VAR-161: recover the **physical implementation** of Toyota's now-closed logical Vehicle Movement Manager chain. US20200070849A1 plus current GTS separate application request, arbitration-result feedback, post-arbitration request generation, and priority Vehicle Movement Control. The strongest Camry join is `0x08A` = TSS request-side package, `0x081` = Brake/VMM result/status publication, and protected B6 = final steering-controller target/instruction. Exact F33 independently attributes B6 loss to the Brake System Control Module, and later internal route44/profile2 evidence proves native B6 delivery despite older Panda-visible zero-B6 rlogs. Rlog cadence still cannot attribute the protected physical transmitter. VAR-091/101/107 keep the observed FRC/native-Bus-1 side E2E-P05 rather than SecOC, while CORR-194 keeps FRC ECU-Security-Key/private-preauthentication distinct from physical Bus-4 publication. Candidate Brake/CGW/Skid producer firmware is now specifically valuable for locating the request-generation transform, hidden/admitted B6 transport, source suppression, and SecOC signing ownership—not for deciding whether a logical request->target layer exists.
