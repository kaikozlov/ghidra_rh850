...
Static priority is OQ-054 after CORR-149/CORR-194: obtain Brake/CGW/Skid producer firmware and trace the FRC request handoff into the Bus-4 SecOC publication. Rlog cadence cannot attribute a transmitter. VAR-091/101/107 close the observed FRC side as request-plane/native-Bus-1 E2E rather than SecOC, while CORR-194 reopens FRC ECU-Security-Key ownership/private pre-authentication as distinct from physical Bus-4 transmission. VAR-092/094 preserve default-bank `D0218` and consecutive-`5282`-on-Bus-1 closures; VAR-085 closes E1/E2. Candidate downstream producer firmware is useful for physical-TX/proxy dataflow and for locating CMAC generation, not as a presupposed `0x08A -> B6` transformer.
...
start producer analysis. Until then any Brake-owned B6 Tx/signing/freshness implementation and `0x08A` producer/security ownership are acquisition-blocked, not negative; no `0x08A -> B6` relationship is assumed and FRC ownership is not assumed.
