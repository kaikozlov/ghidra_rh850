...
**Current exact live target (VAR-051/052):** the maintainer's 2026 Camry provides EPS F181 `8965F3307000 / 8A3113303100` on normal-harness `(bus1,param1)`, successful same-route PROGRAMMING handoff, and a retained stationary READY CAN baseline whose `00F/025/030/090/D7/0AA/101/116/127/176/51E` geometry is strongly H/F-like; `0x030` satisfies the exact H/F additive rule on 6,188/6,188 frames and `0x51E B0[7]` supplies the first retained Ready `0->1` transition. The follow-up NRTD pass now closes upstream identities too: FRC `0x792→0x79A` / F181 `8646F3315000`, Brake/EPB `0x7B0→0x7B8` / F181 `F152633K0000`. Current-GTS+ FRC cruise DIDs are live on the exact car, and synchronized operator input directly joins MAIN/RES+/SET-/CANCEL to bus-1 `0x0FE/32`; following-distance DID `0x1912` is dynamically validated, with `0x251/8` and `0x5AF/32` retained only as candidate ordinary-CAN state carriers. Corolla Brake DID `0x107E` is disproved as a directly transferable Camry oracle under tested default/extended sessions. **Do not run the H/F canary or transfer H/F boot-SA/RAM-exec geometry to this F181:** exact `8965F3307000` CodeFlash is still absent. The highest-value next vehicle work is now READY-state: stock LTA off→active→off with all buses, stationary P/R/N/D transitions, and actual cruise engage/cancel synchronized to FRC `0x1905/0x1914`. The existing zero-B6 segment did not exercise LTA and is only a bounded negative. XCP `0x7F7/0x7F8` is already negative on the tested EPS route/session and should not be blindly repeated. Canonical baseline: [../variants/camry-2026-live-baseline.md](../variants/camry-2026-live-baseline.md).
