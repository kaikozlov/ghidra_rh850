...
**Current development-path update (VAR-060/089/102, SECOC-074):** exact F33 persistent Gate-2 patch construction remains closed offline from a fresh bare import: `0x8F952 e0d1→e001`, exact image SHA `42dce8ef…d9b0e7`, stock-valid high CRC region, repaired fixup `0xD9AF33AF`, deterministic byte-exact restore, and DBFULL `0x400` flash pacing. VAR-089 separately supplies the audited reset-to-stock high-RAM zero-MAC28 re-admission candidate. The old stock-template sender was removed in `b9e86924` / `abf3ca70a`, but CORR-140 records that a new exact-F181 non-release path is present at opendbc `8da4bb9b` / parent `6dd58cf5e`. Do not revive the disproved stock-template premise. The immediate gate is receiver deployment plus stationary application/safety validation; OQ-054 proxy ownership and the chassis/reference-to-steering-assembly authority handoff remain separate architecture questions. OQ-053's volatile application pivot is specifically the remaining RAM deployment blocker, not a prerequisite for the persistent patch option. Canonical: [../variants/camry-2026-live-baseline.md](../variants/camry-2026-live-baseline.md) §14 and [../variants/camry-2026-tss3-opendbc-port.md](../variants/camry-2026-tss3-opendbc-port.md) §§3.4–5.
...
For the Camry, two next steps run in parallel but do not block each other. The execution path first needs the VAR-114 stationary B6 acceptance ladder: ID0 then bounded ID11 while observing `5364/80C8`, `80C9/F13E/ADB9`, `ADB0/AE90`, `CAFF/ACBD`, and `CB00`; only after positive application admission should testing move downstream to motor response, override, timeout/release, source coexistence/suppression, and fault recovery. The stock-architecture path needs synchronized FRC Operation FFD `5282/5285/57DE/5265` plus source-capable evidence for the protected-request owner. Do not send `0x08A` to EPS.
