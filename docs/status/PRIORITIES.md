...
**Current development-path update (VAR-060/089/102/115, SECOC-074):** exact F33 stage-1 persistent Gate-2 construction remains closed: `0x8F952 e0d1→e001`, stage-1 image SHA `272843a2…9f65`, fixup `0xD9AF33AF`, and valid residue. VAR-115 now adds the live consequence: under that reboot-verified stage-1 image a clean stationary zero-MAC28 ID11 stream produced 85/85 Panda TX echoes but did not replace `ADB0/AE90` and left `CB00=7`; stage 1 is therefore insufficient for this candidate. Exact call-site bytes bound stage 2 to `0x8F948 1a38→003a` so callback `8F94C` sees zero while stage 1 remains installed. The cumulative final fixup is `0xD12ADB05`, residue `FFFFFFFF`, final SHA `6a371a2a…d59c`, with deterministic zero-write preflight/APPLY/stage-2-only RESTORE/post-apply packages. The immediate gate is **live stage-2 preflight/APPLY/persistence verification followed by the same admission-only probe**; no steering offset before `ADMITTED`. VAR-089 remains the reset-to-stock high-RAM alternative and OQ-053 remains the production RAM-pivot problem, neither blocking this persistent development experiment. OQ-054 stock proxy ownership remains separate. Canonical: [../variants/camry-2026-live-baseline.md](../variants/camry-2026-live-baseline.md) §§14,57.4.
...
For the Camry, two next steps run in parallel but do not block each other. VAR-115 has now completed the first stationary B6 acceptance ladder under the installed final-compare-only Gate-2 patch: 85/85 active B6 TX echoes were observed, but `ADB0/AE90` remained on the prior ID0/current-angle snapshot and `CB00=7`, so the exact result is `payload_not_delivered`. The immediate execution sequence is the deterministic stage-2 callback-result experiment: zero-write exact preflight of `8F948=1A38` on the CRC-valid stage-1 image, APPLY `1A38->003A` only if preflight is exact, full OFF->READY, zero-write persistence verification, then repeat the same admission-only ladder. Do not request a steering offset until that repeated ID11 phase is `ADMITTED`. The stock-architecture path independently needs synchronized FRC Operation FFD `5282/5285/57DE/5265` plus source-capable evidence for the protected-request owner. Do not send `0x08A` to EPS.
