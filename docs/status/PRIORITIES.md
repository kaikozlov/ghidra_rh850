...
**Current development-path update (VAR-117/CORR-157):** stop adding persistent SecOC-result patches. Stages 3–5 were each applied, reboot-verified, and remained non-delivering at direct PDU44 COM. A raw/function-boundary-independent exact-F33 walk now closes the configured receive path from RSCFD rule39 and CanIf descriptor39 through PduR44, SecOC profile2 queueing at `FEBE547A/FEBE54D4`, freshness/command-7 processing, native-success `8F546`, recovered route44 callback `7D72C`, COM window `FEBE4BFF`, new-data counter `FEBE5364`, and unpacker `4BD46 -> 80BC/80B8`. No additional downstream result/status gate is justified. The **single immediate discriminator** is the RAM-only queue observer/native-route44 bridge: heartbeat `FEBFFBEC`, queue-present `FEBFFBF0`, zero-MAC-seen `FEBFFBF4`, injected `FEBFFBF8`. Install it once in NRTD, heartbeat-attest the returned application, then transition directly to READY without a full power cycle and run the stationary admission probe with `--require-bridge`; no nonzero offset before `ADMITTED`. `queue_present==0` moves the problem to physical/on-wire ingress; a positive queue/injection count tests the exact stock route44 handoff without another CodeFlash patch. OQ-054 stock proxy ownership remains separate. Canonical: [../variants/camry-2026-live-baseline.md](../variants/camry-2026-live-baseline.md) §§40,57.6.
