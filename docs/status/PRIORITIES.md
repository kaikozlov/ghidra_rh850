...
1. **Recover Toyota's stock request/signing and source-suppression architecture.** VAR-153/CORR-186 close the EPS-side software question: F33 has one configured B6 receive path, copies each successfully delivered 32-byte profile2 PDU unchanged into route44, and has no recovered B6 constructor, sanitizer, replacement path, or autonomous route44 ticker. The retained zero-host-TX control therefore contains independent successful profile2 deliveries; it does not show Panda ID11 being rewritten to ID0. The Sep-10 two-stage ingress observer additionally saw D7 and native B6 at the deterministic post-CanIf/pre-SecOC boundary while seeing none of 121 Panda-returned ID63 markers. For that exact treatment, neither EPS SecOC rejection nor EPS post-ingress rewriting explains the miss.
   ...
   OQ-054 now owns the native production blocker. `0x08A` is the upstream request and `0x081` the selected/result feedback; the leading hypothesis is that a Brake/Skid/CGW-domain arbitrator separately constructs authenticated B6 on a non-observed path to F33. Prove or disprove the final `0x08A -> private B6` edge with a synchronized stock-LTA run that reads the internal native B6 ID/target fields, then reproduce the `0x08A` SecOC domain with EPS ICU-S slot 4 before attempting a bounded request injection. Do not send `0x08A` to EPS itself.
...
`0x08A -> B6` stock transform. VAR-091/107 close the **observed native Bus-1** FRC plane as
request-side E2E rather than the Bus-4 SecOC publication. CORR-194 supersedes CORR-149's
stronger key-holder exclusion: FRC-family ECU-Security-Key provisioning keeps private camera
pre-authentication open, while the downstream Brake/Skid versus CGW **physical publisher** is
still unresolved. Native Bus 1's 22 camera/radar streams contain perception objects but neither
consecutive `5282` nor the `0x08A` application. That stock-pipeline attribution is not the first
B6 development blocker.
