...
a TSK key holder, but do not choose the downstream Brake/Skid versus CGW proxy signer. Native
Bus 1's 22 camera/radar streams contain perception objects but neither consecutive `5282` nor
the `0x08A` application. That stock-pipeline attribution is not the first B6 development blocker.
...
B28..B31 strongly match Toyota ordinary-P5 `FV4 || MAC28`; the downstream physical transmitter/proxy signer, request handoff/encoding, signer key/profile selection, and actual grant remain open. VAR-113/CORR-153 explicitly leave transformed, multi-field, multiplexed, event-driven, and private-link alternatives open rather than assuming the handoff is private. CORR-135 rejects the old upstream-to-B6 inference; CORR-136 preserves the timing/source-attribution correction, while CORR-149 supersedes its former FRC-signer caveat.

**VAR-081/083/087/091/092/093/094/101/107 steering-command boundary — current highest priority:** keep three questions separate: request publication, grant/actuation, and any candidate openpilot ingress. `0x08A` B21=Target Lateral ID, B18:B19=signed target angle at the numerically matching `1024/17870` scale, B26 is modulo-64 behavior, and B28..B31 strongly match Toyota ordinary-P5 `FV4 || MAC28`. VAR-091/CORR-149 place it on captured Bus 4, exclude exact F33 Tx/Rx, and exclude the FRC from the TSK key-holder/signing role: VAR-107 closes native Bus-1 traffic as exact AUTOSAR E2E Profile-5 framing—CRC-16/CCITT `0x1021`, B2 alive counter, implicit DataID=CAN ID—with no secret-bearing authenticator, while Toyota's recovered TSK implementation keeps AES-CMAC keys in protected ICU-S storage on TSK-capable chassis participants. The remaining signer question is which downstream Brake/Skid/CGW proxy receives the FRC request and publishes the authenticated PDU. Rlog timestamps batch a median 14 bus-0 frames, invalidating the former 20/30 ms/`0x0D7`-queue attribution. VAR-093 reads the Bus-1 family as plaintext object slots; VAR-094 closes only consecutive `5282` on native Bus 1 and the inbound SAS-echo direction. VAR-092 closes default-bank `D0218` as not an F33 COM milliradian.
...
Static priority is OQ-054 after CORR-149: obtain Brake/CGW/Skid producer firmware and trace the FRC request handoff into downstream SecOC generation. Rlog cadence cannot attribute a transmitter. VAR-091/101/107 close the FRC as request-side, non-TSK signer; VAR-092/094 preserve default-bank `D0218` and consecutive-`5282`-on-Bus-1 closures; VAR-085 closes E1/E2. Candidate downstream producer firmware is useful for both physical-TX and proxy-signer dataflow, not as a presupposed `0x08A -> B6` transformer.
