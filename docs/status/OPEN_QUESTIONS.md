...
FINDINGS/CORRECTIONS bookkeeping transaction. Use `tools/know QUERY` when prior
ledger context is useful.
...
- **OQ-052 — True-TSS3 longitudinal wire/auth/arbitration execution contract.**
  **September-16 Vehicle-Movement-Manager architecture closure:** Toyota patent
  US20200070849A1 changes the scope of this question. Longitudinal lower/upper
  requests are application-identified acceleration **bounds** that are arbitrated
  independently; separate request-generation units create controller targets; the
  powertrain then compares/clips driver demand against those bounds and reports the
  source actually employed. GTS mirrors both sides of that split: Brake `0x10A1..A4`
  are TSS upper/lower request acceleration/IDs, while `0x10A5..0x10AA` are upper/lower
  **Vehicle Motion Control** target acceleration/IDs/driving force. FFD `5280/5281`
  are the request packages and `5284/57DB/57D3` the result package. Current `0x08A`
...
- **OQ-054 — Recover the exact FRC feature-selector/secured-egress implementation and the downstream Brake request-to-target handoff.** Toyota US20200070849A1 separates application request, result feedback, post-request target generation, and priority Vehicle Movement Control. Exact-Camry evidence now fixes the first placement more tightly than the generic patent diagram: **LTA/LDA/LCA/PDA/PCS are simultaneously enableable FRC-resident applications, and the FRC application state machine selects the current owner before generic `5282` / protected `0x08A` egress.** The repin experiment closes the external direction: protected `0x08A` is native on the **FRC/camera-side endpoint** of the intercepted Toyota Bus-4 pair and moves toward Brake; `0x081` is native on the **Brake/chassis-side endpoint** and returns toward FRC. FRC normal-Tx suppression removes `0x08A`; Brake normal-Tx suppression removes `0x081`, and Brake keeps publishing `0x081` with request-loss supervision when FRC request traffic disappears. Thus the supported whole loop is **FRC internal feature-owner selection -> signed/protected `0x08A` request -> downstream Brake/VMM request arbitration -> `0x081` arbitration-result/status back to FRC + post-arbitration request generation -> B6 final target toward EPS**. The FRC's selection only chooses which internal feature populates the request; it does not replace the downstream VMM arbiter. Exact F33 maps B6 loss to Brake System Control Module and internally receives native B6/profile2, but never receives `0x08A` or `0x081`.
  ...
  The direct-Panda Sep-10 marker is now localized more tightly. All 121 ID63 B6 frames were returned by Panda, but none reached F33's post-ring/pre-SecOC observer while native B6 and D7 positive controls continued. Exact F33 rule39 is ordinary `GAFLID=0x0B6`, mask selector0 -> `0xC00007FF`, receive-FIFO destination2; neighboring `0x090/0x0D7` use the same mask/destination. CanIf descriptor39 is only `FD | 0x0B6`, DLC32, and PDU44's legacy checksum hook is non-enforcing. No recovered pre-observer logic can inspect Target Lateral ID, SecOC tag or transmitter identity. Therefore the host frame is lost **before successful F33 controller1 decode/CanIf admission**; EPS SecOC/PduR/COM/application rejection is closed. Physical/link decode before GAFL remains the narrow receiver-side residue, otherwise the filter/routing point is external to F33. The EPS-resident signer later steered by modifying an already-admitted native B6, proving the final instruction is sufficient after admission.

  GTS gives a concrete topology clue and CORR-197 sharpens its interpretation. Across all 18 exact-Camry options, Brake Booster `0x28` and Skid Control `0x29` are logical Bus 4 via `No. 2 Global CAN Junction Connector`, while EPS `0x32` is Bus 4 via the literal **`EBU` junction/attachment**. `EBU` is not an installed ECU component in the exact Camry component set. Toyota-authored patent vocabulary expands EBU as **electronic brake module or unit**; current GTS itself leaves the acronym unexpanded. Brake-family DDBs separately expose `(EBU node)` dynamics. Therefore do **not** model `No.2 junction -> separate EBU ECU -> EPS`. The leading implementation is an upstream Brake/Skid VMM request-generation/routing boundary, probably within the skid-control/brake-actuator assembly, feeding EPS over its single EBU-labelled local CAN attachment. Contemporary Toyota repair procedures requiring ECU-Security-Key update for a replaced skid-control ECU/brake-actuator assembly reinforce that security/request-generation ownership belongs in that serviceable brake domain; they do not by themselves identify the exact Camry B6 CMAC routine.

  Exact open items now split cleanly by side: **FRC** — recover the feature-local eligibility/priority/owner state machine that populates `5282`, identify which internal SoC/HSM/network-security controller owns the `0x08A` SecOC key, freshness state, CMAC service and Tx descriptor, and determine whether there is a controllable pre-sign feature-selected request object; **Brake `F152633K0000`** — recover `0x08A` verification, the downstream VMM request-arbitration implementation, `0x081` arbitration-result production, post-arbitration request generation, and B6 construction/signing/routing; **Brake->EPS physical path** — prove the concrete `ch2`/EBU-labelled channel/filter and close the small residual link-format branch. The physical `0x08A` publisher and FRC-internal LTA-vs-LDA-vs-PDA feature-owner selection are no longer open Brake/CGW questions. Canonical architecture: [../architecture/toyota-tss3-vehicle-movement-arbitration.md](../architecture/toyota-tss3-vehicle-movement-arbitration.md); machine evidence: `data/generated/camry_2026_08a_producer_bounds.json`, `data/generated/camry_2026_08a_signer_continuity.json`, and `data/generated/camry_2026_ebu_topology.json`.

  **Historical OQ-054 evidence follows; its downstream-`0x08A`-publisher model is superseded by the later relay-open/FRC-CommunicationControl source result.** VAR-091/CORR-149 established the older placement boundary: `0x08A` was observed on captured Bus 4 with ordinary-P5 `FV4||MAC28`, absent from Bus 1, and excluded from exact F33 Tx/Rx. GTS+ puts logical FRC on Bus 1 and Skid/Brake Booster/EPS/SAS/Airbag on Bus 4 behind Central Gateway, but that topology is not a CAN-ID source map. Retained rlog timestamps are multi-frame publication timestamps, so the former 20/30 ms arbitration/`0x0D7`-queue attribution is invalid. VAR-107 closes the observed native Bus-1 family as exact AUTOSAR E2E Profile 5. CORR-194 then established camera-family key provisioning. Those facts originally motivated a downstream proxy hypothesis; **do not carry that hypothesis forward**. Later source isolation proves the secured publisher is inside the FRC assembly. VAR-094's useful surviving result is only that consecutive recorder layout `ID||pinion||assist` is absent from native Bus-1 CAN; the selected request is packaged internally before chassis-facing `0x08A` egress. Independently capture FRC Operation FFD feature-local state plus `5282`, then `5285/57DE/5265/560D` with `0x081` to separate the FRC owner transition from downstream Brake result/plant state. Do not send `0x08A` to EPS or infer a byte-preserving `0x08A -> B6` transform. Protected B6 remains a separate candidate openpilot interface.
  Request/grant grading is VAR-095/CORR-137.
  **Historical VAR-096 inference, superseded:** no separate arbitration/request ECU co-installs with FRC_P5 498; the old topology-only analysis therefore bounded the `0x08A` transmitter/signer candidate set to Brake/Booster/CGW. Later physical source evidence removes that candidate set and places the secured publisher inside FRC.
  **Historical VAR-097 ambiguity, superseded for lateral feature ownership:** the FRC recorder separated feature requests (`5531/5631/5Axx`), generic request `5282`, result `5285/57DE`, and external control/plant observations `5265/560D`, but at that stage FRC-vs-Brake selection placement was unresolved. Current evidence closes the relevant split: FRC selects the LTA/LDA/LCA/PDA/PCS owner before `5282`/`0x08A`; Brake supplies downstream request arbitration/result/request generation. Synchronized Operation FFD is now for recovering the **FRC internal selector**, not deciding whether Brake chooses among those features. Canonical §45.
  VAR-101 refines the question only at the **publication-continuity** level: the
  secured `0x08A` family continues at zero request (stationary READY, `B21=0` in
  2,475/2,475 frames, live FV4 epoch tracking, `+1 mod 64` B26, frame-unique
  MAC28). This proves that the authenticated publication/security pipeline is not
  request-gated; it does **not** locate the CMAC engine or key holder. At that
  historical stage CORR-194 reopened FRC private pre-authentication because
  camera-family ECU-Security-Key provisioning was positive evidence; the then-live
  Brake/CGW physical-publisher candidate set is superseded by the later relay-open
  source result, which places `0x08A` publication inside the FRC assembly. Canonical §47.
  ...
  At the VAR-110 stage the remaining lateral problem was framed as identifying the
  chassis producer/arbitrator of `0x08A`/`0x081` plus the **final chassis-reference
  to local EPS authority/actuator handoff**. The producer half is now superseded:
  `0x08A` is FRC-owned egress and `0x081` is Brake-owned arbitration-result/status;
  the exact Brake/VMM arbitration/request-generation implementation and final EPS
  handoff remain open. VAR-111 now exhausts the most plausible
