...
- **OQ-054 — Recover the exact FRC feature-selector/secured-egress implementation and the downstream Brake request-to-target handoff.** Toyota US20200070849A1 separates application request, result feedback, post-request target generation, and priority Vehicle Movement Control. Exact-Camry evidence now fixes the first placement more tightly than the generic patent diagram: **LTA/LDA/LCA/PDA/PCS are simultaneously enableable FRC-resident applications, and the FRC application state machine selects the current owner before generic `5282` / protected `0x08A` egress.** The repin experiment closes the external direction: protected `0x08A` is native on the **FRC/camera-side endpoint** of the intercepted Toyota Bus-4 pair and moves toward Brake; `0x081` is native on the **Brake/chassis-side endpoint** and returns toward FRC. FRC normal-Tx suppression removes `0x08A`; Brake normal-Tx suppression removes `0x081`, and Brake keeps publishing `0x081` with request-loss supervision when FRC request traffic disappears. Thus the supported whole loop is **FRC internal feature selection -> signed/protected `0x08A` -> Brake/VMM -> `0x081` result back to FRC + B6 final target toward EPS**. Exact F33 maps B6 loss to Brake System Control Module and internally receives native B6/profile2, but never receives `0x08A` or `0x081`.
  ...
  Exact open items now split cleanly by side: **FRC** — recover the feature-local eligibility/priority/owner state machine that populates `5282`, identify which internal SoC/HSM/network-security controller owns the `0x08A` SecOC key, freshness state, CMAC service and Tx descriptor, and determine whether there is a controllable pre-sign selected-request object; **Brake `F152633K0000`** — recover `0x08A` verification, downstream vehicle-motion/result/request generation, `0x081` result production and B6 construction/signing/routing; **Brake->EPS physical path** — prove the concrete `ch2`/EBU-labelled channel/filter and close the small residual link-format branch. The physical `0x08A` publisher and LTA-vs-LDA-vs-PDA selection are no longer open Brake/CGW questions. Canonical architecture: [../architecture/toyota-tss3-vehicle-movement-arbitration.md](../architecture/toyota-tss3-vehicle-movement-arbitration.md); machine evidence: `data/generated/camry_2026_08a_producer_bounds.json`, `data/generated/camry_2026_08a_signer_continuity.json`, and `data/generated/camry_2026_ebu_topology.json`.
  ...
  **Historical VAR-097 ambiguity, superseded for lateral feature ownership:** the FRC recorder separated feature requests (`5531/5631/5Axx`), generic request `5282`, result `5285/57DE`, and external control/plant observations `5265/560D`, but at that stage FRC-vs-Brake selection placement was unresolved. Current evidence closes the relevant split: FRC selects the LTA/LDA/LCA/PDA/PCS owner before `5282`/`0x08A`; Brake supplies downstream result/request generation. Synchronized Operation FFD is now for recovering the **FRC internal selector**, not deciding whether Brake chooses among those features. Canonical §45.
  ...
  request-gated; it does **not** locate the CMAC engine or key holder. CORR-194
  makes FRC private pre-authentication a live branch again because camera-family
  ECU-Security-Key provisioning is positive evidence. Physical Bus-4 publication
  remains bounded to brake-family/Central-Gateway candidates. Canonical §47.
