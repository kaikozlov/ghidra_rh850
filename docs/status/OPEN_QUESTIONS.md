...
- **OQ-054 — Recover the exact Camry Vehicle-Movement-Manager physical handoff, source admission/suppression, and SecOC ownership.** **The logical architecture is now closed much more strongly than this question's older wording.** Toyota US20200070849A1 separates application request, arbitration-result feedback, post-arbitration request generation, and a priority Vehicle Movement Controller; it places that vehicle-movement-manager function in the Brake ECU in the disclosed embodiment. GTS independently provides the same three names: FFD `5282` **TSS request - lateral ID/pinion**, FFD `5285/57DE` **arbitration result lateral ID/pinion**, and EMPS `0x1CEE` **Target Lateral ID / Target Steering Angle After Output Compensation**. Exact F33 then maps B6/PDU44 loss to U012987 **Lost Communication with Brake System Control Module**, and later internal captures prove native B6/profile2 deliveries exist. The leading logical chain is therefore request-side `0x08A` -> Brake/VMM arbitration/request generation -> result/status `0x081` + final steering target B6 -> EPS. This supersedes the older “do not infer any `0x08A -> B6` relation” language; what remains unproved is the exact F33 physical implementation of that relation.
  ...
  Exact open items: acquire/decode category-435 Brake `F152633K0000` and identify the concrete B6 Tx/bridge/filter/signing path; determine whether `EBU` names an internal brake-unit sub-node or only the harness attachment; close the residual physical-link-format branch if needed; determine which ECU/core executes each VMM sub-block; whether `0x08A` is raw per-application input or already aggregated TSS request; `0x08A`/`0x081`/B6 SecOC signer/key/profile ownership; exact field transformation/cadence; and the clean stock-source suppression/replacement point. Canonical architecture: [../architecture/toyota-tss3-vehicle-movement-arbitration.md](../architecture/toyota-tss3-vehicle-movement-arbitration.md); machine evidence: `data/generated/camry_2026_ebu_topology.json`.

  **Historical OQ-054 evidence follows:** VAR-091/CORR-149 preserve the hard placement boundary: `0x08A` is observed on captured Bus 4 with ordinary-P5 `FV4||MAC28`, absent from Bus 1, and excluded from exact F33 Tx/Rx. GTS+ puts FRC on Bus 1 and Skid/Brake Booster/EPS/SAS/Airbag on Bus 4 behind Central Gateway, but that topology is not a CAN-ID source map. Retained rlog timestamps are multi-frame publication timestamps, so the former 20/30 ms arbitration/`0x0D7`-queue attribution is invalid. VAR-107 closes the observed native Bus-1 family as exact AUTOSAR E2E Profile 5—CRC-16/CCITT `0x1021`, B2 alive counter, implicit DataID=CAN ID—not TSK/SecOC. **CORR-194 corrects the former hardware overreach:** current FRC diagnostics/service procedure prove camera-family ECU-Security-Key provisioning, so native Bus-1 E2E framing does not exclude a private FRC pre-authentication step or other key use. A downstream Bus-4 participant must still proxy/physically publish the chassis PDU, but cryptographic CMAC generation/key selection may be upstream/private or downstream until joined by firmware/trace. VAR-094 proves only that consecutive recorder layout `ID||pinion||assist` is absent from native Bus-1 CAN; VAR-113 further bounds direct single-field linear/monotonic carriers within declared sweeps, but transformed, multi-field, nonlinear, multiplexed, event-driven, or private-link handoff encodings remain open. Remaining candidates for **physical proxy/publication** are Central Gateway, Skid Control, or Brake Booster; FRC must remain in the separate **cryptographic ownership/private-preauth** candidate set. Close this with exact candidate firmware, a source-identifying physical capture/isolation experiment, synchronized source-side diagnostics plus all-bus CAN, or a live FRC key-update/private-link trace—not batched rlog cadence. Independently capture FRC Operation FFD `5282/5631/5285/57DE/5265/560D` to distinguish request, arbitration result, grant, and EPS feedback. Do not send `0x08A` to EPS or infer an `0x08A -> B6` stock-LTA transform. Protected B6 remains a separate candidate openpilot interface. Canonical: [../variants/camry-2026-live-baseline.md](../variants/camry-2026-live-baseline.md) §§20,30,38,41–43,47,51; VAR-081/087/091/092/094/101/107/113; CORR-135/136/149/153/194.
