...
- **OQ-054 — Recover the exact Camry Vehicle-Movement-Manager physical handoff, source admission/suppression, and SecOC ownership.** Toyota US20200070849A1 separates application request, arbitration-result feedback, post-arbitration request generation, and a priority Vehicle Movement Controller. Current GTS independently supplies the same three semantic layers. The repin experiment now closes the request/result direction more strongly than the older GTS-only topology model: protected `0x08A` is native on the **FRC/camera-side endpoint** of the intercepted Toyota Bus-4 pair and moves toward Brake; `0x081` is native on the **Brake/chassis-side endpoint** and returns toward FRC. FRC normal-Tx suppression removes `0x08A`; Brake normal-Tx suppression removes `0x081`, and Brake keeps publishing `0x081` with request-loss supervision when FRC request traffic disappears. Thus the supported whole loop is **FRC assembly -> signed/protected `0x08A` -> Brake/VMM -> `0x081` result back to FRC + B6 final target toward EPS**. Exact F33 maps B6 loss to Brake System Control Module and internally receives native B6/profile2, but never receives `0x08A` or `0x081`.
  ...
  Exact open items now split cleanly by side: **FRC** — identify which internal SoC/HSM/network-security controller owns the `0x08A` SecOC key, freshness state, CMAC service and Tx descriptor, and whether there is any controllable pre-sign request object; **Brake `F152633K0000`** — recover `0x08A` verification, application arbitration/request generation, `0x081` result production and B6 construction/signing/routing; **Brake->EPS physical path** — prove the concrete `ch2`/EBU-labelled channel/filter and close the small residual link-format branch. The physical `0x08A` publisher is no longer an open Brake/CGW proxy question. Canonical architecture: [../architecture/toyota-tss3-vehicle-movement-arbitration.md](../architecture/toyota-tss3-vehicle-movement-arbitration.md); machine evidence: `data/generated/camry_2026_08a_producer_bounds.json`, `data/generated/camry_2026_08a_signer_continuity.json`, and `data/generated/camry_2026_ebu_topology.json`.
