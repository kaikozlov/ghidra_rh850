...
- **Stock-ACC claim:** the selected September-10 working steering intervals carry conventional 0x88/0x90 cruise, not adaptive 0xA0/0xC0. Physical lateral authority remains demonstrated, but that evidence does not close the final lateral-plus-adaptive-cruise combination. Recovery now requires the GTS distance-control mode 1/2/4, permission, and a fully present clear ACC-unavailable flag; truncated replies and conventional mode 3 cannot count as success. Pre-clear evidence is saved before mutation and retained on failure.
...
- **Current loop:** `FRC assembly -> protected 0x08A -> Brake/VMM -> 0x081 back toward FRC + B6 toward EPS`. Exact F33 receives neither `0x08A` nor `0x081`; its native B6 loss monitor names Brake System Control Module.
