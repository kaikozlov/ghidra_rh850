...
- **Correct open questions:** (1) identify the `0x08A` producer and its exact SecOC/security/arbitration ownership; independently, (2) identify which exact external/local state selects or modulates F33's B6-independent `D0218/CC60/CC50` path during stock LTA/LCA. Only after those are understood should protected B6 be evaluated as a separate candidate openpilot actuation interface, with its own signing/freshness/suppression contract.
...
- **Authentication correction:** zero Bus-1 `0x00F` plus near-constant last-4 on all periodic Bus-1 streams proves only that those observed PDUs do not end in ordinary-P5 `FV4||MAC28`. It does not prove which MCU computes the Bus-4 `0x08A` CMAC, that FRC lacks ICU-S/another CMAC primitive, or that a private FRC→gateway/brake transport cannot carry a pre-authenticated PDU image. Physical transmission and SecOC computation are separate unknowns.
...
- **Canonical:** VAR-091/094; `data/generated/camry_2026_08a_producer_bounds.json` schema v3; `tests/verify_camry_2026_08a_producer_bounds.py`; [../variants/camry-2026-live-baseline.md](../variants/camry-2026-live-baseline.md) §§41–43.
...
- **Preserved facts:** bus 1 demonstrably carries feedback/plant-shaped data rather than commands — the exhaustive zero-leads negative (VAR-082) and the Bus-1 request-absence negative (VAR-094) stand unchanged; `0x160` remains a ~40 Hz camera/radar-domain stream.
...
- **2026-08-30 live consequence:** a validate-only Gate-2 preflight matched exact application F181 and passed boot SecurityAccess/`10F0`, but the wrongly selected DID value led to zero payload telemetry; the deployer timed out with `apply_ready=false` and performed **no flash write**. The failed run is retained under `targets/camry-2026/raw-20260830/secoc-patch-preflight-oldstack-0203-regression/`.
...
- **2026-08-30 live consequence:** even after DID0203, VMA, asynchronous handoff, exact boot F181, and pre-FF00 RX-clear corrections, the bounded validate-only preflight still produced zero telemetry and remained `apply_ready=false`; no flash write occurred. A subsequent live retry with the unverified helper calls removed and the callback `0x06` Tx handshake corrected also produced zero telemetry. Disassembly of that exact payload then exposed one remaining pre-witness mutation: `runtime_init()` still zeroed community scratch words `FEBF1F00/FEBF1F04`, which lie outside the authenticated 4-KiB window and are not proven free on F33. The runtime now performs no bootloader-RAM writes before its first CAN telemetry witness.
...
### CORR-144 — the shared F33 RAM-exec handoff was still an approximation; the retained Aug-26 helper is the proven execution path

- **Superseded implementation:** after CORR-142 the shared runner described its custom `_enter_programming_bootloader()` as the “F33-proven asynchronous handoff.” It reproduced the broad DEFAULT→EXTENDED→PROGRAMMING idea but did not actually use the retained helper that produced the successful Aug-26 acquisition.
- **Decisive control:** a three-byte loop-bound derivative of the exact retained Aug-26 Calvin payload failed with zero frames through the shared runner, then succeeded immediately through the original Aug-26 host script on the same current post-repin bus 0. It returned address 0 / CodeFlash word `0x06E0001F` in 6 ms. Thus the payload, `0x7A9` callback sender, Panda bus0/2 visibility, relay repin, 500/2000 CAN-FD state, opendbc ISO-TP/UDS, and Panda version are not the zero-telemetry cause.
- **Exact host distinction:** the working path calls `tsk.lib.programming.enter_programming_bootloader`, whose preserved-route rediscovery re-enters DEFAULT and confirms F181, then performs the retained boot F181 read, EXTENDED + read-only SID23 probe, and one fresh DEFAULT→EXTENDED→PROGRAMMING ladder before SecurityAccess. The shared duplicate path is removed from live use; it now delegates to that field-proven helper and records the handoff/probe result.
- **Permanent rule:** when a target has a retained successful programming/bootstrap helper, do not reimplement its stateful session/rediscovery choreography and label the approximation “proven.” Reuse the exact helper or prove equivalence dynamically.
- **Canonical:** SECOC-078; `exploit/common/ram_exec.py`; `tests/verify_secoc_manifest_patcher.py`; `tests/verify_ram_exec_variant_requirements.py`; `targets/camry-2026/raw-20260830/secoc-ram-exec-exact-host-control/`; [../variants/camry-2026-live-baseline.md](../variants/camry-2026-live-baseline.md) §14.
