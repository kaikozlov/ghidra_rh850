...
- **Consequence:** the downstream actuation question is substantially closed. The unresolved steering contradiction is now **upstream of `CC50/CC62`**: factory LTA physically steered during 73.303384 s of strongly identified LTA/LCA-active operation with zero B6 on the actual Brake/EPS network, VAR-082 finds no second ordinary CAN steering carrier, and VAR-084 finds no concrete alternate producer across the major hidden-ingress blind spots. Do not dismiss that observation as generic assist; identify the state/value that gives the shared funnel stock-LTA authority or falsify the remaining computed-store/runtime-DMA residuals.
