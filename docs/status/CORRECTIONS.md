...
- **Consequence:** the downstream actuation question is substantially closed. At this correction stage the contradiction remained upstream of `CC50/CC62`; VAR-085 subsequently closed the computed-store/runtime-DMA residuals, and CORR-134/VAR-081 then recovered Bus-4 `0x08A` as the upstream request representation while exact F33 still excludes it from ingress. The current unresolved boundary is therefore the `0x08A` producer/authentication/transformation into protected B6 plus signer/freshness ownership and authority/arbitration, not another hidden-F33 producer search.
...
- **Right interpretation:** waiting for a stock B6 template/cadence is no longer a valid Camry steering-integration prerequisite. The runtime sender was subsequently removed (`opendbc@b9e86924`, `kai-openpilot@abf3ca70a`) rather than weakening its disproved admission gate; low-level B6 construction/freshness/debug-safety helpers remain analysis/test-only. CORR-134/VAR-081 now identify the current missing contract as the observed Bus-4 `0x08A` producer, integrity/authentication, producer-side transformation into protected B6, signer/freshness ownership, and authority/arbitration. VAR-085 separately closes VAR-084's two computed/DMAC residuals.
- **Consequence:** do not repeat blind stock-B6 drives, fabricate a stock template, or revive the removed runtime sender. Preserve only the historical/test scaffolding until the producer/protection/arbitration architecture is recovered.
...
- **Preserved boundary:** exact F33 still does not accept `0x08A`; it accepts protected
  ...
  be sent to EPS. Every retained `0x08A` is on the Bus-4 capture and its producer remains
  unknown; the producer-side path into protected B6, integrity/authentication, signer,
  freshness, and arbitration remain unrecovered. Steering output remains unauthorized.
- **Canonical:** VAR-081;
