...
- **Evidence boundary:** VAR-145's source-off and Panda framing observations remain valid, including the conclusion that active stock LTA was not required for the observed non-response. Route 48 does **not** test the corrected dummy-CMAC sender, so it cannot close receiver behavior for the restored normal envelope. VAR-146/CORR-177 subsequently close the first-in-epoch freshness-phase concern from exact F33 plus the complete corpus; a new controlled capture is still required to test the corrected dummy-CMAC envelope and downstream queue/raw-COM/application authority.
...
- **Current implementation consequence:** opendbc `f207c273` resets the full B6-local message counter to 0 on `RESET_CNT` change and increments it per send. This is correct because F33 seeds the same full value 0 from low2=0 and then reconstructs the same +1 progression; it also matches current upstream Toyota SecOC counter ownership. No `0 -> 1` code change is warranted. The current dummy-CMAC envelope still needs a road test for receiver/application authority, but freshness **phase** is not a remaining discriminator.
