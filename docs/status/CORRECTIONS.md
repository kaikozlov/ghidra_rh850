...
- **Envelope consequence:** the recovered Toyota `MAC_01` host workflow is master/slave and endpoint-generic: each selected ECU contributes its 16-byte `SafekeyNumber`, receives a server-matched `M1[16] || M2[32] || M3[16]` package, and returns/polls `M4[32] || M5[16]`. Current GTS+ deliberately retains both RoutineControl transports, RID `0x3002` and RID `0x1010`, for that logical exchange-key family. The yc Venza SRS firmware proves the same RID-`0x1010` M1--M5 wire envelope can terminate in a local secure-service ABI different from the tracked EPS ICU-S path. Therefore non-Renesas FRC silicon is not evidence for a different Toyota envelope; another HSM/security engine or equivalent protected backend is sufficient.
