...
- **Gate-2 correction:** exact `FUN_0008F906` invokes the SecOC/freshness callback before the patched predicate at `0x8F952`. On real command-7 failure the callback receives the failure-marked freshness ID and `90D6A` restores pending B6 freshness from committed state; the installed patch then forces the stock verified/delivery arm. The patch therefore means **deliver despite failed authentication**, not "authentication succeeded everywhere." Conversely, normal delivered-PDU receive indication can clear the COM reception state, so the static code also does not justify claiming every zero-MAC28 B6 remains application-unhealthy.
