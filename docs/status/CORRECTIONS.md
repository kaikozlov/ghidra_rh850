...
- **Consequence:** the VAR-077/CORR-127 external-ingress conclusion is preserved and sharpened: B6 remains the only recovered generated-COM steering target/mode ingress, while F33 can generate nonzero command torque internally with B6 absent. The retained Class-L motor/current shift therefore cannot be used by itself to infer a hidden external planner or OEM LTA authority. Exact attribution among the remaining internal terms is still bounded.
