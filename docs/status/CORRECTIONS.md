...
### CORR-146 — relay-closed TSS3 was only a workaround; forwarded classic CAN was being promoted to CAN-FD, and `0x08A` state belongs on native bus 2
...
- **Forwarding root cause:** failing relay-open rlogs show real returned forwarding transmissions in both directions, byte-identical payloads, and zero bus-off/overflow/error counters. Panda's FDCAN TX path nevertheless used `canfd_auto` for forwarded packets: after seeing CAN-FD on the mixed Toyota bus, it retransmitted classic <=8-byte RX traffic as CAN-FD. Forwarded packets now preserve the exact received CAN/CAN-FD and BRS format (`panda@dad7ae23`), and the dev mode is back to relay-open software forwarding (`panda@99b6f09c`, `opendbc@11a0c049`, parent `9af322518`). The operator reports no recurring system/FRC malfunction after READY operation plus an additional comma restart; live CAN health showed zero bus-off/errors/TX loss/RX loss.
...
- **Permanent integration rule:** parse Camry `0x08A` from `Bus.cam` / Panda bus 2 and run the dev cruise + stock-lateral exclusivity hook on bus 2; keep PT/EPS/sync inputs and B6 TX on bus 0. `opendbc@7a0f9fd5` implements both layers; route replay recognizes cruise with LTA disabled and the focused Camry TSS3 suite is 31/31. Do not reintroduce the relay-closed workaround or a bus-0 `0x08A` RX dependency.
...
- **Remaining non-native bring-up guard:** Panda still rejects active B6 when native Toyota Target Lateral ID is nonzero. That is a temporary fail-closed coexistence guard, not the desired final architecture. Final integration must suppress the stock lateral source through the relay/forwarding layer, as normal comma ports do, then remove the coexistence guard. Do not replace that work with user-procedure requirements or controller-side arbitration hacks.
- **Implementation:** `opendbc` follow-up after `ae105331`; focused Toyota/TSS3 interface/controller tests cover the standstill capability and `CC.latActive` ownership.
