...
- **Composed correction:** later evidence closes that branch. Toyota's recovered TSK implementation on the participating RH850 chassis ECUs stores/selects AES-CMAC keys through protected Renesas ICU-S slots; the FRC request domain is not an ICU-S-equipped TSK key-holder/signing participant. VAR-107 independently recovers the native Bus-1 family as an 8-bit alive counter plus reproducible affine-linear E2E integrity, with no SecOC/TSK authenticator. The car nevertheless continuously publishes valid authenticated Bus-4 control-plane PDUs, including zero-request `0x08A` (VAR-101). Therefore an FRC semantic request must cross into a downstream TSK-capable proxy that arbitrates/repacks as needed and produces the authenticated Bus-4 publication.
- **What remains open:** the proxy's exact identity and handoff remain unresolved. Current topology bounds the candidate set to Skid Control / ABS, Brake Booster, or Central Gateway. Physical Bus-4 transmitter, private request encoding, final arbitration executor, SecOC profile/freshness owner, and exact ICU-S key selection still require downstream producer/private-link evidence.
