...
- **Sender correction:** exact application-field fate recovery removes the remaining generic "secondary-field template" excuse for the current active candidate. Opendbc `f207c273b645` sends every recovered command-relevant companion in the required direction: sig263=0, sig265=0, sig268 +1 modulo64, sig269/270=100, with currently unconsumed companions zero; its nominal 50-Hz cadence is inside the exact 35-ms receive-loss bound. VAR-146/147 already close freshness phase and zero-vs-dummy wrong-MAC as road discriminators. This does not claim the slot-4 key is known or the dummy MAC is stock-valid.
