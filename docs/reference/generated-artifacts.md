...
| `data/generated/decompilations.jsonl` | Whole-image pseudocode for all 6,037 functions plus canonical non-flow instruction/data references for alias-resistant RAM lookup | `tools/generate_decompiler_corpus.py` (`make generate-decompiler-corpus PROJECT_DIR=…`) | `tests/verify_decompiler_corpus.py` |
| `data/generated/secoc_gate_resolution_4512000.json` | Strong semantic Gate-2 resolver result from the annotated Sienna working project, including program SHA, mapped MAC-result provenance, synthesized local branch patch, and success/failure convergence | `ResolveSecocAcceptanceGate.java` via `tools/g` | `tests/verify_secoc_semantic_patch_resolver.py` |
