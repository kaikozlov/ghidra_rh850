...
| `data/generated/corolla_8965H1202000_application_diagnostic_decompiler_evidence.json` | Compact image-bound H-native decompiler evidence for all 180 configured RDBI producers, all 35 nonzero RoutineControl callbacks, and 25 helper/downstream functions used by the output-bound and lifecycle audit; every record carries raw-body and decompiled-C SHA-256 | `tools/extract_variant_application_diagnostic_evidence.py` from a disposable H Ghidra corpus | downstream comparison validates every raw-body/decompiler hash before using the evidence |
| `data/generated/corolla_8965H1202000_application_diagnostics_diff.json` | Deterministic Sienna↔H application diagnostic comparison: 17-SID service shape, 242→226 DID generation, F181 17→33, exhaustive 180-producer H RDBI write audit, and all 19 RoutineControl policy/width rows plus material callback semantic deltas | `tools/compare_variant_application_diagnostics.py` | `tests/verify_corolla_8965H1202000_application_diagnostics.py` regenerates it exactly |
| `data/generated/corolla_8965H1202000_fd_control_decompiler_evidence.json` | Compact H-native decompilation evidence for the B6/025/030 field paths, retained Sienna-shaped steering branches, and selected CEDAE consumers; every function is raw-body/C SHA-bound | `tools/extract_variant_function_evidence.py` from disposable H corpus | consumed only after exact H image/body/hash validation |
| `data/generated/corolla_8965H1202000_fd_control_reference_census.json` | Complete exact-substring direct-reference/source-writer census over the disposable H decompiler corpus for B6 staging/snapshots, AE12/AE20 branch cells, and FD030 sources; explicitly bounded against computed-pointer accesses | `tools/extract_variant_decompiler_reference_census.py` | `tests/verify_corolla_8965H1202000_fd_control.py` via generated report |
| `data/generated/corolla_8965H1202000_fd_control_interface.json` | Deterministic FD/control report: shared-vs-H-only FD Rx generation, complete B6 scalar map/consumer roles, AE12/AE20 correction, and FD030 direct field/source writer map | `tools/build_corolla_h_fd_control_interface.py` | `tests/verify_corolla_8965H1202000_fd_control.py` regenerates exactly |
| `data/generated/sienna_8965B4512000_steering_supervisor_structural_evidence.json` | Compact raw-body-bound structural fingerprints for the Sienna `CB86E` supervisor root and its 94 direct stages | `tools/extract_variant_structural_evidence.py` from disposable/canonical structural export | consumed by the H supervisor stage-ledger generator |
| `data/generated/corolla_8965H1202000_steering_supervisor_structural_evidence.json` | Compact raw-body-bound structural fingerprints for H `CEDAE` and all 123 direct stages | `tools/extract_variant_structural_evidence.py` from disposable H structural export | consumed by the H supervisor stage-ledger generator |
| `data/generated/corolla_8965H1202000_steering_supervisor_insertions_decompiler_evidence.json` | Compact H-native decompilation evidence for `CEDAE` plus every H-order-unpaired stage used for bounded semantic classification | `tools/extract_variant_function_evidence.py` | stage-ledger semantic pins + raw body hashes |
| `data/generated/corolla_8965H1202000_steering_supervisor_stage_ledger.json` | Complete evidence-graded 94→123 steering-supervisor direct-stage alignment with 83 pairs, 40 H insertions, 11 S removals, and bounded role classes | `tools/build_corolla_h_steering_supervisor_stage_ledger.py` | `tests/verify_corolla_8965H1202000_steering_supervisor.py` regenerates exactly |
| `data/generated/corolla_8965H1202000_secoc_key_provenance_decompiler_evidence.json` | Compact H-native evidence for SecOC config get/set/init, CryptoIf begin/update/finalize/dispatch, ICU command7 prepare/execute, disabled slot4 KAT, and command8 update path; every function raw-body/C SHA-bound | `tools/extract_variant_function_evidence.py` from disposable H corpus | consumed only after exact H image/body/hash validation |
| `data/generated/corolla_8965H1202000_secoc_key_provenance.json` | Deterministic profile→config/job→ICU-slot provenance, command8 refresh boundary, disabled-KAT corroboration, and existing exhaustive DataFlash raw-key negative | `tools/build_corolla_h_secoc_key_provenance.py` | `tests/verify_corolla_8965H1202000_secoc_key_provenance.py` regenerates exactly |
| `data/generated/corolla_8965H1202000_supervisor_external_ingress_census.json` | Exact-image/corpus provenance census for all scalar generated-COM fields reaching the H `CEDAE` supervisor cone, normalized by wire shape against Sienna and raw-body-bound at each cited consumer/unpacker; fails extraction if any non-B6 changed field or changed >=12-bit scalar survives | `tools/extract_corolla_h_supervisor_external_ingress_census.py` | `tests/verify_corolla_8965H1202000_supervisor_external_ingress.py` validates immutable-image hashes and closure predicates |
| `data/generated/corolla_8965H1202000_system_orchestration_decompiler_evidence.json` | Compact target-native H evidence for the recovered system/scheduler roles plus supporting regenerated COM/RTE copy fragments; contiguous application functions carry raw-body/C hashes, while reset `0x1F2` is explicitly represented by non-contiguous decompilation plus fixed raw windows | `tools/extract_corolla_h_system_orchestration_evidence.py` from disposable H corpora | `tests/verify_corolla_8965H1202000_system_orchestration.py` validates all tracked raw-body/decompiler/window hashes |
| `data/generated/corolla_8965H1202000_system_orchestration.json` | Deterministic Sienna↔H high-level orchestration comparison: eight scheduler/system role mappings, exact mode event schedules, 74→64 per-tick guard delta, wrapper/startup chains, and bounded fragmented COM/RTE surface | `tools/build_corolla_h_system_orchestration.py` | `tests/verify_corolla_8965H1202000_system_orchestration.py` regenerates exactly |
| `data/generated/corolla_8965H1202000_static_coverage_matrix.json` | Evidence-graded denominator over all 1,113 named canonical CodeFlash functions: exact transfer, inspected unique-shape, target-native role recovery, complete target-surface recensus, structural-only, or genuinely unresolved; H-only inspected additions counted separately | `tools/build_corolla_h_static_coverage_matrix.py` | `tests/verify_corolla_8965H1202000_static_coverage.py` regenerates exactly and enforces promotion discipline |
