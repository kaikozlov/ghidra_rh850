...
| LTA/angle command | protected `0x131` path recovered and converges with torque mode | active queue lacks `0x131`; protected `0x0B6` signal255 is target angle, signal254 is the OEM request selector, receiver loss is 7 foreground ticks, and signal261 is modulo-64 sequence state | module topology is `FRC_P5` 498 + `ABS_P5`/Brake-EPB 435 + `EMPS_P5` 405; TMS-051 identifies Brake System Control/category-435 as the immediate authenticated B6 source family, but the six local `0792` FRC images are opaque and no `07B0` Brake image exists, so code-level origin/forwarding, wall-clock cadence and SecOC signing/freshness ownership still require decoded producer firmware or synchronized stock-LTA traffic; do not transplant old `0x131` wire scaling |
...
| Techstream producer probes | older diagnostic controls are only contextual | FRC fixed vibration routines and category-435 ABS/Brake Active Tests are cataloged; the latter are brake-actuator-only and expose no named steering setpoint writer. TMS-051 also finds no named FRC/ABS Target-Lateral/Target-Steering data-monitor carrier; the exact request-ID dictionary remains steering-observer-side | use these as capture/probe triggers only; current-corpus static sender search is exhausted, and normal B6 payload production now specifically requires decoded `07B0` Brake + matched `0792` FRC firmware or synchronized traffic |
