...
A related Panda distinction now explains YC's `allOutput` observation: unlike ELM's
8-byte diagnostic-only transmit hook, debug-only `SafetyModel.allOutput` uses
`CAN_MODE_NORMAL` and permits arbitrary CAN TX, so `allOutput + logical bus 1`
admits a 32-byte `0x0B6` CAN-FD transmit toward the unmodified stock CAN1 wires
when the bus is configured for the target FD timing. That proves a useful static
direct-injection path, **not** a retained live-B6 result or a software replacement
for the physical repin: CAN1 remains unsplit and the stock producer cannot be electrically removed.
Therefore "parse/send on bus 1" is not itself a production architecture. Exact H/F
...
- Corrections with this document as canonical home: [CORR-108](../reference/index.md#correction-corr-108), [CORR-118](../reference/index.md#correction-corr-118)
