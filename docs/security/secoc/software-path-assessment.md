...
>
> **Question:** Can attacker-controlled software behavior reach unused ICU-S
> commands, expose slot 4, or provide an equivalent signing/key-recovery capability
> without physical fault injection or side-channel analysis?
>
...
6. unused ICU-S command characterization only after a software foothold and an
   independently verifiable known-`RAM_KEY` baseline exist.
...
| Direct unused command | retained application helper | characterize candidate commands 9/10 without persistent mutation | audited static harness; hardware result pending | command 9 is one-input/no-output and must pass selector-E CMAC KAT before command 10; command 10 is zero-input/seven-output and must pass independent SHE export KAT |
...
| Candidate commands 9/10 | constructible retained application foothold | identify Renesas volatile RAM-key load/export ABI | **bounded mapping; exact-F33 KAT harness prepared** | firmware command 8/11/0x22 shapes anchor SHE LOAD_KEY/INIT_RNG/GET_ID; dynamic known-answer result is still required |
...
#### Application-context volatile RAM_KEY probe (current first unused-command discriminator)

The earlier one-shot bootloader/command-13 concept is superseded. It guessed a
selector-bearing four-output contract for command 13 even though standard SHE
`CMD_EXPORT_RAM_KEY` has zero inputs and seven outputs. It also tested the wrong
lifecycle first: the exact F33 already has a live-qualified retained application resident
and stock ICU-S interrupt/completion machinery.

The current exact-F33 probe therefore stays in initialized application context and uses
two byte-audited low-RAM helpers:

1. candidate command `9`: one known 128-bit input block (`00..0f`), zero outputs;
2. require clean engine status and then use the stock serialized command-5 wrapper with
   selector `0xE` to reproduce a full 128-bit known CMAC under that exact test key;
3. disarm/reload only the low-RAM helper, without resetting ECU/HSM state;
4. candidate command `10`: zero input blocks, exactly seven output blocks (112 bytes);
5. accept `EXPORT_RAM_KEY` semantics only when the result satisfies independent SHE
   structure/KDF checks (`M1 == M4[:16]`, ID nibble `E0`, known `K3` `M4*`, known
   `K4` `M5`).

Both helpers require the stock asynchronous ICU-S driver to be idle before direct command
submission, use the stock FIFO/finalizer callbacks, have bounded timeout+abort recovery,
and never issue command 8 or write flash. The only key-state mutation is volatile
`RAM_KEY`, which remains overwritten until reset/power-cycle.
...
#### Recovered command-shape anchors and register-map hypothesis

The exact firmware now gives three independent SHE-shaped anchors at the CPU/ICU-S
boundary:

- command `8`: four input and three output blocks, already traced as the
  `M1/M2/M3 -> M4/M5` authenticated key-update path (`CMD_LOAD_KEY` shape);
- command `11`: no input or output FIFO callbacks (`CMD_INIT_RNG` shape); and
- command `0x22`: one input block and two output blocks (`CMD_GET_ID` shape).

Renesas support publicly describes ICU-S as a fixed state machine supporting SHE. If the
local register encoding is contiguous between command 8 and command 11, commands 9/10
are therefore the strongest candidates for `CMD_LOAD_PLAIN_KEY` and
`CMD_EXPORT_RAM_KEY`. Static shape alone does **not** establish the numbering; the
cryptographic KAT above is the discriminator.

This also removes the practical motivation for the old command-13 substitution paths.
The prior direct command-13 helper and Car-Kit launcher have been removed rather than
left as a misleading key-export experiment. Command 13's exact Renesas semantic can
remain unknown without blocking slot-4 work: standard SHE has no nonvolatile-slot export
or copy-to-RAM_KEY operation, and the useful slot-4 primitive is command 5.

External semantics:

- Renesas Engineering Community: ICU-S is a fixed state machine that supports SHE,
  <https://community.renesas.com/mcu/rh850/f/rh850-rl78f/17819/rh850vp1m-x-icum-secure-documents>.
- AUTOSAR *Specification of Secure Hardware Extensions*: `CMD_LOAD_PLAIN_KEY` accepts
  one 128-bit plaintext key; `CMD_EXPORT_RAM_KEY` emits M1..M5 (112 bytes) only for a
  plaintext-origin RAM_KEY,
...
1. Test the already-recovered **selector-4 command-5** path in normal application
   context; this is the direct signing-oracle question for the provisioned SecOC key.
2. Run the exact-F33 candidate command-9/10 RAM_KEY KAT to close the vendor register
   ABI without touching any persistent key.
3. If command 5 rejects, retain command 7 as the chosen-input physical SCA surface and
   characterize command 1 only as a secondary oracle.
4. Treat any future persistent-slot copy/export claim as a Renesas-specific extension
   requiring direct dynamic/manual proof; do not infer it from SHE or RAM-key export.
5. Validate any recovered 16-byte slot-key candidate against multiple stock SecOC
   frames; a MAC oracle or SHE export envelope is not plaintext-key recovery.
