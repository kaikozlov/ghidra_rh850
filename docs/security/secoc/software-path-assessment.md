...
| Service `0xAB` / `0xBA` | UDS requests | event disclosure / proprietary async operation | bounded | `0xAB` is event-record subfunction service; `0xBA` separately owns operation-F1 callback `0x8D344`; neither bounded closure hits selected crypto/ICU targets |
...
- The complete `FEBE508F` writer set remains startup clear `0x68006`, activator
  set-to-1 `0x69026`, and terminal finalizer write `0x68D32`. Because the
  activator only arms state zero and the finalizer leaves `0x02`/`0xFF`, a fresh
  application boot is required for a deterministic new bank-1 run.
- Once active, CAN inputs can select a software key selector, 16-byte message,
  ...
  exposes pass/fail state; no stock byte-output transport was recovered.
