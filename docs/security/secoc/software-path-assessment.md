...
| Diagnostic crypto-test bank | WDBI DID `0x100E/0x100F` plus CAN `0x01B..0x01F` | command 5/7 oracle and output | stock activation recovered; byte-output transport absent | WDBI action table `0x25804` row 8 points to wrapper `0x8A782`, which directly calls bank-1 activator `0x69018`; selector 1 is enabled, has zero data fields, policy 0 has no SecurityAccess levels, and the stock bank still compares generated bytes locally |
...
| crypto-test inputs `0x01B..0x01F` | normal PDU route | bank collectors after diagnostic activation | bank 1 is armed by WDBI DID `0x100F` selector 1 |
...
- `crypto_test_bank1_activate` (`0x69018`) initializes active byte `FEBE508F`,
  ...
  direct-xref census missed its stock diagnostic caller because the WDBI table
  points one hop earlier to wrapper `0x8A782`, not directly to `0x69018`.
- Application WDBI DID `0x100F`, selector 1, reaches action wrapper `0x8A782`,
  which directly calls `0x69018`. The DID is enabled, selector 1 consumes zero
  data fields, policy index 0 has zero SecurityAccess levels and allows session
  records 1/2/3; the outer SID-`0x2E` service gate leaves programming/extended
  as the effective sessions. The stock activation request is therefore
  `2E 01 10 0F` after entering an allowed session.
...
The missing edge is a one-hop WDBI callback:

- the 19×12-byte callback table at `0x25804` has DID `0x100F` in row 8 and action
...
- the paired precondition callback `0x8A768` is an unconditional success stub;
- selector 1 is enabled for DID `0x100F` and has zero configured data fields;
- policy index 0 has zero SecurityAccess-level entries and three diagnostic
  session records (1, 2, 3); and
- the outer application SID-`0x2E` gate permits programming/extended sessions.

Consequently `2E 01 10 0F` is a stock diagnostic bank-1 activation request in
an allowed session. CAN `0x01B..0x01F` still cannot arm the bank *by themselves*,
...
Ghidra rebuild now seeds both WDBI callback columns so the wrapper and its direct
call are part of the persistent recovered graph.
...
The useful **static software** questions in this report are now mostly closed. Repository-known gate material still provides arbitrary bootloader-context execution, and the application has a fully specified serialized command-5 path. Stock WDBI DID `0x100F` supplies diagnostic activation for bank 1; the remaining stock limitation is that generated bytes at `FEBE51AA` are compared locally rather than returned. The ICU result path has no recovered software-controlled stale-output disclosure. Hardware-only behavior remains separate: live slot-4 command-5 permission, undocumented ICU commands, physical leakage, and fault-induced sequencing require dynamic work rather than another broad static pass.
