...
### Is one operational SecOC key shared across participating ECUs?

For the recoverable Toyota Security/SecOC family represented by Willem's public
extractor and the later Sienna work, **yes at the vehicle security-domain level**.
This is no longer merely a cryptographic design possibility. The pinned Toyota
openpilot sender loads one `SecOCKey` and uses that same AES-128 value for
`STEERING_LKA` (`0x2E4`), `STEERING_LTA_2` (`0x131`), and `ACC_CONTROL_2`
(`0x183`), with independent freshness/message counters rather than independent
keys. The independently published `8965B4514000` field recovery starts from an
EPS DataFlash candidate and validates that one candidate against `0x131`
`226/226`, `0x2E4` `225/225`, and `PRE_COLLISION_2` (`0x344`) `112/113`, crossing
steering and PCS/AEB message domains.

The service lifecycle corroborates the same architecture. The pinned optskug
Toyota-Security record says replacement of a SecOC participant can cause the
security key to be replaced on **all parts of the car**, rather than only loading
the old value into the new part. Our independent Techstream V18 recovery likewise
shows MACKey Registration discovering a master plus multiple slaves and applying
server-returned M1/M2/M3 update packages to every selected endpoint. Those update
packages can differ per endpoint because they are wrapped for each ECU identity;
that does not imply different final operational SecOC keys.

Keep this distinct from **cross-vehicle key reuse**. The public extraction tooling
handles `SecOCKey` as vehicle-specific secret material, and a dealer rekey changes
the operational key. Current evidence therefore supports one shared operational
key across participating secured ECUs/messages *within a vehicle security domain*;
it does not establish that two different Siennas, or any H/F Corollas, have the
same key. Cross-vehicle/calibration/model/region/fleet uniqueness remains OQ-016.
...
| recoverable Toyota/Sienna SecOC uses one operational vehicle-domain key across steering/longitudinal/PCS protected traffic and participating rekey endpoints | **Observed/recovered across pinned external artifacts and Techstream** |
| the same operational SecOC key is reused across different vehicles | **Unknown; OQ-016** |
...
- Findings with this document as canonical home: [SECOC-005](../../reference/index.md#finding-secoc-005), [SECOC-009](../../reference/index.md#finding-secoc-009), [SECOC-010](../../reference/index.md#finding-secoc-010), [SECOC-076](../../reference/index.md#finding-secoc-076), [STORE-007](../../reference/index.md#finding-store-007)
- Corrections with this document as canonical home: [CORR-002](../../reference/index.md#correction-corr-002), [CORR-003](../../reference/index.md#correction-corr-003), [CORR-005](../../reference/index.md#correction-corr-005), [CORR-006](../../reference/index.md#correction-corr-006), [CORR-010](../../reference/index.md#correction-corr-010), [CORR-011](../../reference/index.md#correction-corr-011)
