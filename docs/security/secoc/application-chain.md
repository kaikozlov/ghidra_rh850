...
and output-length pointer, and returns up to 16 generated bytes. The only
configured caller is an application crypto-test harness; static analysis has not
proved that slot 4 authorizes generation on the donor or that the harness is
reachable with attacker-chosen inputs in normal vehicle operation.

`../tests/verify_secoc_application.py` checks the configuration, routing,
disabled KAT gate, command-5/command-7 driver families, key-slot selection,
freshness/MAC profile, and object-15 state directly from the committed images.
...
- no configured call hard-codes selector 4 into command 5; the sole caller takes
  the selector from runtime RAM populated by generated application input;
- static CodeFlash cannot show whether a provisioned slot 4 permits generation,
  whether the harness is operationally enabled, or whether a bootloader-context
  payload can use the ICU-S state without application initialization.
...
3. In application context, submit a chosen message through command 5 with
   selector 4, collect the 16-byte result, and feed that result to command 7 with
   the same selector/message. A successful generate/verify round trip proves
   operation compatibility without exposing the key.
4. Validate any object-15 candidate `K`, and any command-5 output, against
...
| Command 5 is configured to use slot 4 in this calibration | **Unsupported** |
