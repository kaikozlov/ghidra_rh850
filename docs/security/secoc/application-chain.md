...
CAN `0x01B..0x01F`. Stock diagnostics can arm bank 1 first: WDBI DID `0x100F`
selector 1 dispatches through wrapper `0x8A782`, which directly calls
...
five COM update counters. It is reached by stock application WDBI DID `0x100F`:
the action table points to wrapper `0x8A782`, whose call at `0x8A786` directly
invokes the activator. Selector 1 has zero data fields, and its policy has no
SecurityAccess-level requirement; after the outer SID-`0x2E` session gate, the
request is `2E 01 10 0F` in programming or extended session. Ordinary receipt
...
| Stock diagnostics can activate bank 1 with WDBI `2E 01 10 0F` via `0x8A782 -> 0x69018` | **Recovered / verified static route (SECOC-040 corrected)** |
