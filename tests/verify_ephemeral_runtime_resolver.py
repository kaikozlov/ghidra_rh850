...
WRAPPER = REPO / "tools/security/resolve_ephemeral_runtime_image.sh"
...
check("wrapper uses canonical disposable work project, not committed project", "source \"$ROOT/tools/lib/build_paths.sh\"" in wrapper and "$BUILD_WORK/ephemeral-runtime-targets" in wrapper and 'ResolveSecocAcceptanceGate.java' in wrapper and 'ResolveEphemeralRuntime.java' in wrapper and "$ROOT/projects" not in wrapper)
