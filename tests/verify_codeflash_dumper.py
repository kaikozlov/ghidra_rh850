...
check("read payload waits for full stock Tx-ready mask before message RAM writes", ready_wait < message_write)
check("read payload waits for a completion result only after TMTR", submit < result_wait < status_clear)
check("read payload accepts encoded RSCFD results 2/3 via bit-2 success mask", "tx_result_success_mask 0x04u" in dumper_c and "(status & tx_result_success_mask) != 0u" in dumper_c)
check("read payload no longer treats encoded RSCFD result 1 as success", "tx_result_success      0x02u" not in dumper_c)
check("read payload bounds ready/completion waits and result retries", all(token in dumper_c for token in ("tx_spin_limit", "tx_result_retries", "transport_fault_halt")))
