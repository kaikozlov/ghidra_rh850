...
current_row = _panda_row((0x1FE00002, bytes.fromhex("ff20000808000101"), 0))
legacy_row = _panda_row((0x1FE00002, 1234, bytes.fromhex("ff20000808000101"), 0))
check("current Panda three-field tuple is accepted", current_row == (0x1FE00002, None, bytes.fromhex("ff20000808000101"), 0))
check("legacy wider Panda tuple retains raw busTime", legacy_row == (0x1FE00002, 1234, bytes.fromhex("ff20000808000101"), 0))
...
check("live mode is refused because stock protocol dispatch is disabled", unsafe.returncode != 0 and "fixed CodeFlash gate 0x30D68=0x5A" in unsafe.stderr)
live_ack = subprocess.run([sys.executable, str(tool), "--profile", "source-terms", "--execute", "--stock-observation-confirmed", "--capture-output", "/tmp/should-not-exist.ndjson"], cwd=REPO, capture_output=True, text=True, check=False)
check("acknowledged live mode remains refused", live_ack.returncode != 0 and "fixed CodeFlash gate 0x30D68=0x5A" in live_ack.stderr)
