...
check("fault readiness role carries exact new closures", all(x in roles["EPS readiness / steering faults"]["tss3_corolla_evidence"] for x in ("B6[2]", "6000/6000", "C159B49", "force-7 override", "17-state", "deepest recovered clear/normal classifier path", "0x51E B0[7]", "0x1033 Ready Status", "Ready=1", "power-supply receive-validity/freeze gate", "distinct from B6 loss", "B6[3]/B10[3]/B13[4]", "cannot represent exact cooperative authority")))
...
check("cruise role has exact P5 Data-ID semantics/direct RDBI transport but no CAN join", all(x in roles["cruise availability / set speed / ACC faults / follow distance"]["tss3_corolla_evidence"] for x in ("0x1905", "0x1906", "0x1914", "0x1901", "0x1912", "ordinary SID 0x22 RDBI requests")) and all(x in roles["cruise availability / set speed / ACC faults / follow distance"]["remaining_blocker"] for x in ("direct FRC SID-0x22", "outer-session prerequisite")))
