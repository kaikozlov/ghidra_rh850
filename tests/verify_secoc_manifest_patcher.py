...
result_check = runtime_c.index("tx_result_success_mask", status_clear)
check("telemetry waits for the full stock Tx-ready mask before message RAM writes", ready_wait < message_write)
check("telemetry polls completion only after setting TMTR", submit < result_wait)
check("telemetry clears completion status before checking encoded result", result_wait < status_clear < result_check)
check("telemetry accepts encoded RSCFD results 2/3 via bit-2 success mask", "tx_result_success_mask   0x04u" in runtime_c and "(status & tx_result_success_mask) == 0u" in runtime_c)
check("telemetry no longer treats encoded RSCFD result 1 as success", "tx_result_success        0x02u" not in runtime_c)
