...
print('\n== source identity ==')
check('schema v2',d['schema']=='corolla-8965H1202000-b6-target-angle-ingress-v2')
...
check('raw 25/27 special pair is bounded','raw IDs 25 (0x19) and 27 (0x1B)' in profiles['additional_raw_id_use'] and 'only 25' in profiles['additional_raw_id_use'])
check('signal254 naming boundary retained','Exact Toyota feature labels' in mode['boundary'] and 'corroborating family vocabulary' in mode['boundary'])
...
check('next target is profile labels then upstream producer','Toyota feature labels/behavior' in c['next_static_target'] and 'FRC_P5 -> Brake/EPB' in c['next_static_target'])
