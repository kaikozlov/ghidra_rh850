...
print('\n== carrier / mailbox MPU geometry ==')
g=a['carrier_geometry']; m=a['mailbox_geometry']
check('carrier exact 776-byte pocket',g['base']=='0xFEBF0000' and g['end_inclusive']=='0xFEBF0307' and g['end_exclusive']=='0xFEBF0308' and g['size']==776 and g['first_recovered_normalized_direct_or_simple_gp_reference']=='0xFEBF0308')
check('carrier region5 B8/B8',g['mpu_region_index']==5 and g['mpu_bounds']==['0xFEBEF400','0xFEBF33FC'] and g['ctx0_mpat']==g['ctx1_mpat']=='0x000000B8')
check('carrier absence claim is bounded',all(x in g['static_boundary'] for x in ('direct/simple-GP','computed aliases','DMA','live canary')))
check('mailbox exact 60-byte span',m['base']=='0xFEBFFB80' and m['end_inclusive']=='0xFEBFFBBB' and m['end_exclusive']=='0xFEBFFBBC' and m['size']==60 and m['normalized_direct_or_simple_gp_reference_count']==0)
...
check('canary exact audited build',can['size']==CANARY_BIN.stat().st_size==334 and can['headroom']==442 and can['sha256']==sha(CANARY_BIN.read_bytes())=='facd4f590581f7422dab0fc4fcea21f6d73e4c361b1f4d54960d7001e89bdbb0' and can['entry_offset']==can['relocations']==0 and can['command5_calls'] is False)
check('proxy exact audited build',prox['size']==PROXY_BIN.stat().st_size==464 and prox['headroom']==312 and prox['sha256']==sha(PROXY_BIN.read_bytes())=='0ea9b9d460c3678ad4341817ae606d720bb2a13f4d14ec7dc1e0c8f569db94d3' and prox['entry_offset']==prox['relocations']==0 and prox['input_length']==36 and prox['key_selector']==4)
...
check('static candidate closed only',z['static_target_native_carrier_candidate_closed'] and not z['verified_variant_ram_exec_requirement_promoted'])
check('Camry absent from verified RAM geometry',not any('camry' in x or '8965f3307000' in x for x in variants))
check('retention/slot4/latency remain open',not z['live_retention_closed'] and not z['live_slot4_command5_permission_closed'] and not z['command5_latency_jitter_closed'])
...
check('live sequence starts with inert canary and reset-to-stock', [x['stage'] for x in a['live_sequence']]==[1,2,3,4] and 'canary' in a['live_sequence'][0]['name'] and 'reset-to-stock' in a['live_sequence'][1]['name'])
