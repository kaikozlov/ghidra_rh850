...
```bash
uv sync --locked
tools/test <suite-or-prefix>   # run only what exercises the work you changed
tools/test list [query]        # discover suites
tools/test plan <query>        # preview an explicit suite/prefix/group
tools/test core                # deliberate small repository-mechanics smoke
```

Documentation/provenance edits do not require a test run. `full`, `local`,
processor, and external-corpus sweeps are milestone/debugging tools, not the RE
edit loop.
