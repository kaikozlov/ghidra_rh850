...
The current generated `build/out/camry-f33-car-kit/` is **v5**. Its first live RAM action is the ABI-preserving exact-F33 runtime/source-term discriminator: a 534-byte staged shell copies a 406-byte resident into the live-proven `FEBFF9F0..FEBFFBFB` high tail, uses direct RH850 `JARL disp32` for the exact stock startup/foreground target sequence, requires application F181 before any added observer write, then snapshots the canonical D0218 source terms to `FEBF0000..FEBF0024` and SHA-attests the resident. After a positive NRTD result, the same resident is read-only sampled after a direct NRTD->READY transition while Park/stationary; no second RAM execute occurs. The older C-based B6 observer/bridge remain packaged as reproducibility artifacts but are explicitly **not live-qualified**; their shared `call0(address)` startup trampoline is not accepted as ABI-correct evidence. The stage-2/stage-3 flash package remains **historical/recovery only**; current installed firmware is persistence-verified cumulative stage 5. Source verification is `tests/verify_camry_f33_b6_stationary_probe.py` (including the ABI-preserving runtime discriminator contract).
