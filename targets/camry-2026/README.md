...
The historical CONNECT-only probe sent standard `0x7F7` and observed no usable
`0x7F8`; VAR-134/CORR-165 supersede its interpretation. Exact F33 rule46 is
classic extended `0x1FDC0002` with response `0x1FE00002`. A 2026-09-06 parked
non-command marker on that endpoint live-reached XCP staging `FEBE4C34`, but fixed
CodeFlash `0x30D68=0x5A` blocks protocol dispatch before CONNECT. Stock exact-F33
XCP command reachability is therefore closed negative, not a route/session unknown.
The direct live marker evidence is tracked as
`raw-20260906/xcp-extended-ingress-probe.json`; its payload is deliberately
non-command and the record pins the before/after SID23 staging bytes and Panda
TX-block delta.

The deterministic compact interpretation is
`data/generated/camry_2026_tsk_baseline.json`, generated by
`tools/targets/camry/analysis/analyze_camry_2026_baseline.py` and checked by
...
```bash
python3 tools/targets/camry/builders/build_camry_f33_car_kit.py
```

The current generated `build/out/camry-f33-car-kit/` is **v7**. Its first live RAM action is now the generic ABI-preserving exact-F33 runtime monitor, not a hard-coded steering observer. A 648-byte staged shell copies a 520-byte resident into the live-proven `FEBFF9F0..FEBFFBFB` high tail. The resident preserves the corrected direct `JARL disp32` Toyota startup/foreground semantics, performs no added application-memory write before foreground count 224, then exposes eight host-configurable 4-byte LocalRAM watch windows. Host commands use the already-live-proven extended-CAN ingress `0x1FDC0002 -> FEBE4C34` with a deliberately non-XCP `00 F3 seq opcode arg32-le` frame; observation results remain read through application SID `0x23`. After one positive NRTD install (`runtime_monitor_live`), transition directly NRTD->READY without OFF and use `status/watch/run/stop/snapshot/capture/shell` without rebuilding or another RAM execute. The older source-term discriminator, C-based B6 observer, and route44 bridge remain packaged as reproducibility artifacts but are not the live path. The stage-2/stage-3 flash package remains historical/recovery only; current installed firmware is persistence-verified cumulative stage 5. Source verification is `tests/verify_camry_f33_b6_stationary_probe.py`, which also pins the generic monitor protocol/build contract. The kit root now exposes the canonical `./f33` launcher: run `./f33 doctor` while the car is OFF, then field work uses only `./f33 install`, `./f33 status`, `./f33 watch ...`, and `./f33 shell`; it owns cwd, `/usr/local/venv/bin/python`, `PYTHONPATH`, fixed payload paths, and Panda-owner shutdown.
...
Exact firmware analysis originally identified XCP write callbacks as a candidate
placement surface, but VAR-134/CORR-165 now close stock XCP command ingress disabled.
The verified high-tail carrier remains; production placement plus execution-pivot
discovery are both open. Canonical interpretation is
...
is joined directly. Reproduce with `tools/targets/camry/analysis/analyze_camry_2026_cruise_lta_edges.py`; the
...
`tools/targets/camry/extract/extract_camry_frc_lta_rlog.py OUT SEGMENT=RLOG ...` privacy-minimizes the
...
`tools/targets/camry/analysis/analyze_camry_frc_lta_capture.py OUT` performs the stable-interval join,
...
`tools/targets/camry/live/camry_frc_lta_capture.py` remains an exclusive-Panda fallback for controlled
...
```bash
tools/targets/camry/live/camry_tss3_request_capture.py --execute --out captures/oq052_$(date +%Y%m%d_%H%M%S)
tools/targets/camry/analysis/analyze_camry_tss3_request_capture.py captures/oq052_* --json summary.json
```
...
The deterministic reducer is `tools/targets/camry/analysis/analyze_camry_f33_runtime_monitor_20260908.py` and
