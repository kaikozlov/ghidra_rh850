#!/usr/bin/env python3
"""Exact-target 2026 Camry EPS CodeFlash acquisition using Calvin's range payload.

The memory result is read-only CodeFlash. The acquisition mechanism enters the EPS
bootloader, performs one boot SecurityAccess unlock, writes the bootloader's temporary
0203/0201/0202 payload state, downloads a 4 KiB authenticated RAM payload, verifies it
with 0x10F0, then invokes the 0xFF00 callback route. It never sends a CodeFlash
programming payload and refuses any application identity/route other than the retained
8965F3307000 Camry specimen.
"""
...
EXPECTED_PAYLOAD_SHA256 = "860f8a3418d23ccfd0861a97efdb9e1d23a8854c3a629b8d7b6821eb93d0b588"
TX = 0x7A1
RX = 0x7A9
BUS = 1
ELM327_PARAM = 1
...
LOAD_SIZE = 0x1000
CODEFLASH_START = 0x00000000
CODEFLASH_END = 0x00200000
...
def main() -> int:
    ...
    ap.add_argument("--out-dir", type=Path, default=Path("/cache/tsk/camry-codeflash"))
    args = ap.parse_args()

    payload = args.payload.read_bytes()
    if len(payload) != LOAD_SIZE or sha256(payload) != EXPECTED_PAYLOAD_SHA256:
        ap.error("Calvin CodeFlash payload hash/size mismatch")

    args.out_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    base = args.out_dir / f"camry_8965F3307000_codeflash_{stamp}"
    run_path = base.with_suffix(".run.json")
    dump_path = base.with_suffix(".bin")
    coverage_path = base.with_suffix(".coverage.bin")

    report = {
        "schema": "camry-8965f3307000-codeflash-acquisition-v1",
        "started_at": utc_now(),
        "target": {
        ...
        "payload": {
            ...
            "size": len(payload),
            "source": "calvinpark-openpilot dump branch payload_codeflash_00000000_00200000.bin",
            "codeflash_range": ["0x00000000", "0x00200000"],
        },
        "stages": [],
        "result": {},
    }
    ...
    try:
        ...
        try:
            ...
            direct = bytes(boot.read_memory_by_address(CODEFLASH_START, 0x10))
            stage("boot SID 0x23 codeflash probe", "accepted", data_hex=direct.hex())
        except NegativeResponseError as e:
            stage("boot SID 0x23 codeflash probe", "rejected", nrc=f"0x{int(e.error_code):02x}")
        except (InvalidServiceIdError, MessageTimeoutError) as e:
            stage("boot SID 0x23 codeflash probe", "rejected", detail=type(e).__name__)
        ...
        total = CODEFLASH_END - CODEFLASH_START
        words = total // 4
        ...
        while True:
            ...
            if now_mono - started > MAX_SECONDS:
                stage("CodeFlash stream", "timeout", seconds=round(now_mono - started, 3))
                break
            made_progress = False
            try:
                ...
                if consecutive_spi_errors >= 100:
                    stage("CodeFlash stream SPI", "failed", errors=spi_errors, detail="100 consecutive SPI errors")
                    break
                continue
            for addr, data, bus in batch:
                ...
                if (word0 & 0xFF) != 0x07:
                    continue
                address = (word0 >> 8) & 0xFFFFFF
                if address < CODEFLASH_START or address + 4 > CODEFLASH_END or (address & 3):
                    continue
                idx = (address - CODEFLASH_START) // 4
                word = data[4:8]
                off = idx * 4
                if seen[idx]:
                    duplicate_words += 1
                    if image[off:off + 4] != word:
                        conflicts += 1
                        stage("CodeFlash stream", "failed", detail="conflicting duplicate", address=f"0x{address:08x}")
        ...
        report["result"] = {
            ...
            "dump_size": len(image),
            "codeflash_start": f"0x{CODEFLASH_START:08x}",
            "codeflash_end": f"0x{CODEFLASH_END:08x}",
        ...
        stage("CodeFlash stream", "complete" if complete else "partial", unique_words=unique_words,
        ...
        try:
            ...
            stage("post-dump application identity", "observed", observed_hex=post_f181.hex())
        except Exception as e:
            stage("post-dump application identity", "unavailable", detail=type(e).__name__)
