#!/usr/bin/env python3
"""crown_f30_sideband_preflight.py + a UDS TesterPresent (SID 0x3E) heartbeat.

NOT part of Kai's original kit - a local troubleshooting variant. The
original passively observes CAN for `duration` seconds (default 5.0s, which
lines up exactly with the ISO 14229 default S3 session timeout) without
sending any UDS traffic, so the EXTENDED session silently reverts to default
and the closing SID23 read fails with "service not supported in active
session". This copy is byte-identical except it calls client.tester_present()
every ~2s during the observation window and once more immediately before the
closing read. Same read-only contract: no CAN application frame, no RAM/flash
write.
"""
...
def run(duration: float) -> dict[str, Any]:
    ...
    return {
        "schema": "crown-f30-sideband-preflight-v1-heartbeat",
